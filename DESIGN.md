...
3. **Local models (pi)** — a weaker model can't be trusted to roam the repo, so
...
6. **Incidents + error-class pages**, then lint agent, then listener/DG
   compactor profiles, then the claude/pi adapters, then Q&A polish.
