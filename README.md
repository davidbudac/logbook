...
| **Agents** (`harness.py`, `orchestrate.py`, `structured.py`) | say what a notable digest *means*, attach it to the database's history, write prose. Runs `claude`, `codex` or `pi` as a subprocess, agentic or structured (one JSON answer, deterministic code writes every file) | decide what happened, touch git, invent a citation |
...
- At least one agent CLI on `PATH`: `claude`, `codex`, or `pi` (which also
  drives a local model server). Only the adapters you configure need to exist.
