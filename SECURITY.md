...
- **The portal is an unauthenticated HTTP server.** `dbwiki portal serve`
  ...
  behind a reverse proxy that authenticates. It answers only to the bound
  `Host`, its loopback spellings and the names in `portal.allowed_hosts`
  (DNS-rebinding protection); `portal.trusted_proxies` names the proxies
  whose `X-Forwarded-Host` it believes. The same goes for the daily
