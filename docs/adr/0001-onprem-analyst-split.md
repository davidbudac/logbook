...
`stale_hours` (a crashed analyst — the request is re-claimable by hand-moving
it back to `pending/`; automating claim expiry is deferred until a real
occurrence), and the `failed/` count with error categories.
...
- Two writers on the wiki remote. Mitigated by path partitioning (analyst:
  ...
  same-day supersede semantics already accept.
