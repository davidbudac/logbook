...
- **Host names are learned from the documents (2026-09-23).** A bare
  one-label host (`on dbsrv77`) has no shape, and an all-caps dotted one
  (`DBSRV.CORPX`) has the shape of `SCHEMA.OBJECT`; only the vocabulary can
  know either. Rather than have the operator list them in `redact.terms`,
  the compactor records the host fields of every document it already reads
  (`host.name`, `host.hostname`, `agent.hostname`, listener/CRS diag-path
  hosts, `(HOST=…)` values, listener client hosts) in the per-db registry,
  and the vocabulary learns each with its first label and its domain as a
  suffix; a vocabulary host beats every shape heuristic. No extra ES query,
  no model. Guards keep it from eating research text (length, numbers,
  releases, a stoplist, source-domain labels, whole-word matching for
  letters-only names). What remains is a name no document has ever carried.
...
**Stale claims** (amended 2026-09-23): a claim older than
`research.exchange.stale_hours` (researcher: `stale_hours`; default 26, the
analyst queue's) goes back to `requests/pending/` with `attempts + 1`, or to
`failed/` (`stale_claim`) once attempts run out — `exchange.reclaim_stale`,
run by the researcher before every claim and by the on-prem round after its
pull. Until then a human had to move the file back. A reclaim is pushed at
once and dropped if the push fails, so the two sides running it together
move a claim once, never twice. 26h rather than something scaled to the
weekly cadence: a live claim lasts one agent run (`timeout_seconds`), and the
on-prem side folds results in weekly, so any budget under a week reaches the
same fold-in.
