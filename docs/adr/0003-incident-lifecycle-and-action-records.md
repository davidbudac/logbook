...
| `error_pages` | a resolve's `update_error_pages` choice; written only as `error_pages: false`, when the operator declined the error-page rows |
...
other four appear only when set. `error_pages` (added 2026-09-23) is a
boolean that defaults to true and is written only when false, so every
record written before it existed reads back unchanged; only a `resolve` may
carry `error_pages: false`, since no other kind writes error-page rows.
`backfill-resolutions` reads it and skips a resolve that declined the rows,
which it could not tell apart from a forgotten one before. A key outside that list is rejected rather
...
- `Resolve` also appends one `## Resolution history` row to each error page
  ...
  under the `update_error_pages` flag which defaults on (declining it is
  recorded on the record as `error_pages: false`). A linked error page
