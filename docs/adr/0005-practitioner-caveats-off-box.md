...
**Redacted and leak-checked anyway (2026-09-23).** "Public Oracle text" is
what the cause and action paragraphs are supposed to hold, not what code can
promise: a human edit or a researcher's paraphrase can put a host, a service
or a database name in them. So the prompt goes through the same outbound
path as ADR-0002's requests. `research_caveats.outbound_caveats_prompt`
builds the run vocabulary, redacts the cause and action, assembles the
prompt and runs the leak check over the whole of it. A leak fails closed:
the page is skipped before any web call, the run's flags carry
`redaction-leak: <page> not sent` (never the hits), the hits go to
`.state/redaction_leaks.jsonl` for `dbwiki health` to count, and
`research --caveats --dry-run` prints the same redacted prompt and exits 2
on a leak. Pseudonyms the model echoes back are de-mapped before the writer
sees them.
...
a redactor is only as good as its vocabulary. This prompt should carry no
estate data: it is three public strings and a list of blog domains,
assembled by code that never opens an incident page. It is still redacted
and leak-checked before it leaves (see above), so a page that does carry an
estate name is not sent rather than trusted.
