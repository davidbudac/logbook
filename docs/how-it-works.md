...
5. **Detect deltas** against the per-db registry (`.state/registry/<db>.json`,
   ...
   | `after_change` | an error or dataguard group's first occurrence lands within `after_change_hours` (2) of a lifecycle change on the same database — the gap is stated as a fact, so the model never has to infer cause itself |
...
natively), `claude` (`claude -p --output-format json`), `pi` (any provider
`pi` knows, including a local OpenAI-compatible server). Any other name fails
at config load, naming the supported adapters. Each adapter also has a usage parser, so
...
1. With `analyst.enabled` and `report.push` on, pull the wiki remote (rebase
   with autostash; the analyst's pushed report wins a same-path conflict; any
   failure is undone and noted, and the tick carries on). Then fold any
   analyst telemetry results into `.state/agent_runs.jsonl` (ADR-0001; no-op
   when the analyst queue is unused).
...
The daily HTML page is deterministic machine output, like digests: a
*Changes* strip first when anything changed (every database's `changes`, one
fleet timeline, capped at 25 rows), then tiered *Needs attention* (notable databases plus every still-open incident), *Worth a
...
The workbench draws the same strip over a day's report (the fleet's changes
that day) and over a digest (that database's), from the same
`daily_html.change_rows`, capped at the same 25.
