...
3. **Keys via environment** — preferred over keys in the config file, same
   ...
   `lint`/`research`/`analyst`/`review`, and `scores send` posts the queued
   human-feedback scores, so whatever launches those needs the vars. The
   portal never does.
...
- **Trace** `dbwiki-<task>` (`ingest`, `report`, `lint`, `research`, and
  `review` — the weekly review's synthesis call, exported since 2026-09-24 so
  an inbox decision has a trace to score; it belongs to no tick, so it has
  no session), with
...
- **Tags**: task, adapter (`codex`/`claude`/`pi`), model tier
...
- **One generation observation** named after the task: the exact prompt as
  ...
  `codex` runs carry token counts; codex reports no price, so
...
- **Human feedback scores**, added later by `dbwiki scores send` — see
  "Human feedback" below.
...
## Human feedback (issue 02)

`validation_ok` only says a proposal passed the deterministic checks. Whether
a DBA agreed with the page is the signal that makes "acceptance by model" or
"by prompt version" mean anything, so every operator act becomes a score on
the trace of the stage that wrote the page (`src/dbwiki/scores.py`).

**Queue, then send.** The portal never holds Langfuse keys. An act only
appends score records to `.state/pending_scores.jsonl` (same capped,
flock-guarded append as the other `.state` logs; a failure warns once and
never blocks or fails the act):

- every committed `dbwiki incident <verb>` and portal commit
  (`incident_action.publish`, both surfaces), when the page has a machine
  commit carrying a `Run-ID:` trailer;
- every review-inbox acknowledge/suppress that wrote (`review.acknowledge`,
  `review.suppress`; an idempotent repeat queues nothing).

`dbwiki scores send` (cron, hourly at :50, `scores-send` in
`config/schedule.json`) posts the queue with `create_score` and drops what it
handed over. It is a no-op — nothing read into Langfuse, nothing dropped —
without `langfuse.enabled` or without `LANGFUSE_PUBLIC_KEY`/`SECRET_KEY`.

**The scores.**

| act | `human_accepted` (BOOLEAN) | `human_action:<kind>` (NUMERIC, 1 per act) |
|---|---|---|
| `record-action`, `monitor`, `extend`, `resolve`, `reopen` | 1 | `human_action:record-action`, ... |
| `merge` (the page was a duplicate) | 0 | `human_action:merge` |
| inbox acknowledge | 1 | `human_action:acknowledge` |
| inbox suppress | 0 | `human_action:suppress` |

Every act that keeps working an incident page accepts it as a real incident;
a merge folds it away as a duplicate the tick should not have opened. On the
review, acknowledging a finding accepts it and suppressing it says "noise".
Metadata carries `source` (`incident`/`review`), `subject` (slug or finding
fingerprint), `kind`, `stage`, `run_id`, `event_id`, `surface`, `commit` —
identifiers only, never the actor or a page body.

**Which trace.** One `Run-ID` covers every stage of a tick, so:

1. take the newest commit on the page (at the base the DBA acted on) that
   carries a `Run-ID:` trailer — human commits carry none, so a second act
   still scores the tick behind the first;
2. its subject names the stage (`ingest: <db> <day> — ...`, `report`, ...);
3. the `.state/agent_runs.jsonl` line with that run id and task — narrowed by
   the db the subject names, then to the stage that validated — gives the
   `event_id`, and the score goes on `observability.trace_id(event_id)`;
4. when no single line answers (aged out of the capped ledger, or two
   candidates), the score goes on the Langfuse **session** of the run
   instead (`session_id` = `run_id`), still naming the stage in metadata.

A page no tick ever wrote queues nothing. An inbox decision scores the
synthesis stage of the newest review holding the finding (`task: review`,
same `review_id`); a review with no synthesis stage (disabled) queues nothing.

**Idempotency.** Each record's `key` is also the Langfuse `score_id`, which
Langfuse upserts on. `human_action:<kind>` is keyed by the act (the incident
commit; the decision and its instant), so each act counts once;
`human_accepted` is keyed by (trace or session, page or finding), so a later
verdict on the same page replaces the earlier one. A send removes exactly the
lines it handed over, under the append lock, so a score queued mid-send
survives. A `create_score` that raises keeps its record; a failed `flush`
keeps all of them; a crash between posting and dropping re-posts the same
ids — an update, never a second score. The SDK exports in the background and
does not report ingestion errors back, so "sent" means "handed to the SDK
and flushed".

**Health.** With `langfuse.enabled`, `dbwiki health` lists the queue under
`langfuse scores` and prints a `WARNING` line when records are older than
`langfuse.score_backlog_hours` (default 24) — a warning, never a problem, so
it does not turn health red. The usual cause is a node whose cron has no
`LANGFUSE_*` keys.
