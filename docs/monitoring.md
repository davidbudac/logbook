...
Stage staleness is judged against `health.stage_stale_hours` — `run` and
`report` 26h, `lint` and `research` 192h by default. A stage that has *never*
succeeded is never flagged.
...
| `dbwiki stats [--json] [--task X]` | — | cost and quality per (task, adapter, tier, mode) |
