...
| `generated-section-drift` | warning | a `<!-- BEGIN generated: … -->` section of the root `AGENTS.md` that no longer matches its `config/prompts/` file, or a BEGIN marker with no END (no markers, no finding); fix with `uv run python -m dbwiki.prompts sync AGENTS.md` |
