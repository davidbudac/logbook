...
| agent CLI | yes, one of | the LLM stages — `claude`, `codex`, or `pi`, selected by `agents.adapter` (`src/dbwiki/harness.py`) |
...
uv run dbwiki stats --since 12h --by model          # ... last night only, split per model (--until bounds the end)
...
older than `health.stage_stale_hours` (default 26h, also for
`research_history`; `lint`/`research`/`research_caveats`/`review` 192h)
...
`webhook` is one JSON POST per alert (stdlib, 5 s timeout, no retries within
a run), which is what ntfy/Slack/Teams/Discord incoming hooks accept. An
unknown sink name is a configuration error: the run warns, counts a
`sink_error`, and leaves the alert state untouched so the finding alerts again
once the config is fixed. A send that raises (webhook down, disk full) is
counted as a `sink_error`, not as `alerted`, and its fingerprint is kept as
`undelivered`: a later run re-sends it — after 30 min, then 1 h, 2 h, … at
most every 12 h — until it is delivered or the finding clears (then no
recovery is reported, since nobody was told). With `sinks: [a, b]` one dead
sink makes the alert undelivered, so the healthy one may receive it twice.
...
becoming `collection_failure` — alerts again. Global findings (`dirty_tree`,
`stage_stale`, `wiki_missing`, …) have no (db, source), so one clearing while
another appears is a recovery plus a new alert, not an escalation. A fingerprint that disappears is
...
or `pi` (the pi coding agent in one-shot mode; any provider from `pi --list-models`,
...
ones, one tick late. To see them at all the on-prem tick pulls first: with
`report.push` on, it rebases onto the remote with autostash before the
fold-in, keeping the analyst's report where both sides wrote the same day's
page; a failed pull is undone, noted as `analyst_pull_error` and named by
`dbwiki health` (`docs/scheduling.md`).
...
`requests/failed/`. A claim older than `stale_hours` (26) is a dead run's:
the next claim, or the next on-prem `dbwiki research` round
(`research.exchange.stale_hours`), returns it to `requests/pending/` with
`attempts + 1` — to `requests/failed/` (`stale_claim`) once they run out. `dbwiki-researcher --once` (default; `--drain` to empty
