...
The hourly `health --alert` line carries no `--lock-wait`, because the flag
does not exist on that subcommand: `--alert` writes `.state/alerts.json`, so
`cli._with_lock` takes the single-flight lock for it and falls back to
`_lock_wait_default()`, which is 0 unless `DBWIKI_LOCK_WAIT` is set. An hour
...
second run in the same week is a no-op unless you pass `--force`.
...
itself. **Step 1** in the next `dbwiki run` tick (before compaction) folds
the analyst's telemetry (`queue/results/*.json`, written there because the
analyst cannot reach the on-prem `.state/`) into `.state/agent_runs.jsonl`
and deletes the files; a `run_id` already present is skipped, so replaying
wiki history never double-counts a result.
...
- Langfuse (optional, `langfuse.enabled`) — one trace per attempted agent
  ...
  for the export to run (`docs/langfuse.md`).
