...
| `portal` | `bind` (`127.0.0.1:8765`), `operator_email`, `lock_wait_s` (3.0), `allowed_hosts`, `trusted_proxies` | where the incident workbench listens, who it commits as, how long a commit waits for the wiki lock, which host names and proxies it answers to |
...
uv run dbwiki review --deliver-only [--review-id 2026-W38]  # resend a week's review, skipping what already went
...
uv run dbwiki stats [--json] [--task ingest] [--since 12h|2026-09-23] [--until ...] [--by model]
...
```yaml
portal:
  bind: 127.0.0.1:8765   # a non-loopback --bind is allowed but warned about
  ...
  allowed_hosts: []      # extra Host/Origin names beyond the bind and loopback
  trusted_proxies: []    # proxy IPs whose X-Forwarded-Host is checked instead
```

Those are the defaults, except `push`, which has no default of its own and
follows `report.push`.

The server answers only requests whose `Host` names the bind or a loopback
spelling of its port (`127.0.0.1`, `localhost`, `[::1]`); anything else is
403 `bad_host`. That refusal is the protection against DNS rebinding, so keep
it narrow. On loopback, leave `allowed_hosts` empty. When browsers reach the
workbench by another name (a `--bind 0.0.0.0:8765` on the LAN, a Tailscale
name, a reverse proxy), list the names they use:

- A bare name (`dbhost`) matches on any port; `name:port` on that port only.
  A TLS proxy's browser sends `Origin: https://wiki.example.com`, which is
  `wiki.example.com:443`.
- Names compare without regard to case. `*`, URLs and other junk are refused
  when the config is read, so the server does not start.
- `Origin`, when a browser sends one, must be `http(s)://` plus an allowed
  name.

`trusted_proxies` is for a proxy that rewrites `Host` to `127.0.0.1:8765`:
from a peer on that list (IP addresses), the first `X-Forwarded-Host` is the
name checked in place of `Host`. From any other peer the header is ignored.
The proxy must still authenticate users; the portal does not. `serve` warns
on stderr when it binds a non-loopback address with no `allowed_hosts`.
...
delivery:
  ...
  smtp_host: ""            # the relay
  smtp_port: 25
  starttls: false          # true: STARTTLS, verifying the relay's certificate
...
duplicate recipient `id`, the reserved id `inbox` (the inbox row's key), and
an empty `address` on a recipient that names the email channel are all
refusals. So is `external_enabled: true` with an
...
`.state/review/log.jsonl`, the command exits 1 naming the retry, `dbwiki
health` lists it under "review delivery" (so alerts fire) until it goes, and
any later `dbwiki review` in the same week — or `uv run dbwiki review
--deliver-only --review-id YYYY-Www` for an earlier one — resends exactly what
is missing. A failure stops being reported once its review is pruned or its
