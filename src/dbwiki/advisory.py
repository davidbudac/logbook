...
from . import (harness, health, monitoring, prompts, state, structured,
               validate)
...
def _read_digests(target: Target) -> Iterable[tuple[str, str]]:
    ...
    `structured._digest_material`'s blocks: `## Deltas`, `## Changes` (the
    lifecycle groups, ADR-0006), `### Notable` and `### Trace evidence`. A
    digest with none of them is not yielded: a section with an empty body is
    a path the model could cite for no evidence."""
...
#: that introduces the material (`config/prompts/advisory/rules.md`; each
#: row's preamble is `config/prompts/advisory/<id>.md`). `draft-note` spells
#: its own copy instead of reusing this one; its prompt is pinned whole by a
#: test, and the reason for that is on its row below.
_RULES = prompts.load("advisory/rules")


def _instruction(tool_id: str) -> str:
    """A row's preamble from `config/prompts/advisory/<tool_id>.md`, then the
    shared rules and the line that introduces the material."""
    return prompts.load(f"advisory/{tool_id}") + "\n\n" + _RULES
...
TOOLS: Mapping[str, ToolSpec] = {
    "explain-incident": ToolSpec(
        ...
        instruction=_instruction("explain-incident"),
    ...
    "ask-incident": ToolSpec(
        ...
        instruction=_instruction("ask-incident"),
        max_answer_chars=1200,
        target_field="",
        role=identity.DRAFT_ROLE,
        tier=Tier.STRONG,
    ...
    "similar-incidents": ToolSpec(
        ...
        sources=(SourceKind.INCIDENT, SourceKind.OCCURRENCES,
                 SourceKind.NEIGHBOURS),
        instruction=_instruction("similar-incidents"),
    ...
    "summarize-evidence": ToolSpec(
        ...
        sources=(SourceKind.INCIDENT, SourceKind.CLOSURE, SourceKind.DIGEST,
                 SourceKind.ACTIONS),
        instruction=_instruction("summarize-evidence"),
        max_answer_chars=1200,
        target_field="",
        role=identity.DRAFT_ROLE,
        tier=Tier.CHEAP,
    ...
    "draft-note": ToolSpec(
        ...
        sources=(SourceKind.INCIDENT, SourceKind.ERROR_PAGE,
                 SourceKind.DIGEST),
        instruction=prompts.load("advisory/draft-note"),
    ...
    "next-checks": ToolSpec(
        ...
        instruction=_instruction("next-checks"),
        max_answer_chars=900,
        target_field="",
        role=identity.DRAFT_ROLE,
        tier=Tier.CHEAP,
    ...
    "explain-closure": ToolSpec(
        ...
        instruction=_instruction("explain-closure"),
        max_answer_chars=900,
        target_field="",
        role=identity.DRAFT_ROLE,
        tier=Tier.CHEAP,
...
class Runner:
    ...
    def __init__(self, settings: Settings, *, state_dir: Path,
        ...
        # serialises `start` from its idempotency read to the queued write,
        # so two re-posts of one click cannot both find nothing and both be
        # admitted; taken before `_lock`, never inside it
    ...
    def start(self, spec: ToolSpec, target: Target, *, at: str) -> AdvisoryRun:
        ...
        The read, the admission and the queued write happen under one
        `_starting` lock, so concurrent re-posts of one click converge on one
        run instead of each paying for a call. Anything that raises after
        admission (`pack`, the write, the thread) gives the slot and the
        reservation back and leaves a `failed` record, never a `queued` one
        no worker will pick up; the exception still reaches the caller."""
        self.sweep()
        run_id = run_id_for(spec.id, target.key, at, target.question)
        with self._starting:
            return self._start(spec, target, at=at, run_id=run_id)

    def _start(self, spec: ToolSpec, target: Target, *, at: str,
               run_id: str) -> AdvisoryRun:
        ...
        try:
            ...
            try:
                self._write(replace(
                    blank, status=RunStatus.FAILED, finished=self.now(),
                    duration_s=0.0,
                    error=structured._cap(f"{type(e).__name__}: {e}",
                                          MAX_ERROR_CHARS)))
            except OSError:
                pass  # the write is what failed; the raise below says so
            raise
        return run

    def read(self, run_id: str) -> AdvisoryRun | None:
        """The record as this process should read it (`as_of`), or None. The
        poll endpoint's whole body. An id that is not a plain id is never
        joined onto `runs_dir`, so no spelling reads outside it."""
        if not validate.is_safe_id(run_id):
            return None
    ...
    def _release(self, spec: ToolSpec) -> None:
        ...
        run: from the worker's `finally`, or from `_start` when the run never
        reached its worker."""
        with self._lock:
