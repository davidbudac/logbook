...
* **Undelivered is not alerted.** A send that raised leaves its fingerprint
  in state marked `undelivered` (failure count, next retry time, the original
  reason) instead of `alerted_at`, and a later dispatch re-sends it once the
  backoff (30 min doubling, capped at 12 h) has passed. It is not counted in
  `alerted`, and if it clears before anyone was told, its recovery is not
  reported either.
* **Deferred:** configurable re-alert persistence intervals (a still-broken,
  *delivered* fingerprint re-alerting after N hours). Single-shot dedupe
  first; add them only once it demonstrably fails an operator (see
  CHANGELOG.md).
* **Transport stays dumb.** `Sink` is the seam: the built-ins write to
  ...
  `alerts.sinks` fans out to several at once. No sink retries within a run,
  formats for a particular receiver, or decides anything — delivery failures
  are counted and retried on a later tick, and a dead transport never fails
...
from . import events
from .errors import COLLECTOR_HOST, recovery_hint  # noqa: F401 — re-exported
from .health import _now, _parse

ALERT_SCHEMA_VERSION = 1
ALERT_STATE = "alerts.json"
AlertReason = Literal["new", "escalation"]
...
# an undelivered alert is re-sent after 30 min, then 1 h, 2 h, ... at most
# every 12 h: `health --alert` ticks hourly, so the first retry is the next
# tick and a dead webhook costs one timeout per step rather than per tick
RETRY_BASE = dt.timedelta(minutes=30)
RETRY_CAP = dt.timedelta(hours=12)
...
class _Finding:
    ...
    @property
    def key(self) -> tuple:
        return _key(self.category, self.db, self.source)


def _key(category: str, db: str, source: str) -> tuple:
    """What an escalation is measured against: the (db, source) a finding is
    about. A global finding (both "-") has no such subject — `dirty_tree`,
    `stage_stale` and `wiki_missing` are unrelated problems — so it is keyed
    by its kind instead, and one clearing while another appears is a recovery
    plus a new alert, never an "escalation"."""
    db, source = db or "-", source or "-"
    return (db, source) if (db, source) != ("-", "-") else ("-", "-", category)
...
def findings(h: dict) -> list[_Finding]:
    ...
    for s in h.get("stale_stages") or []:
        ...
        out.append(_Finding("stage_stale", "-", "-",
                            ...
                            detail=s["task"]))
    for d in (h.get("review_delivery") or {}).get("failed") or []:
        # one per review: a dead relay fails every recipient at once, and
        # one retry command covers them all
        out.append(_Finding("delivery_failed", "-", "-",
                            {"review_id": d["review_id"],
                             "recipient": d["recipient"], "at": d["at"],
                             "error": d["error"]},
                            detail=str(d["review_id"])))
...
def evaluate(health: dict, state: dict, now: str,
    ...
    for e in prior.values():
        prior_by_key.setdefault(_entry_key(e), set()).add(e.get("category"))
    ...
    for f in current:
        ...
        if old:  # still broken, identically: bookkeeping only
            ...
            if not _retry_due(old, now):
                continue
            # never delivered, and its backoff has passed: send it again as
            # what it was, optimistically marked alerted (dispatch reverts a
            # failed send)
            u = old["undelivered"]
            fps[f.sha] = {k: v for k, v in fps[f.sha].items()
                          if k != "undelivered"} | {"alerted_at": now}
            to_send.append(Alert(
                run_id=run_id, fingerprint=f.sha, category=f.category,
                db=f.db, source=f.source,
                reason=str(u.get("reason") or "new"),
                escalated_from=u.get("escalated_from"), window=f.window,
                first_seen=str(old.get("first_seen") or now), at=now,
                last_success=_last_success(health),
    ...
    recovered = [
        ...
        if sha not in fps and not arrived(_entry_key(e))
        and not e.get("undelivered")    # nobody was told it broke
    ]
    return to_send, {"schema_version": ALERT_SCHEMA_VERSION,
                     "fingerprints": fps, "recovered": recovered}


def _entry_key(e: Mapping) -> tuple:
    return _key(e.get("category"), e.get("db", "-"), e.get("source", "-"))


def _retry_due(entry: Mapping, now: str) -> bool:
    """An undelivered entry whose retry time has come. An unreadable time
    retries rather than waits: a lost alert is the worse failure."""
    u = entry.get("undelivered")
    if not isinstance(u, Mapping):
        return False
    at, n = _parse(u.get("retry_at")), _parse(now)
    return at is None or n is None or at <= n


def _undelivered(entry: dict, alert: "Alert", previous: Mapping | None,
                 now: str) -> dict:
    """`entry` as a send that raised leaves it: not alerted, with the failure
    count, the next retry time and what the alert was, so the retry is the
    same alert."""
    u = (previous or {}).get("undelivered")
    failures = (_acount(u.get("failures")) if isinstance(u, Mapping) else 0) + 1
    delay = min(RETRY_BASE * 2 ** min(failures - 1, 16), RETRY_CAP)
    base = _parse(now) or dt.datetime.now(dt.timezone.utc)
    return {k: v for k, v in entry.items() if k != "alerted_at"} | {
        "undelivered": {
            "failures": failures,
            "retry_at": (base + delay).strftime("%Y-%m-%dT%H:%M:%SZ"),
            "reason": alert.reason,
...
_OPEN_DROPPED: Mapping[str, str] = {
    ...
    "detail": "free text out of the assessment; the group carries its own "
              ...
              "many",
    "undelivered": "the dispatcher's retry bookkeeping for a send that "
                   "failed; the finding is open either way",
}
...
def enabled(cfg) -> bool:
    ...
    return bool(cfg.alerts.get("enabled"))
...
def dispatch(cfg, health: dict, run_id: str, *, now: str | None = None,
    ...
    *not* written and the pending findings still alert once the config is
    ...
    A send that raised is not "alerted": its fingerprint is kept as
    `undelivered` and re-sent by a later dispatch after a backoff, and it is
    not counted in `alerted`."""
    now = now or _now()
    path = Path(cfg.state_dir) / ALERT_STATE
    prior = load_state(path)["fingerprints"]
    to_send, new_state = evaluate(health, {"fingerprints": prior}, now, run_id)
    try:
    ...
    for a in to_send:
        try:
            ...
            fps[a.fingerprint] = _undelivered(fps[a.fingerprint], a,
                                              prior.get(a.fingerprint), now)
    save_state(path, new_state)
    facts = {"alerted": len(to_send) - errors,
             "recovered": len(new_state["recovered"])}
