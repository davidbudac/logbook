...
def _utc(ts: str) -> dt.datetime:
    """An already-validated ISO-8601 timestamp as an aware UTC instant; one
    without an offset is taken as UTC. Offsets make string order meaningless
    (`23:30+02:00` is before `22:00Z`), so every comparison goes through
    this."""
...
def _utc_stamp(ts: str) -> str:
    """`ts` as the compactor writes window bounds: UTC with a `Z`. A value
    already in that form comes back unchanged."""
    t = _utc(ts)
    frac = f".{t.microsecond:06d}".rstrip("0") if t.microsecond else ""
...
def validate_awr_summary(summary, where: str = "awr summary") -> dict:
    ...
    db = _str(where, summary, "db", "db")
    if not is_safe_id(db):
        # it becomes `<digest_dir>/<db>/`: no separators, no `..`, no dotfile
        _fail(where, "db", f"must be a safe directory name "
                           f"([A-Za-z0-9][A-Za-z0-9._-]*, no '..'), got {db!r}")
...
def awr_digest(summary: dict) -> dict:
    ...
    # UTC, like every other digest window: the day (and the file name built
    # from these) must not depend on the offset the collector wrote
    t0, t1 = _utc_stamp(snap["begin_time"]), _utc_stamp(snap["end_time"])
...
def emit_awr_digest(cfg, digest: dict) -> tuple[Path, Path]:
    ...
    Atomic, like `Compactor.emit`: the ELK shipper tails this directory."""
    from . import state
    ...
    state.atomic_write_text(jp, json.dumps(digest, indent=1))
    state.atomic_write_text(mp, render_md(digest))
    return jp, mp
