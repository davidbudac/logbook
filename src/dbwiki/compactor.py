...
from .normalize import (UnsupportedSchemaError, field_paths, normalize,
                        observed_hosts, service_name_like)
from .patterns import (PatternLibrary, extract_codes, extract_trace_paths,
                       message_template)
from .state import Registry, StateStore, instant_key
...
ANOMALY_WINDOW_HOURS = 3.0  # trailing window a burst is also measured over


def _minus_hours(ts: str, hours: float) -> str:
...
# A rate anomaly's window length and per-hour rate move with every tick of
# the day even when not one event changed; in the hash they would make every
# tick look new and wake the agent again for the same burst.
_WINDOW_FIELDS = ("window_hours", "rate_per_hour")


def _hash_view(delta: dict) -> dict:
    if delta.get("type") != "rate_anomaly":
        return delta
    return {k: v for k, v in delta.items() if k not in _WINDOW_FIELDS}
...
class _Group:
    ...
    def add(self, ev: dict, max_samples: int) -> None:
        self.count += 1
        k = instant_key(ev["ts"])
        if k < instant_key(self.first_ts):
            self.first_ts = ev["ts"]
        if k > instant_key(self.last_ts):
            self.last_ts = ev["ts"]
...
class Compactor:
    def __init__(self, cfg: Config):
        ...
        self.anomaly_window_hours = c.get("rate_anomaly_window_hours",
    ...
    def compact(self, db: str, t0: str, t1: str, day: str,
        ...
        for name in (sources or list(self.cfg.sources)):
            ...
            notable_total += sum(g["count"] for g in section["notable"]) \
                + sum(section.get("dropped_notable_events", {}).values())
    ...
    def _compact_source(self, name: str, db: str, t0: str, t1: str,
        ...
        # the same counters over the trailing `anomaly_window_hours` only: a
        # burst late in the day is diluted to nothing over midnight -> now
        recent: Counter = Counter()
        recent_t0 = max(t0, _minus_hours(t1, self.anomaly_window_hours),
                        key=instant_key)
        recent_key = instant_key(recent_t0)
        class_counts: Counter = Counter()
        # (rule, template, sorted codes): the code set is part of a group's
        # identity, because two long messages can share the whole template
        # width ("Errors in file <long path>:") and differ only in the ORA
        # code below it — merged, the second code would vanish
        groups: dict[tuple, _Group] = {}
        n_groups: Counter = Counter()          # rule -> groups kept
        shown_codes: set[str] = set()          # codes carried by kept groups
        dropped_groups: dict[str, set] = {}    # rule -> distinct keys dropped
        dropped_events: Counter = Counter()    # rule -> events dropped
        code_counts: Counter = Counter()       # code -> notable events
        ...
        for hit in self.es.scan(scfg["index_patterns"], query,
            ...
            # machine names on the document: the redaction vocabulary learns
            # them from the registry (ADR-0002), no extra ES query
            for host in observed_hosts(hit, ev):
                reg.note_host(host, ev["ts"])
            if name == "listener":
            ...
            if not rule.notable:
                if rule.klass == "routine":
                    ck = rule.counter_key(ev)
                    counters[ck] += 1
                    if instant_key(ev["ts"]) >= recent_key:
                        recent[ck] += 1
                continue
            ...
            key = (rule.name, message_template(ev["message"]), tuple(sorted(codes)))
            grp = groups.get(key)
            if grp is None:
                cap = self.max_unmatched if rule.name == "unmatched" else self.max_groups
                # at the cap, a group that carries a code no kept group shows
                # still gets in — up to one more cap's worth — so a storm of
                # one template can never hide a new ORA code behind it
                if n_groups[rule.name] >= cap and (
                        n_groups[rule.name] >= 2 * cap
                        or all(c in shown_codes for c in codes)):
                    dropped_groups.setdefault(rule.name, set()).add(key)
                    dropped_events[rule.name] += 1
                    continue
                n_groups[rule.name] += 1
                shown_codes.update(codes)
        ...
        return {
            ...
            "notable": notable,
            "dropped_notable_groups": {r: len(k) for r, k in dropped_groups.items()},
            # both only when the cap dropped something, like trace_evidence:
            # the events behind the dropped groups, and every code on a
            # notable event with its count, so a dropped group never hides one
            **({"dropped_notable_events": dict(dropped_events),
    ...
    def _deltas(self, source: str, db: str, reg: Registry, day: str, t0: str,
                t1: str, total: int, counters: Counter, win_s: float,
                recent: tuple[Counter, float] | None = None) -> list[dict]:
        deltas: list[dict] = []
        ...
        deltas.extend(self._rate_anomalies(source, reg, day, t0, counters,
                                           win_s, recent))
    ...
    def _rate_anomalies(self, source: str, reg: Registry, day: str, t0: str,
                        counters: Counter, win_s: float,
                        recent: tuple[Counter, float] | None) -> list[dict]:
        ...
        Measured over the whole window and over the trailing `recent` one
        (`rate_anomaly_window_hours`); the higher rate that fires wins. The
        trailing window is what catches a burst late in the day, which the
        midnight -> now rate dilutes over up to 23 hours.

        Sticky per day: the strongest anomaly a counter showed is kept in the
        registry and restated by every later digest of that day (a window
        starting at the day's midnight), so the burst the 10:15 tick saw is
        still in the 23:30 consolidation digest after the trailing window has
        moved past it. Its numbers stay those of the tick that saw it."""
        spans = [(counters, win_s)]
        if recent is not None and recent[1] < win_s:
            spans.append(recent)
        fired: dict[str, dict] = {}
        for counter in counters:
            ...
            if med <= 0:
                continue
            for cnt, secs in spans:
                n = cnt.get(counter, 0)
                if secs < ANOMALY_MIN_WINDOW_S or n < self.anomaly_min_count:
                    continue
                hours = secs / 3600
                rate = n / hours
                if rate > self.anomaly_factor * med and \
                        rate > fired.get(counter, {}).get("_rate", 0):
                    fired[counter] = {
                        ...
                        "_rate": rate}
        out = []
        for counter, d in fired.items():
            d.pop("_rate")
            out.append(reg.note_anomaly(day, source, d))
        if instant_key(t0) == instant_key(f"{day}T00:00:00Z"):
            out.extend(d for c, d in sorted(reg.anomalies_of(day, source).items())
                       if c not in fired)
        return out
    ...
    def content_hash(digest: dict) -> str:
        ...
        import json
        core = {
            "deltas": [_hash_view(d) for d in digest["deltas"]],
            "sources": {
