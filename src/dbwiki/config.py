...
class Config:
    def __init__(self, raw: dict, root: Path):
        ...
        self.wiki_repo = root / raw["wiki_repo"]
        self.state_dir = root / raw["state_dir"]
        self.digest_dir = root / raw["digest_dir"]
        ...
        self.trace_lookup = raw.get("trace_lookup") or {}
        self.compactor = raw.get("compactor", {})
        self.agents = raw.get("agents", {})
        self.report = raw.get("report", {})
        self.research = raw.get("research", {})
        ...
        self.analyst = raw.get("analyst", {})
        # absent block -> enabled false -> nothing exported, langfuse unimported
        self.langfuse = raw.get("langfuse", {})
        self.advisory = raw.get("advisory", {}) or {}
        self.review = raw.get("review", {}) or {}
        self.delivery = raw.get("delivery", {}) or {}
    ...
    def db_filter(self, source: str, db: str) -> list[dict]:
        ...
        if svc:
            ...
            variants: list[str] = []
            for base in (db, f"{db}_DGMGRL"):
                for cased in {base, base.upper()}:
                    variants.append(cased)
                    variants.extend(f"{cased}.{d}" for d in domains)
            terms.append({"terms": {svc: variants}})
