...
add a parameter to the query it lands in. Percent-encoding is undone by the
browser before Kibana parses the rison state inside the fragment, so values
that sit in a rison string (`Template.rison`) are rison-escaped first, and
values quoted inside a KQL query are KQL-escaped before that (`_kql`).
"""
...
class Template:
    ...
    #: the placeholders that sit inside a rison `'...'` string in `url`
    rison: frozenset[str] = frozenset()
...
TEMPLATES: Mapping[str, Template] = MappingProxyType({
    "oracle-error-context-v1": Template(
        ...
        frozenset({"data_view", "query", "from", "to"})),
...
def _check_templates(templates: Mapping[str, Template]) -> None:
    ...
    for template_id, template in templates.items():
        ...
        if not template.rison <= template.placeholders:
            raise ValueError(f"{template_id}: rison names a placeholder the "
                             f"template does not allowlist")
...
def _fill(template: Template, *, base: str, values: Mapping[str, str]) -> str:
    ...
    filled = {name: urllib.parse.quote(
                  _rison(str(value)) if name in template.rison else str(value),
                  safe="")
...
def _rison(value: str) -> str:
    """`value` as the body of a rison `'...'` string: `!` is rison's escape
    character and `'` would end the string."""
    return value.replace("!", "!!").replace("'", "!'")


def _kql(value: str) -> str:
    """`value` as a KQL quoted string, quotes included."""
    return '"' + value.replace("\\", "\\\\").replace('"', '\\"') + '"'
...
class Resolver:
    ...
    def logs(self, ref: EvidenceRef, *,
        ...
        query = f"{_kql(ref.entity.id)} and {_kql(ref.signature.value)}"
        return DeepLink(
            LinkState.AVAILABLE,
