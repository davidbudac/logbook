...
def _warn_once(key: str, msg: str) -> None:
    if key in _warned:
        return
    _warned.add(key)
    print(f"warning: eval: {msg}", file=sys.stderr)
...
def _incident_consistent(proposal: dict, wiki: Path) -> bool:
    ...
    target = (inc["existing_page"] or "").strip().lstrip("./")
...
def evaluate_item(item: dict, cfg) -> tuple[dict | None, dict]:
    ...
    verdict = {"parsed_ok": False, "apply_ok": False, "codes_allowlisted": 0.0,
               "incident_consistent": False, "flags": 0, "duration_s": None,
...
def _pinned(cfg, model: str, provider: str | None):
    ...
    out = copy.copy(cfg)
    agents = dict(getattr(cfg, "agents", None) or {})
...
def run(items: list[dict], cfg, *, model: str, provider: str | None = None,
    ...
    client = _client(getattr(cfg, "langfuse", None) or {})
    if client is not None:
        _run_experiment(client, name=name, run_name=run_name,
                        ...
                        metadata={"model": model, "provider": provider or "",
                                  "app_sha": _app_sha()})
    # the experiment already called the task for every item it ran; an empty
    # results list means it never started (disabled, uninstalled, unreachable)
    if not results:
        for item in items:
            task(item=item)
