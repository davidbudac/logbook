...
class RunEvent:
    ...
    The tick facts (`report_path` through `consolidation`) are hoisted flat on
    purpose. A `Mapping` bag for them would be exactly the hole that
    `CLASSIFICATION` completeness exists to close: an unclassified key could
    ship inside the bag without any test noticing."""
...
def by_model(evs: Sequence[RunEvent]) -> tuple[ModelRoll, ...]:
    ...
    for ev in evs:
        if ev.kind is Kind.STAGE:
            held.setdefault(ev.model, []).append(ev)
    rolls = []
    for model, stages in held.items():
        tiers: dict[str, int] = {}
        for ev in stages:
            tiers[ev.model_tier] = tiers.get(ev.model_tier, 0) + 1
        rolls.append(ModelRoll(
            model=model,
    ...
    return tuple(sorted(rolls, key=lambda r: (-r.totals.stages, r.model)))
