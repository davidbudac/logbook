...
PREVIEW_CHARS = 200
MAX_STEPS = 200
#: Seconds an adapter's process group gets between SIGTERM and SIGKILL.
KILL_GRACE_S = 2.0
...
#: The agent CLIs `run_agent` can drive. `agents.adapter` and
#: `research.adapter` must name one of these (`check_adapter`, at config load).
ADAPTERS = ("codex", "claude", "pi")


class UnknownAdapterError(HarnessError, ValueError):
    """A config (or caller) names an adapter `run_agent` cannot drive."""


def check_adapter(name, where: str = "adapter") -> str:
    """`name` when it is a supported adapter, else `UnknownAdapterError`
    naming the supported ones — a clear config error at load instead of a
    KeyError, or a failed run hours later in cron."""
    if name in ADAPTERS:
        return name
    hint = ("; the ollama adapter was removed — run a local model through "
            "pi (agents.pi.provider) instead" if name == "ollama" else "")
    raise UnknownAdapterError(
        f"{where}: unknown adapter {name!r}; supported adapters: "
        f"{', '.join(ADAPTERS)}{hint}")
...
class InvalidResultError(HarnessError):
    """Agent wrote a result file that is not JSON, or JSON that is not the
    contract's shape (`check_result`). Its own class so the retry
...
def _note(telemetry: dict | None, **facts) -> None:
    """Write telemetry facts in place, when the caller asked for them."""
...
def _group_alive(pgid: int) -> bool:
    try:
        ...
        return False
    except PermissionError:  # a member we may not signal still exists
        return True
    return True
...
def _kill_group(proc: subprocess.Popen, grace: float) -> None:
    """SIGTERM the adapter's whole process group, give it `grace` seconds,
    then SIGKILL whatever is left. The group, not the pid: the `codex`
    launcher script and every tool subprocess an agent starts are members,
    and killing only the leader left them editing the wiki after the
    orchestrator had already rolled it back."""
    pgid = proc.pid  # start_new_session made the adapter its group's leader
    _signal_group(pgid, signal.SIGTERM)
    try:
        proc.wait(timeout=grace)
    except subprocess.TimeoutExpired:
        pass
    # always, not only when the leader ignored SIGTERM: a member that
    # outlives the leader is exactly the process this exists to stop
    _signal_group(pgid, signal.SIGKILL)


def _reap_leftovers(pgid: int, grace: float) -> None:
    """After a normal exit: stop any group member the adapter left running
    in the background. Nothing is left in the common case, and that costs one
    `killpg(pgid, 0)`."""
...
def _spawn(cmd: list[str], *, cwd: Path,
    ...
    """`subprocess.run(cmd, capture_output=True, text=True, timeout=…)` with
    the adapter as the leader of its own process group (`start_new_session`),
    so a timeout ends the whole tree rather than the direct child alone —
    and so does a normal exit that left something running in the
    background. Raises `TimeoutExpired` carrying whatever output the group
    produced, the way `subprocess.run` does.

    Reading after the kill is bounded too: a process that escaped the group
    (its own `setsid`) could hold the pipes open forever."""
    with subprocess.Popen(cmd, cwd=cwd, stdout=subprocess.PIPE,
        ...
        _reap_leftovers(p.pid, KILL_GRACE_S)
...
def _run(cmd: list[str], cwd: Path, timeout: int, adapter: str = "",
    ...
    t0 = time.monotonic()
    try:
        proc = _spawn(cmd, cwd=cwd, timeout=timeout)
    except subprocess.TimeoutExpired as e:
        out = _text(e.stdout)
...
def _usage_codex(out: str) -> dict | str:
    ...
    output=…`) stays as a fallback for a codex build that does not emit the
    event stream. Codex reports no price,
...
STEP_PARSERS = {"codex": _steps_codex, "pi": _steps_pi}
...
def run_agent(adapter: str, prompt: str, wiki: Path, model: str | None,
    ...
    if not result_path.exists():
        ...
        raise NoResultError(f"{adapter} finished but wrote no {RESULT_FILE}; "
                            f"last output: {tail}")
    try:
        return check_result(json.loads(result_path.read_bytes()))
    except (ValueError, OSError) as e:  # JSONDecodeError, UnicodeDecodeError
        raise InvalidResultError(f"invalid result JSON: {e}") from e
    finally:
        result_path.unlink(missing_ok=True)


#: The result contract's field types (wiki AGENTS.md, "Result JSON
#: contract"). Which keys a task *requires* is the orchestrator's call
#: (`Orchestrator._validate`); this only says what a key holds when present.
RESULT_STR_KEYS = ("task", "db", "summary")
RESULT_LIST_KEYS = ("pages_touched", "incidents_opened", "incidents_updated",
                    "flags")
RESULT_BOOL_KEYS = ("notable",)


def check_result(result) -> dict:
    """The parsed result file, provided it has the contract's shape: a JSON
    object whose known keys hold a string, a list of strings, or a boolean.
    A key holding null is dropped, so it reads as absent and the
    orchestrator's own default or its missing-key check applies. Anything
    else raises `ValueError`, which `run_agent` turns into
    `InvalidResultError` — the same retry-with-feedback path an unparseable
    file takes — rather than letting `[]` or `{"summary": 5}` reach a stage
    that calls `.get` on it and dies without rolling back."""
    if not isinstance(result, dict):
        raise ValueError(f"expected one JSON object, got "
    ...
    for key in RESULT_LIST_KEYS:
        if key in result and not (isinstance(result[key], list) and all(
