...
from dbwiki.errors import (CATEGORIES, RETRYABLE, ValidationError,  # noqa: F401 — re-exported
                           WikiMissingError, recovery_hint)
from dbwiki.harness import HarnessError, NoResultError, pi_model_id
from dbwiki.lock import LockBusyError
from dbwiki.normalize import UnsupportedSchemaError
from dbwiki.observability import warn_once
...
#: run/report and `research --history` are daily; lint, research (agentic),
#: `research --caveats` and review run weekly, so a week plus slack.
DEFAULT_STAGE_STALE_HOURS = {"run": 26, "report": 26, "lint": 192,
                             "research": 192, "research_caveats": 192,
                             "research_history": 26, "review": 192}
...
STAGE_SUCCESS_KINDS = {"run": ("compaction", "ingestion"),
                       ...
                       "review": ("review",)}
#: `dbwiki research` records every variant as `command="research"`; the
#: variant is `facts.mode`. Each is its own stage: the nightly model-free
#: `--history` succeeding says nothing about the weekly agentic research,
#: and keying them together hid five failed Mondays behind it. An absent
#: mode is the agentic research (and its monthly `--sources-only` run).
RESEARCH_MODE_KINDS = {None: "research", "caveats": "research_caveats",
                       "history": "research_history"}
#: how far ahead of now an event's timestamp may be and still count as the
#: source's newest event: covers a shipper labelling local time as UTC in any
#: timezone. Beyond it the event is a data-quality problem, never liveness.
FUTURE_SLACK_HOURS = 15
...
def categorize(exc: BaseException) -> str:
    """Stable failure category for an exception."""
...
#: serializes `_append_capped` between threads of one process (the portal
#: serves requests on threads); the per-log flock below does the same between
#: processes. Both are needed: the flock alone would also serialize threads,
#: but `_line_counts` is process state and wants the in-process lock anyway.
_append_lock = threading.Lock()


@contextmanager
def _log_lock(path: Path):
    """Exclusive hold on `<log>.lock` beside `path` for one append (+trim).
    A separate file because a trim replaces the log's inode: a flock on the
    log itself would be taken on the old inode by one writer and the new one
    by the next, and exclude nothing."""
    with _append_lock, open(path.with_name(path.name + ".lock"), "a") as fh:
...
def _append_capped(path: Path, event: dict, max_events: int) -> None:
    ...
    Append and trim run under `_log_lock`: a trim reads the log and then
    renames a shortened copy over it, and a line another thread or process
    appended in between used to vanish with the rename."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    with _log_lock(path):
        _append_and_trim(path, event, max_events)


def _append_and_trim(path: Path, event: dict, max_events: int) -> None:
...
def _record(path: Path, event: dict, max_events: int) -> bool:
    """The recorders' one failure boundary: `_append_capped`, or — on any
    failure — one stderr warning per process and False. Telemetry must not
    break the pipeline, so nothing a recording raises reaches the caller."""
    try:
        _append_capped(path, event, max_events)
    except Exception as e:  # noqa: BLE001 — telemetry is never a failure mode
        warn_once("run-health", f"run-health recording failed: {e}")
        return False
    return True
...
def record_run(state_dir: Path, event: dict) -> None:
    ...
    _record(Path(state_dir) / HEALTH_LOG, event, MAX_EVENTS)
...
def record_agent_run(state_dir: Path, fields: dict, tele: dict | None = None,
    ...
    if _record(Path(state_dir) / AGENT_LOG, event, MAX_AGENT_EVENTS):
        return event_id
    return None
...
def append_agent_run_event(state_dir: Path, event: dict) -> None:
    ...
    `record_agent_run`."""
    _record(Path(state_dir) / AGENT_LOG, event, MAX_AGENT_EVENTS)
...
def append_advisory_run(state_dir: Path, event: dict) -> None:
    ...
    _record(Path(state_dir) / ADVISORY_LOG, event, MAX_ADVISORY_EVENTS)
...
def _record_run_start(state_dir: Path, rec: RunRecord) -> None:
    ...
    event = {"event_id": f"{rec.run_id}-start", "run_id": rec.run_id,
             ...
             "schema_version": HEALTH_SCHEMA_VERSION}
    _record(Path(state_dir) / RUN_STARTS_LOG, event, MAX_RUN_START_EVENTS)
...
def _future_cutoff(now: str) -> str | None:
    n = _parse(now)
    if n is None:
        return None
    return (n + dt.timedelta(hours=FUTURE_SLACK_HOURS)).strftime(
        "%Y-%m-%dT%H:%M:%SZ")
...
def collection_states(cfg, es, now: str, stale_hours: float) -> list[dict]:
    ...
    The newest event is looked for up to now + FUTURE_SLACK_HOURS only: one
    mis-parsed line dated years ahead would otherwise be the newest event
    forever, and its negative age kept a dead source `ok`. The first event
    beyond that bound, if any, is carried as `future_event` so `assess` can
    report it as the data-quality problem it is."""
    cutoff = _future_cutoff(now)
    probes = []
    for name in cfg.sources:
        try:
            if cutoff is None:
                latest, future = _latest_event(es, cfg, name), None
            else:
                latest = _edge_event(es, cfg, name, cutoff, recent=False)
                future = _edge_event(es, cfg, name, cutoff, recent=True)
            probes.append({"source": name, "latest_event": latest,
                           "age_hours": _hours_between(latest, now),
                           "state": None,
                           **({"future_event": future} if future else {})})
...
def _model_server(cfg, get_json) -> dict:
    ...
    min_context = int(cfg.health.get("min_context", DEFAULT_MIN_CONTEXT))
...
def _portal(cfg, get_json) -> dict:
    ...
    line. `errors._DEP_HINTS["portal"]` turns a `portal_unreachable` finding
    into the restart command."""
    from .config import PORTAL_DEFAULTS
    bind = str(cfg.portal.get("bind") or PORTAL_DEFAULTS["bind"])
...
def _wiki_remote(cfg, wiki: dict) -> dict:
    ...
    the inside."""
    out = {"push": bool(cfg.report.get("push")),
           "problems": []}
    if not wiki.get("git"):
...
def dependencies(cfg, wiki: dict, *, get_json=None) -> dict:
    ...
    from and a stable `detail` that `errors.recovery_hint` turns into the
    exact next command."""
...
def _last_successes(events: list[dict], ledger: dict) -> dict:
    ...
    for e in events:
        ...
        if cmd == "research" and not facts.get("dry_run"):
            # a dry run proves nothing about the stage the cron line runs
            kind = RESEARCH_MODE_KINDS.get(facts.get("mode"))
            if kind:
                note(kind, fin, cmd, str(facts.get("pages", "")))
        if cmd == "review":
    ...
    return {k: got.get(k) for k in
            ("compaction", "ingestion", "report", "lint", "research",
             "research_caveats", "research_history", "review")}
...
def exchange_stale_hours(cfg) -> float:
    """`research.exchange.stale_hours`: the age at which a research-exchange
    claim is a dead researcher's. One reader for both uses — this module's
    flag and the on-prem reclaim (`exchange.reclaim_stale`) — so a claim
    health calls stale is the one the next research round puts back."""
    from .exchange import STALE_CLAIM_HOURS
    xcfg = cfg.research.get("exchange") or {}
    return float(xcfg.get("stale_hours", STALE_CLAIM_HOURS))
...
def _queue_state(wiki_repo: Path, now: str, stale_hours: float) -> dict:
    ...
    entries older than `stale_hours` (a crashed analyst — the next
    `dbwiki analyst` returns it to `pending/`, `queue.reclaim_stale`), and
    the failed/ count by error
...
def _review_delivery(cfg) -> dict:
    """Weekly-review deliveries whose last attempt failed and that a retry
    can still reach: `{"failed": [...]}`, one row per (review, recipient).

    Read from `review/log.jsonl`, the audit `delivery.dispatch` writes, last
    row per key winning (the log is append-only, so file order is time
    order). A failure stops counting once its review is pruned or its
    recipient is no longer planned by the current `delivery:` policy: neither
    can be retried, and a problem nobody can clear teaches people to ignore
    health."""
    from . import delivery  # delivery and review import us
    from .review import AUDIT_LOG, list_reviews, load_review
    state_dir = Path(cfg.state_dir)
    kept = set(list_reviews(state_dir))
    last: dict[tuple[str, str], dict] = {}
    for row in _read_jsonl(state_dir / AUDIT_LOG):
        rid, key = str(row.get("review_id") or ""), str(row.get("key") or "")
        if row.get("event") in ("delivered", "delivery_failed") \
                and rid in kept and key:
            last[(rid, key)] = row
    failed = [row for row in last.values()
              if row.get("event") == "delivery_failed"]
    if failed:
        try:
            policy = delivery.Policy.resolve(cfg)
        except ValueError:
            policy = None      # a broken policy is the review stage's error
        if policy is not None:
            planned = {rid: {d.key for d in delivery.decide(
                           load_review(state_dir, rid) or {}, policy,
                           now=_now())}
                       for rid in {str(r["review_id"]) for r in failed}}
            failed = [r for r in failed
                      if str(r["key"]) in planned[str(r["review_id"])]]
    return {"failed": [
        {"review_id": r.get("review_id"), "key": r.get("key"),
         "channel": r.get("channel"), "recipient": r.get("recipient"),
         "at": r.get("at"), "error": r.get("error")}
        for r in sorted(failed, key=lambda r: (str(r.get("review_id")),
                                               str(r.get("key"))))]}
...
def _blockers(wiki: dict) -> list[tuple[str, str]]:
    """`(category, text)` for every wiki condition that stops the agent
    stages. `assess` publishes the texts; `format_health` rebuilds the pairs
    from the same `wiki` dict to pick each blocker's recovery hint."""
    out = []
    if not wiki["present"]:
        out.append(("wiki_missing", f"wiki repo missing: {wiki['path']} "
                    f"(agent stages and digest files are unavailable)"))
    elif not wiki["git"]:
        out.append(("wiki_missing",
                    f"wiki repo is not a git repository: {wiki['path']}"))
    if wiki["stray"]:
        shown = ", ".join(wiki["stray"][:5]) + ("…" if len(wiki["stray"]) > 5 else "")
        out.append(("dirty_tree",
                    f"wiki working tree dirty outside digests/ ({shown}); "
...
def assess(cfg, *, es=None, now: str | None = None, get_json=None) -> dict:
    ...
    problems = []
    blockers = [text for _, text in _blockers(wiki)]
    for p in sources:
        if p.get("future_event"):
            problems.append(f"future-dated events: {p['source']} has events "
                            f"dated {p['future_event']} or later, more than "
                            f"{FUTURE_SLACK_HOURS}h ahead of now (check the "
                            f"shipper's timestamp parsing; they are ignored "
                            f"for collection state)")
    ...
    analyst_stale_hours = float(cfg.analyst.get("stale_hours",
    ...
    for c in queue["stale_claimed"]:
        problems.append(f"analyst queue: claimed {c['file']} by "
                        ...
                        f"the next `dbwiki analyst` returns it to pending/)")
    if queue["failed_count"]:
        ...
        problems.append(f"analyst queue: {queue['failed_count']} failed "
                        f"request(s) ({cats})")
    # the latest tick's pull before the fold-in (queue.pull_before_fold):
    # while it fails, analyst results stay on the remote and every on-prem
    # push is rejected, and nothing else here would show it
    last_run = next((e for e in reversed(events) if e.get("command") == "run"),
                    None)
    if last_run and (err := (last_run.get("facts") or {})
                     .get("analyst_pull_error")):
        problems.append(f"analyst queue: the last tick could not pull the wiki "
                        f"remote ({err[:160]}); analyst results do not fold in "
                        f"and on-prem pushes are rejected until it can")

    exchange_stale = exchange_stale_hours(cfg)
    exchange = _exchange_state(cfg, now, exchange_stale)
    for c in exchange["stale_claimed"]:
        problems.append(f"research exchange: claimed {c['file']} by "
                        f"{c['claimed_by']} is stale ({c['age_hours']}h > "
                        f"{exchange_stale}h; a crashed researcher — the next "
                        f"researcher claim or on-prem `dbwiki research` "
                        f"returns it to requests/pending/)")
    ...
    try:
        review_delivery = _review_delivery(cfg)
    except Exception as e:  # noqa: BLE001 — an unreadable audit is not a crash
        review_delivery = {"failed": [], "error": categorize(e)}
    for f in review_delivery["failed"]:
        problems.append(f"review delivery failed: {f['review_id']} to "
                        f"{f['recipient']} ({f['error']}, {f['at']})")

    # unsent human-feedback scores: a warning in the report, never a problem
    try:
        from .scores import backlog as score_backlog  # scores imports us
        score_state = score_backlog(cfg, now)
    except Exception as e:  # noqa: BLE001 — an unreadable queue is not a crash
        score_state = {"enabled": False, "error": categorize(e)}
...
def _score_lines(h: dict) -> list[str]:
    """The pending human-feedback scores (`scores.backlog`), shown only when
    Langfuse is enabled and something is queued. A stale backlog is a
    WARNING line, deliberately not a problem: feedback that has not reached
    Langfuse yet is no reason to turn health red."""
    sc = h.get("scores") or {}
    if not (sc.get("enabled") and sc.get("pending")):
        return []
    out = ["", "langfuse scores",
           f"  pending: {sc['pending']} (oldest {sc.get('oldest')})"]
    if sc.get("stale"):
        out.append(f"  WARNING: {sc['stale']} unsent for more than "
                   f"{sc['threshold_h']:g}h — run `dbwiki scores send` and "
                   f"check its cron line and the LANGFUSE_* keys")
    return out
...
def format_health(h: dict) -> str:
    ...
    for p in h["sources"]:
        ...
        if p.get("future_event"):
            out.append(f"  {'':<11} future-dated events from "
                       f"{p['future_event']} ignored (timestamp parsing?)")
    ...
    failed_reviews = (h.get("review_delivery") or {}).get("failed") or []
    if failed_reviews:
        out += ["", "review delivery"]
        for f in failed_reviews:
            out.append(f"  FAILED {f['review_id']} -> {f['recipient']} "
                       f"({f['channel']}, {f['error']}) at {f['at']}")
        for rid in sorted({str(f["review_id"]) for f in failed_reviews}):
            out.append(hint("delivery_failed", detail=rid))

    out += _score_lines(h)

    out += ["", "blockers"]
    blockers = _blockers(h["wiki"])
    if blockers:
        for category, text in blockers:
            out.append(f"  {text}")
            out.append(hint(category))
    else:
        out.append("  (none)")
    if h["problems"]:
