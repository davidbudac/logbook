...
def history_tokens(block: str, day: str) -> frozenset[str]:
    ...
    tokens.update(patterns.extract_codes(block))
...
def run_item(item: dict, cfg, *, arm: str, history_days: int) -> ArmResult:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            refs = _repeated(proposal, history_tokens(block, day))
...
def run(items: list[dict], cfg, *, model: str, provider: str | None = None,
    ...
    resolved = (getattr(cfg, "agents", None) or {}).get(
        "history_days", structured.DEFAULT_HISTORY_DAYS)
...
def render_report(rows, *, db: str, model: str, days: int) -> str:
    ...
    for item, *results in rows:
        ...
        for label, cells in (
                ...
                ("flags", [str(r.verdict["flags"]) for r in results]),
                ("input tokens",
