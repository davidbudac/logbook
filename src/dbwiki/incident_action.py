...
#: The annotation a repeatable path field carries. Compared by value, because
#: `tuple[str, ...]` builds a fresh alias each time it is written.
_PATHS = tuple[str, ...]
...
class Action:
    ...
    - `at` is `YYYY-MM-DDTHH:MM:SSZ`, the record's identity: re-running the
    ...
    def __post_init__(self) -> None:
        ...
        if not incidents.ISO_Z_RE.match(self.at):
            raise FieldError(
                "at", f"at: {self.at!r} is not YYYY-MM-DDTHH:MM:SSZ")
...
class LockBusy:
    ...
    the portal answers 423. `holder` is `lock._holder`'s sentence, naming the
    pid, command and start time read back from the lock file."""
