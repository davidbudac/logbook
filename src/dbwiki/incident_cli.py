...
def _lock_wait(args) -> float:
    ...
    from .cli import _lock_wait_default
    wait = getattr(args, "lock_wait", None)
    return wait if wait is not None else _lock_wait_default()
...
def cmd_incident_apply(args) -> int:
    ...
    try:
        action = incident_action.decode(
            slug=args.slug, verb=verb, fields=_fields(args, verb), actor=actor,
            base=args.base or transaction.head(wiki), at=args.at or _now())
