...
class EvidenceRef:
    ...
    the record is rendered into the page."""
    ...
    def __post_init__(self) -> None:
        if not EVIDENCE_RE.match(str(self)):
...
class MonitoringWindow:
    ...
    def __post_init__(self) -> None:
        for name, value in (("start", self.start), ("until", self.until)):
...
class ActionRecord:
    ...
    identical bytes; a retry with a fresh `at` is a second record.
    ...
    ActionMalformed when: `at` is not `YYYY-MM-DDTHH:MM:SSZ`; `kind` is not
    in ACTION_KINDS; `actor`, `intent` or `summary` is empty; any field
    except `notes` spans more than one line; an `evidence` entry is not
    ...
    def __post_init__(self) -> None:
        ...
        if not ISO_Z_RE.match(self.at):
            raise ActionMalformed(
                f"at: {self.at!r} is not YYYY-MM-DDTHH:MM:SSZ")
        ...
        for name, value in scalars:
            if "\n" in value or "\r" in value:
                raise ActionMalformed(
                    f"{name}: must be one line, got {value!r}")
...
def render_action(record: ActionRecord) -> str:
    ...
    block = yaml.safe_dump({k: values[k] for k in ACTION_FIELDS if values[k]},
...
def read_incident(text: str, path: str) -> Incident:
    ...
    heading = next((ln[2:].strip() for ln in text.splitlines()
...
#: A wikilink or a backtick code span. Both are single-line constructs, so
#: masking per line is the whole of it.
LINK_OR_CODE_RE = re.compile(r"\[\[[^\]]*\]\]|`[^`]*`")
