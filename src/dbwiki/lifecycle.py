...
from .incidents import (ACTIVE, MERGE_SUMMARY, ActionMalformed, ActionRecord,
                        Incident, MonitoringWindow, Outcome, RecoverySignal,
                        Status, append_action, incident_path, read_incident,
                        set_status)
...
#: The statuses the page a `Merge` keeps may have. The table above is keyed
#: by the *dropped* page's status; this is the other half of the rule. Only
#: an active page can keep an incident: folding into a resolved page closes
#: the incident without a `Resolve`, and A->B then B->A closed both pages and
#: emptied the queue (issue 14).
MERGE_KEEPS: frozenset[Status] = ACTIVE
...
def resolution_row(incident: Incident,
    ...
    prefix = f"| {record.at[:10]} | {oneline(incident.db)} | [[{target}]] | "
    ...
    match command:
        ...
        case Resolve():
            fields = dict(intent="close the incident", summary=command.summary,
                          ...
                          error_pages=command.update_error_pages)
        case Reopen():
        ...
        case Merge():
            ...
            fields = dict(
                ...
                summary=f"{MERGE_SUMMARY}{kept}", outcome=Outcome.SUCCEEDED,
...
def _merged_into(tree: Tree, dropped: Incident, dropped_text: str,
    ...
    page this revision does not hold, into another database's page, into
    one whose own status is off-vocabulary, which would park carried evidence
    under a status no reader can classify, or into one that is not active
    (`MERGE_KEEPS`), which would close the incident without a `Resolve`.
    ...
    if kept.status not in MERGE_KEEPS:
        raise MergeRefused(dropped.path, path,
                           f"{path} is {kept.status}, not active; a merge "
                           f"folds a duplicate into the page that still "
                           f"tracks the incident, so reopen {path} first "
                           f"or merge the other way round")
    record = ActionRecord(
...
def _window_after(incident: Incident, command: Command,
    ...
    `until`, which must be later than both the current `until` and `at`:
    an "extension" that shortens the window, or one that ends before it is
    written, is refused (issue 14). Resolve, Reopen and Merge: None.
    Otherwise unchanged."""
    match command:
        ...
        case ExtendMonitoring(until=until):
            ...
            if window is None:
                raise ActionMalformed(
                    ...
                    f"window to extend")
            # A replay of this very extend finds its own `until` already in
            # force; equal is only refused for a fresh act.
            replay = any(r.at == at and r.kind == KINDS[ExtendMonitoring]
                         for r in incident.actions.records)
            if until < window.until or (until == window.until
                                        and not replay):
                raise ActionMalformed(
                    f"until: {until!r} is not later than the current until "
                    f"{window.until!r}; extend only pushes the window out")
            if until <= at:
                raise ActionMalformed(
                    f"until: {until!r} is not later than now ({at!r}); the "
                    f"window would be over before it is written")
