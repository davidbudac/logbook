...
from .prompts import drift as _prompt_drift
...
RULES: dict[str, tuple[Severity, str]] = {
    ...
    "generated-section-drift": (
        "warning", "run `uv run python -m dbwiki.prompts sync AGENTS.md` to "
        "re-render the generated section from config/prompts/"),
}
...
_INDEX = "index.md"
#: the agentic contract; its generated sections mirror config/prompts/shared/
...
def _frontmatter_findings(rel: str, text: str) -> list[Finding]:
    ...
    except Exception as e:  # noqa: BLE001 - `opened: 2026-02-30` is a ValueError
        first = (str(e).splitlines() or [type(e).__name__])[0]
        return [_f(rel, "frontmatter-malformed",
                   f"frontmatter is not valid YAML: {first}")]
...
def _generated_section_findings(rel: str, text: str) -> list[Finding]:
    """AGENTS.md's `<!-- BEGIN generated: … -->` sections against the prompt
    files they are rendered from (`dbwiki.prompts.drift`). No markers, no
    finding: a wiki adopts the section when its operator pastes it in. A
    warning, not an error: the structured prompts read the file itself, so a
    drifted copy misleads only the agentic mode, and blocking an ingest over
    it would stop the wiki for a text fix."""
    if rel != _AGENTS_MD:
        return []
    return [_f(rel, "generated-section-drift", msg)
            for msg in _prompt_drift(text)]
...
def lint_wiki(wiki_root: Path | str,
    ...
    for rel, text in sorted(texts.items()):
        ...
        findings += _generated_section_findings(rel, text)
...
def linkers(wiki_root: Path | str, targets: Collection[str]) -> set[str]:
    """The pages under `wiki_root` whose `[[links]]` resolve to one of
    `targets` — resolved against the pages present *plus* `targets`, so a
    page deleted from the tree still counts as the thing its linkers point
    at. A proposal that deletes or renames a page lints these too: its own
    paths look clean, and the broken links are all on somebody else's
    page."""
    wiki = Path(wiki_root)
    wanted = set(targets)
    if not wanted:
        return set()
    pages = _markdown_files(wiki)
    inventory = set(pages) | wanted
    out = set()
    for rel, path in pages.items():
        if rel.startswith("digests/") or rel in wanted:
            continue
        try:
            text = path.read_text(encoding="utf-8")
        except (UnicodeDecodeError, OSError):
            continue
        if any(resolve_link(t, inventory) in wanted
