...
BUSY = "another dbwiki command holds the lock"


@dataclass(frozen=True)
class Holder:
    """Who holds the lock, as the holder wrote it into the lock file. The
    busy sentence is a render of this; a caller that needs one fact (the
    portal asking "is it me?") reads the field."""
    ...
    def __str__(self) -> str:
        return f"{BUSY} (pid {self.pid}, {self.command}, since {self.since})"
...
class LockBusyError(RuntimeError):
    ...
    `holder` is who holds it, or None when the lock file could not say."""

    def __init__(self, message: str, holder: Holder | None = None):
        super().__init__(message)
        self.holder = holder


def _holder(fh) -> Holder | None:
    """Who holds the lock, read back from the lock file. Diagnostics only, so
    an empty or half-written file degrades to None — the message without the
    names — rather than to a second failure."""
    try:
        fh.seek(0)
        d = json.loads(fh.read() or "{}")
        return Holder(pid=int(d["pid"]), command=str(d["command"]),
                      since=str(d["since"]))
...
def lock_wait_default() -> float:
    ...
    0 — fail fast, which is what an operator at a terminal wants. Here rather
    than in `cli` so `incident_cli` reads it without importing `cli` back."""
    try:
    ...
    try:
        while True:
            try:
                ...
                if left <= 0:
                    holder = _holder(fh)
                    raise LockBusyError(str(holder or BUSY), holder) from None
