...
def _blocks(lines: Sequence[str], out: list[str], headings: list[Heading],
    ...
    while i < len(lines):
        ...
        if _ULI.match(line) or _OLI.match(line):
            ...
            continue
        # Every rule above either consumed `line` or declined it, so it is
        # prose. It is taken unconditionally: a `|` line that starts no table
        # (no separator row after it) is declined by the table rule *and*
        # would stop the loop below, and an unadvanced `i` spins forever.
        para = [line.strip()]
        i += 1
