...
def _flow_resumed(source: str, digests: Sequence[dict], closed: bool,
                  gaps: tuple[str, ...]
                  ) -> tuple[str, tuple[dict, ...], tuple[str, ...]]:
    observed: list[dict] = []
    found = False
    for d in digests:
        events = int(((d.get("sources") or {}).get(source)
                      or {}).get("total_events") or 0)
        observed.append({"day": _day(d), "digest": d.get("_path", ""),
...
def evaluate(incident: Incident, digests: Sequence[dict], *, now: str,
    ...
    events for the source meets it; every day at zero with the window closed
    refutes it. `manual` is never settled here, by design.
    ...
    classifies routine (`RFS[`, `Media Recovery Log`) can never match. And the
    day's last tick runs at 23:30, so a window ending at midnight has its
    final half hour in no digest: `met` means "nothing in the evidence
    collected"."""
...
class ClosureCase:
    ...
    verdict: str                  # met | not_met | insufficient | error
    evaluated_at: str
    source_revision: str
    stale: bool
