...
from .transaction import (BaseMoved, Committed, LintBlocked, NothingToDo,
                          TreeDirty)
from .trigger import digest_needs_escalation as _digest_needs_escalation
...
HISTORY_RULE = ('The "Database history" block is context from earlier days: '
                "use it to judge whether today is new, recurring, or follows "
                "a recorded change, and never cite it as evidence for today.")
...
def telemetry_fields(task: str, run_id: str, tele: dict, *, model_tier: str,
    ...
    message, a page body, or a credential (see DESIGN.md)."""
    r = result or {}
    fields = {
        "run_id": run_id,
        "task": task,
...
class Orchestrator:
    ...
    def _run_id(run_id: str | None) -> str:
        from .health import new_run_id  # health imports us; keep this lazy
    ...
    def _telemetry(self, task: str, run_id: str, tele: dict, *,
        ...
        `mode` and `prompt` exist for those records only and never reach the
        ledger block. Unlike the ledger, the Langfuse export carries content:
        the prompt and result JSON go to the configured host
        (docs/langfuse.md)."""
        try:
            fields = telemetry_fields(task, run_id, tele, **kw)
            ...
            fields = {"run_id": run_id, "task": task, "telemetry_error": msg}
        self.last_telemetry = fields
        from .health import record_agent_run  # health imports us; keep lazy
    ...
    def digest_needs_escalation(digest: dict) -> bool:
        """Delegates to trigger.digest_needs_escalation — kept as a thin
        staticmethod so existing callers/tests still work."""
        return _digest_needs_escalation(digest)
    ...
    def _rollback(self, base: str) -> None:
        """Return the agent's writes to `base`, leaving anything a human
        staged during the run where they put it."""
        staged = set(transaction.staged_paths(self.wiki))
        transaction.restore(
            self.wiki,
            [p for p in transaction.stray_paths(self.wiki) if p not in staged],
            base)

    def _refused(self, proposal: transaction.Proposal, base: str,
                 refused: transaction.Outcome) -> RuntimeError:
        ...
        return RuntimeError(f"wiki changed under the lock: {refused}")
    ...
    def ingest(self, db: str, digest_json: Path, dry_run: bool = False,
        ...
        escalate = self.digest_needs_escalation(digest)
        w = digest["window"]
        ...
        def block(**kw) -> dict:
            fields = self._telemetry("ingest", run_id, tele, model_tier=tier,
                                     digest_bytes=digest_bytes, db=db,
                                     mode="structured" if structured
                                     else "agentic", prompt=prompt, **kw)
            if structured:  # same dict object as self.last_telemetry
                fields["mode"] = "structured"
            return fields
        ...
        try:
            if structured:
                result = self._structured_ingest(db, digest, prompt,
                                                 rel_md.as_posix(), escalate,
                                                 tele)
            else:
                result = self._run_agent_with_retry(
                    self.adapter, prompt, self._model(escalate), self.timeout,
                    provider=self._provider(), tele=tele, validate=validate)
        except NoResultError as e:
            result = self._synthesize_result("ingest", db, digest["notable"], e)
            if result is None:
                self._fail_ledger(ledger_key, content_hash, categorize(e),
                                  [str(e)[:500]],
                                  block(validation_ok=False, rolled_back=False))
                raise
        except HarnessError as e:
            self._rollback(base)
            self._fail_ledger(ledger_key, content_hash, categorize(e),
        ...
        proposal = self._propose(
            base, f"ingest: {db} {w['day']} — {result.get('summary', '')[:100]}",
            run_id)

        def refuse(problems: list[str], lint_findings: int = 0) -> ValidationError:
            transaction.restore(self.wiki, proposal.paths, base)
            self._fail_ledger(ledger_key, content_hash, "validation_failed",
                              problems,
                              block(result=result, validation_ok=False,
                                    rolled_back=True,
                                    lint_findings=lint_findings))
            return ValidationError("; ".join(problems))
        ...
        match self._publish(proposal):
            ...
            case LintBlocked(findings=findings):
                raise refuse([f"{f.file}: {f.rule}: {f.message}"
    ...
    def report(self, day: str, window: tuple[str, str], ingested: list[dict],
        ...
        analyst_cfg = getattr(self.cfg, "analyst", {}) or {}
        ...
        def block(**kw) -> dict:
            fields = self._telemetry("report", run_id, tele, model_tier=tier,
                                     mode="structured" if structured
                                     else "agentic", prompt=prompt, **kw)
            if structured:  # same dict object as self.last_telemetry
                fields["mode"] = "structured"
        ...
        try:
            if structured:
                result = self._structured_report(day, window, ingested, prompt,
                                                 suffix, health, tele,
            ...
            # the report page is already on disk and paid for; only the result
            # JSON is missing (see CHANGELOG, report NoResult). Synthesize it and
            # let the normal validation below judge the work — a run that
            # wrote no report page still fails there, exactly as before.
            ...
            if (structured or not escalate or fallback != "structured"
                ...
                raise
            # the rebinds are load-bearing: block() and the Langfuse export
            # read `structured`, `prompt` and `tele` at call time, so the
            # second attempt's ledger row must describe the structured run and
            # not the dead agentic one. `tele` starts fresh because the
            # agentic dict still holds that run's exit_code/timed_out.
            from .structured import build_escalated_report_prompt
            structured = True
            prompt = build_escalated_report_prompt(
                day, window, ingested, self.wiki, health=health)
        ...
        if isinstance(result, dict):
            result.setdefault("notable", escalate)
        proposal = self._propose(
            base, f"report: {day}{suffix} — {result.get('summary', '')[:100]}",
            run_id)

        def refuse(problems: list[str], lint_findings: int = 0) -> ValidationError:
            transaction.restore(self.wiki, proposal.paths, base)
            block(result=result, validation_ok=False, rolled_back=True,
                  lint_findings=lint_findings)
            return ValidationError("; ".join(problems))
    ...
    def run_queued_report(self, request: dict) -> dict:
        ...
        try:
            result = self._run_agent_with_retry(
                self.adapter, request["prompt"], self._model(True), self.timeout,
                provider=self._provider(), tele=tele, validate=validate)
        except NoResultError as e:
            result = self._synthesize_result("report", None, True, e)
            if result is None:
                fields = self._telemetry("report", run_id, tele,
                                         model_tier="strong",
                                         prompt=request["prompt"],
                                         validation_ok=False, rolled_back=True)
                self._rollback(base)
                fail_request(self.wiki, request, categorize(e),
        ...
        if problems:
            fields = self._telemetry("report", run_id, tele, model_tier="strong",
                                     prompt=request["prompt"],
                                     result=result, validation_ok=False,
                                     rolled_back=True, lint_findings=len(lint))
            transaction.restore(self.wiki, proposal.paths, base)
            fail_request(self.wiki, request, "validation_failed",
                        telemetry_record=fields, push=push, lock=self.lock)
    ...
    def lint(self, run_id: str | None = None) -> dict:
        ...
        def schema(result: dict, changed: Sequence[str]) -> list[str]:
            return [p for p in self._validate(result, "lint", changed)
                    if "no changes" not in p and "log.md" not in p]

        def validate(result: dict) -> list[str]:
            proposal = self._propose(base, "lint")
            return (schema(result, proposal.paths)
                    + self._incident_problems(proposal, base)
                    + self._blocked(proposal))
        ...
        def refuse(problems: list[str], lint_findings: int = 0) -> ValidationError:
            ...
            self._telemetry("lint", run_id, tele, model_tier="cheap",
                            ...
                            validation_ok=False,
                            rolled_back=True, lint_findings=lint_findings)
            return ValidationError("; ".join(problems))

        if problems := (schema(result, proposal.paths)
                        + self._incident_problems(proposal, base)):
            raise refuse(problems)
        match self._publish(proposal):
            case Committed() | NothingToDo():
    ...
    def _exchange_root(self) -> Path:
        ...
        rcfg = getattr(self.cfg, "research", {}) or {}
        ...
        if not root.is_absolute():
            root = Path(getattr(self.cfg, "root", Path.cwd())) / root
    ...
    def _fold_exchange(self, xroot: Path, *, push: bool, today: dt.date,
        ...
        from .research_offload import (apply_research_result,
                                       apply_source_review_result,
        ...
        for path in files:
            ...
            if kind == "research":
                problems = validate_research_result(res, self.wiki)
                key = f"{run_id}-{res.get('code')}"
                if not problems and not Redactor.mapping_path(self.cfg.state_dir, key).is_file():
                    problems.append(f"unknown run_id {run_id!r} (no redaction mapping "
                                    f"on-prem for {key})")
                if problems:
                    ex.reject_result(xroot, path, "; ".join(problems))
                    flags.append(f"exchange: rejected {path.name}: {problems[0]}")
                    continue
                red = Redactor.load(self.cfg.state_dir, key)
                clean = red.demap_obj(res)
                rel = f"errors/{res['code']}.md"
                apply_research_result(self.wiki, rel, clean, today)
                subject = f"research (offload) — {rel}"
            elif kind == "source-review":
                problems = validate_source_review_result(res, self.wiki)
                if problems:
                    ex.reject_result(xroot, path, "; ".join(problems))
                    flags.append(f"exchange: rejected {path.name}: {problems[0]}")
                    continue
                rel = f"sources/{res['slug']}.md"
                apply_source_review_result(self.wiki, rel, res, today)
                ...
                subject = f"source review (offload) — {rel}"
            else:
                ex.reject_result(xroot, path, f"unknown result kind {kind!r}")
                flags.append(f"exchange: rejected {path.name}: unknown kind")
                continue
    ...
    def research(self, pages: list[str], review_sources: list[str] | None = None,
        ...
        def block(**kw) -> dict:
            # structured research never binds `prompt` (per-page prompts are
            # built inside _structured_research); the conditional short-circuit
            # keeps the closure from touching the unbound name
            fields = self._telemetry("research", run_id, tele, model_tier=tier,
                                     mode="structured" if structured
                                     else "agentic",
                                     prompt=None if structured else prompt, **kw)
            if structured:  # same dict object as self.last_telemetry
                fields["mode"] = "structured"
            return fields
        ...
        try:
            result = self._run_agent_with_retry(
                ...
                provider=self._provider(adapter), tele=tele, validate=validate)
        except NoResultError as e:
    ...
    def caveats(self, pages: list[str], run_id: str | None = None) -> dict:
        ...
        from .research_caveats import (apply_caveats,
                                       ...
                                       build_caveats_prompt, propose_caveats)
        ...
        def record(**kw) -> None:
            tele.update(
                ...
                usage=({"input_tokens": int(in_tok),
                        ...
                        "cost_usd": round(cost, 6) if cost_known else UNKNOWN}
                       if have_usage else UNKNOWN))
            fields = self._telemetry("research", run_id, tele, mode="caveats",
                                     model_tier="cheap", **kw)
            # same dict object as self.last_telemetry: which research mode ran
            # is what an operator reading the run-health event needs, and the
            # ledger block deliberately carries no mode
            fields["mode"] = "caveats"
        ...
        try:
            for rel in pages:
                ...
                try:
                    call_tele: dict = {}
                    answer = propose_caveats(
                        build_caveats_prompt(code, found.cause, found.action,
                                             sources),
                        self.cfg, telemetry=call_tele)
                    calls += 1
                    if isinstance(call_tele.get("duration_s"), (int, float)):
    ...
    def past_fixes(self, dry_run: bool = False,
        ...
        def record(**kw) -> None:
            fields = self._telemetry("research", run_id, tele, mode="history",
                                     model_tier="none", **kw)
            # same dict object as self.last_telemetry: which research mode ran
            # is what an operator reading the run-health event needs
            fields["mode"] = "history"
