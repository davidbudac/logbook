...
def _lines(text: str) -> list[str]:
    """The page's lines, split on `\\n` alone. `str.splitlines` also splits
    on `\\f`, `\\x85`, U+2028 and a CRLF's `\\r`, and the `"\\n".join` every
    editor here ends with would then rewrite those bytes on lines it never
    targeted."""
    return text.rstrip("\n").split("\n")


class _TextDatesLoader(yaml.SafeLoader):
    """`SafeLoader` with no timestamp resolver: a date-shaped scalar stays the
...
def frontmatter(text: str) -> dict:
    ...
    Valid YAML whose date-shaped scalar is no real date (`opened:
    2026-02-30`) makes PyYAML's timestamp constructor raise a bare
    `ValueError`, not a `YAMLError`. Such a block is read a second time with
    dates left as text, so the page keeps its `type` and `status` and stays in
    the incident queue while lint blocks it; anything else PyYAML raises reads
    as unparseable. Never raises."""
    m = FM_RE.match(text)
    if not m:
        return {}
    try:
        ...
        return {}
    except Exception:  # noqa: BLE001 - a constructor error, see above
        try:
            data = yaml.load(m.group(1), Loader=_TextDatesLoader)
...
def set_frontmatter(text: str, updates: Mapping[str, str | None]) -> str:
    ...
    A touched key's value may span lines a human wrote in block style
    (`monitoring:` then indented `kind:`/`until:` lines, or a `- item`
    sequence): those continuation lines go with the key, whether it is
    replaced or deleted, or they would be left dangling under the previous
    key and the block would stop parsing (issue 14).
    ...
    for line in m.group(1).split("\n"):
        if dropping and _continues(line):
            continue
        dropping = False
...
def _continues(line: str) -> bool:
    """Whether a frontmatter line belongs to the value of the key above it:
    indented, or a block-sequence item at column 0 (`- a`), which YAML
    allows directly under a mapping key."""
    return line[:1] in (" ", "\t") or line == "-" or line.startswith("- ")
