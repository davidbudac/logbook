...
# Hex literals fold whole, before the digit fold: `ADDR:0x7FFE3A` and
# `ADDR:0x7EDC9F` must share a template, or an ORA-7445 storm (one address
# per core dump) fills the group cap by itself. A placeholder keeps the `0`
# of `0x#` out of the digit fold.
_HEX_RE = re.compile(r"(?<![0-9A-Za-z])0[xX][0-9A-Fa-f]+\b")
_HEX_MARK = "\x00"
...
def message_template(msg: str, width: int = 160) -> str:
    t = _HEX_RE.sub(_HEX_MARK, msg.replace(_HEX_MARK, ""))
    t = _NUM_RE.sub("#", t).replace(_HEX_MARK, "0x#")
