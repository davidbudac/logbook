...
`WORKBENCH_CSS`, the single `/*@QUIET_DAYS@*/` placeholder with
`readmodel.QUIET_DAYS`, and the single `/*@MAX_CHANGE_ROWS@*/` placeholder
with `daily_html.MAX_CHANGE_ROWS`, and nothing else: no template language, no
...
.rcard .rfix, .rcard .rpast { margin: .45rem 0 0; font-size: .75rem;
  overflow-wrap: anywhere; }
.rcard .rfix .cap, .rcard .rpast .cap { color: var(--muted); }
.rcard .rfix ul, .rcard .rpast ul { margin: .2rem 0 0; padding-left: 1.1rem; }
.rcard .rfix li, .rcard .rpast li { margin: .1rem 0 0; }
...
#: Where a day's changes strip stops and says how many it left out. The
#: daily page's strip stops at `daily_html.MAX_CHANGE_ROWS`, and the portal's
#: day screen draws the same rows, so it stops at the same count.
_CHANGES_PLACEHOLDER = "/*@MAX_CHANGE_ROWS@*/"
...
def page() -> str:
    """The served page: `workbench.html` with the CSS, the quiet threshold
    and the changes cap substituted. Deterministic; two calls return
    identical bytes. Raises RuntimeError when a placeholder is missing, because a page served
    ...
    for marker in (_PLACEHOLDER, _QUIET_PLACEHOLDER, _CHANGES_PLACEHOLDER):
    ...
    return (template
            ...
            .replace(_CHANGES_PLACEHOLDER, str(daily_html.MAX_CHANGE_ROWS)))
