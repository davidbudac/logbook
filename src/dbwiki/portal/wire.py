...
from ..incident_action import Action, Field, Presentation, Widget
...
from ..readmodel import (Hit, JournalEntry, Origin, PastFixRow, Research,
                         Resolution, Snapshot)
...
#: The longest suppression an inbox body may ask for. The page offers 7, 14
#: and 30; a year is room for a deliberate long hush, and the bound keeps a
#: `days` of a billion from overflowing the calendar in `review.suppress`.
MAX_SUPPRESS_DAYS = 365
...
#: widget -> the JSON shape its control posts. One row per `Widget`, so a new
#: widget without a transport fails a test rather than posting a string; that
#: test also checks each field's row against the annotation
#: `incident_action._coerce` dispatches on, so a control cannot be drawn
#: expecting one shape and coerced expecting another.
TRANSPORT_OF: dict[Widget, str] = {
    Widget.LINE: "string",
    Widget.TEXT: "string",
    Widget.CHOICE: "string",
    Widget.INSTANT: "string",
    Widget.LIST: "list",
    Widget.SIGNAL: "object",
    Widget.FLAG: "boolean",
}
...
def _check_at(at: object) -> None:
    """A body's optional `at`: absent, or an instant that exists on the
    calendar. The shape alone let `2026-02-30T10:00:00Z` through to a record
    or to a suppression counted from nowhere."""
    if at is not None and not validate.is_instant(at):
        raise BadRequest("bad_at",
                         f"at: {at!r} is not a real YYYY-MM-DDTHH:MM:SSZ "
                         f"instant")
...
class InboxRequest:
    ...
    def from_json(cls, body: Mapping[str, object]) -> "InboxRequest":
        ...
        `action` (one of `ACTIONS`), `days` (1..MAX_SUPPRESS_DAYS, suppress only),
        ...
        if action == "suppress" and (isinstance(days, bool)
                                     ...
                                     or not 0 < days <= MAX_SUPPRESS_DAYS):
            raise BadRequest("bad_days",
                             f"days: {days!r} is not a number of days to "
                             f"suppress for, 1 to {MAX_SUPPRESS_DAYS}")
        at = body.get("at")
        _check_at(at)
...
def field_json(verb: str, field: Field, pres: Presentation) -> dict:
    ...
    which says only how to draw the control; `TRANSPORT_OF` maps the widget
    to its shape and is tested against the annotation
    `incident_action._coerce` dispatches on, so what the page posts cannot be
    a shape the decoder refuses.
...
def action_json(record: ActionRecord) -> dict:
    ...
    path the page itself was written with; `error_pages` is false only on a
    resolve that declined the error-page rows."""
    return {"at": record.at,
            "kind": record.kind,
            "actor": record.actor,
            ...
            "notes": record.notes,
            "error_pages": record.error_pages}
...
def research_json(code: str, *, exists: Callable[[str], bool],
                  ...
                  past_fixes: Mapping[str, tuple[PastFixRow, ...]]) -> dict:
    """One error code as the incident screen reads it: `{"code", "path",
    "exists", "researched", "cause", "action", "citations", "notes",
    "resolutions", "past_fixes"}`, the citations a list of `{"source", "url", "accessed"}`
    ...
    `past_fixes` is the error page's `## Past fixes` table in page order
    (newest first, as `past_fixes.render` writes it), one `{"day", "db",
    "incident", "path", "action", "outcome", "held"}` row apiece and an empty
    list on a page without one. It is wider than `resolutions`: every fix
    anybody recorded against the code, the ones that failed or were
    rejected included, and whether each held. `held` is the verdict as the
    page spells it (`held (14d)`, `reopened 2026-07-20`), a string to show.
    ...
    return {"code": code,
            ...
            "past_fixes": [{"day": f.day,
                            "db": f.db,
                            "incident": f.incident,
                            "path": f"incidents/{f.incident}.md",
                            "action": f.action,
                            "outcome": f.outcome,
                            "held": f.held}
                           for f in past_fixes.get(code, ())]}
...
def change_row_json(*, db: str, change: Change, page: str) -> dict:
    """One administrative act a digest recorded: `{"db", "day", "ts", "rule",
    "count", "message", "page"}`.

    The five `Change.to_dict` keys, which are what the compactor writes under
    a digest's `changes`, plus the two a fleet timeline needs: whose change
    it was, and `page`, the `digests/<db>/<day>.md` that lists it, or `""`
    when the revision holds no such page to open."""
    return {"db": db, **change.to_dict(), "page": page}


def page_json(*, prov: dict, body: dict, links: tuple[dict, ...],
              changes: tuple[dict, ...] = ()) -> dict:
    ...
    `changes` is `change_row_json` per change the page's day recorded, as one
    timeline: a digest's own, or on a day's report every database's. Every
    other page, and a day on which nobody changed anything, answers an empty
    list rather than no key, so the screen has one shape to draw."""
    return {**prov, **body, "links": list(links), "changes": list(changes)}
...
def incident_json(inc: Incident, *, revision: str, body: str,
                  ...
                  research: Mapping[str, Research],
                  resolutions: Mapping[str, tuple[Resolution, ...]],
                  past_fixes: Mapping[str, tuple[PastFixRow, ...]],
                  last_seen: str | None) -> dict:
    ...
    `resolutions` and `past_fixes` are not keys of their own. They are
    threaded into every `research_json` row, so each card carries the history
    of the code it names and the screen never has to join two lists by code
    itself.
...
def outcome_json(outcome: incident_action.Published, *, base: str, at: str,
    ...
    A `LockBusy` whose `held_by` is this process reads "another workbench
    request is publishing" in both `message` and `holder`: that is a
    portal-versus-portal collision, and naming our own pid back reads as the
    portal blocking itself. Two spellings of one refusal on one screen would
    be worse than either, so both copies change together.
    ...
    match outcome:
        ...
        case LintBlocked(findings=findings):
            return error_json("lint_blocked",
                              ...
                              **pinned)
        case incident_action.LockBusy(holder=holder, held_by=held_by):
            if held_by is not None and held_by.pid == os.getpid():
                holder = SELF_HOLDER
