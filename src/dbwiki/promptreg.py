...
Code is the source of truth. This module never fetches a prompt to run it: it
...
def _agents_md(cfg) -> str | None:
    root = getattr(cfg, "wiki_repo", None)
    if root is None:
        return None
    try:
...
def sync_prompts(cfg) -> list[str]:
    ...
    from .observability import _get_client
    client = _get_client(getattr(cfg, "langfuse", None) or {})
    if client is None:
