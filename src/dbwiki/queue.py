...
* **the on-prem tick vs the analyst's pushes** — `pull_before_fold`, at the
  start of every `dbwiki run` with the analyst on, rebases the on-prem side
  onto the remote. Same path partition, but here the *analyst's* version of
  `reports/<day><suffix>.md` is kept (`-X ours`): an agentic report beats
  the placeholder the on-prem side has not managed to push yet.
...
#: A claim older than this is a crashed analyst's; `claim` returns it to
#: pending/. Same default as `analyst.stale_hours`, which `dbwiki health`
#: reads (health.DEFAULT_ANALYST_STALE_HOURS).
...
def _hours_since(stamp, now: dt.datetime) -> float | None:
    try:
        then = dt.datetime.strptime(str(stamp), "%Y-%m-%dT%H:%M:%SZ")
    except ValueError:
        return None
    return (now - then.replace(tzinfo=dt.timezone.utc)).total_seconds() / 3600


def reclaim_stale(wiki: Path, *, lock: "Held",
                  stale_hours: float = STALE_CLAIM_HOURS,
                  now: dt.datetime | None = None) -> list[str]:
    """Return every claim older than `stale_hours` to `pending/` — the
    analyst that took it crashed or was killed before `complete()` or
    `fail_request()` — in one commit, and return the file names. The dead
    run counts as an attempt, exactly as `fail_request` counts one, so a
    request that keeps killing its analyst ends in `failed/` (category
    `stale_claim`) instead of cycling forever. A claim whose `claimed_at`
    does not parse is left alone: its age is unknown, and `dbwiki health`
    already names it.

    Before this, `health` reported a stale claim and a human had to move the
    file back by hand; until then the request was simply never run."""
    ...
    for name, rec in _read_records(dirs["claimed"]):
        ...
        (dirs["claimed"] / name).unlink(missing_ok=True)
        _write_record(target, record)
        moved.append(name)
    if moved:
        _publish(wiki, f"queue: reclaim {len(moved)} stale claim(s)",
                 "reclaim", lock)
    return moved
...
def claim(wiki: Path, claimed_by: str, *, lock: "Held", kind: str | None = None,
    ...
    (`reclaim_stale`), so a crashed analyst's request is picked up again by
    the next claim rather than waiting for a human; that commit rides out
    with the claim's own push."""
    if push:
        _git(wiki, "pull", "--rebase")
    reclaim_stale(wiki, lock=lock, stale_hours=stale_hours)
...
def pull_before_fold(wiki: Path) -> int:
    """The on-prem side's pull, run by `dbwiki run` right before
    `fold_results` when `analyst.enabled` and `report.push` are both on.
    Returns how many remote commits came in.

    Without it nothing on-prem ever pulled: after the analyst's first push
    every on-prem push was rejected non-fast-forward (and only warned about,
    `push_best_effort`), and `fold_results` never saw `queue/results/`.

    Rebase with autostash, so the digests/ and html/ output a tick keeps
    uncommitted rides along; `-X ours`, so on the one path both nodes write,
    `reports/<day><suffix>.md`, the **analyst's** pushed version wins over an
    unpushed on-prem one — the agentic report superseding the structured
    placeholder is the whole point of the queue. The opposite choice from
    `_push_rebase_retry`, which keeps the rebasing side's content: that side
    is the analyst there too. The inversion again: rebasing local onto
    upstream, upstream is git's "ours" (pinned by
    tests/test_onprem_pull.py against real git).

    All or nothing (`gitutil.pull_rebase`): on any failure the wiki is back
    where it was and `PullFailed` says why; the caller warns and ticks on."""
    from .gitutil import pull_rebase
    return pull_rebase(wiki, strategy_option="ours")
...
def fold_results(wiki: Path, state_dir: Path, *, push: bool = True) -> int:
    ...
    folded = 0
    # pathspec on the staged-diff check and the commit alike, the ownership
    # rule `Orchestrator._commit_machine_dir` applies: a bare `git commit`
    # would sweep whatever a human had staged into "queue: fold …"
    spec = _rel(wiki, results)
    try:
        for p in files:
            try:
            ...
            p.unlink(missing_ok=True)  # an untracked result `rm` never matched
        if not _git(wiki, "diff", "--cached", "--name-only", "--",
                    spec).strip():
            return folded  # untracked results only: nothing to commit
        _git(wiki, "commit", "-m", f"queue: fold {len(files)} analyst result(s)",
             "--", spec)
    except BaseException:
        # the folded events are in agent_runs.jsonl already and dedupe by
        # event id, so putting the files back only costs a re-read next tick;
        # leaving them staged as deletions would make every stage of this
        # tick refuse on a dirty tree
        _git(wiki, "reset", "-q", "--", spec, check=False)
        _git(wiki, "checkout", "--", spec, check=False)
        raise
    if push:
