...
one `transaction.last_commits` walked from that same revision. Digest pages
stay in the inventory for link resolution and are never read, the
`lint_wiki` rule that keeps 787 of 982 files closed.
...
DIGEST_PAGE_RE = re.compile(r"\Adigests/([^/]+)/([^/]+)\.md\Z")
#: One database's sidecar for one whole day, by the name the compactor writes
#: and `daily_html.day_digests` globs (`*/<day>.json`), so a suffixed file is
#: never read as another day's.
DIGEST_SIDECAR_RE = re.compile(
    r"\Adigests/([^/]+)/(\d{4}-\d{2}-\d{2})\.json\Z")
...
@dataclass(frozen=True)
class PastFixRow:
    """One row of an error page's `## Past fixes` table: on this day, on this
    database, this was done on this incident, it came out like this, and
    this is whether it held.

    Read back from the page rather than recomputed with `past_fixes.gather`,
    because the verdict needs the whole incident corpus and a `today`, and
    the page already holds the answer the stage wrote at its last run. The
    cells are kept as the page spells them: `outcome` is the
    `incidents.Outcome` word and `held` the rendered verdict (`held (14d)`,
    `reopened 2026-07-20`, `too soon`, ...), strings to show rather than
    values to reason over. `incident` is the bare slug, as on `Resolution`,
    and a literal pipe in `action` stays backslash-escaped for the same
    reason `Resolution` gives."""
...
class Snapshot:
    ...
    `past_fixes` is keyed the same way again and holds only the codes whose
    page carries a `## Past fixes` table: every fix anybody recorded against
    the code, not only the ones that closed an incident, with whether each
    held. Same reason for riding along.
    ...
    def change_sidecars(self, path: str) -> tuple[str, ...]:
        """The digest sidecars the changes shown on the page at `path` are
        read from, path-sorted.

        A digest page reads its own `.json` beside it, when the revision holds
        one. A report is the wiki's page for a day, so it reads every
        database's `digests/<db>/<day>.json` for its day — the exact name
        `daily_html.day_digests` globs, which is what keeps the portal's day
        screen and the daily page on one roster. Every other page reads none.

        Paths only: the sidecars stay closed in the snapshot for the reason
        `build` gives about digests, and the endpoint reads these few at the
        snapshot's revision when a page asks."""
        if DIGEST_PAGE_RE.match(path):
            sidecar = path.removesuffix(".md") + ".json"
            return (sidecar,) if sidecar in self.inventory else ()
        report = REPORT_PAGE_RE.match(path)
        if report is None:
            return ()
        return tuple(sorted(
            p for p in self.inventory
            if (m := DIGEST_SIDECAR_RE.match(p)) and m.group(2) == report[1]))
...
def parse_past_fixes(code: str, text: str) -> tuple[PastFixRow, ...]:
    """The `## Past fixes` table of one `errors/<code>.md` page, in page
    order. The header row, the `|---|` separator and any malformed row fail
    the pattern and are skipped rather than raised.

    Page order, unlike `parse_resolutions`: `past_fixes.render` already
    writes newest first, and it breaks same-day ties by record time, which
    the table does not carry. A re-sort by day here could only lose that.

    The row pattern is owned here rather than imported from `past_fixes`,
    which imports this module; the round trip is pinned by a test that
    parses what `past_fixes.render` writes."""
    rows = []
    for heading, body in sections(text):
        if heading[3:].strip().casefold() != "past fixes":
            continue
        for line in body.splitlines():
            match = PAST_FIX_ROW_RE.match(line.strip())
            if match is not None:
                rows.append(PastFixRow(code, *match.groups()))
    return tuple(rows)
...
def build(wiki: Path, revision: str, *,
    ...
    paths, one `transaction.last_commits` (pinned to `revision`, never
    HEAD), one `read_touches`. Everything after those is pure parsing over
    ...
    commits = transaction.last_commits(wiki, revision)
    built_at = now()
    ...
    past_fixes: dict[str, tuple[PastFixRow, ...]] = {}
    ...
    for path in readable:
        ...
        try:
            ...
            resolved = (parse_resolutions(error.group(1), body)
                        if error else ())
            fixes = parse_past_fixes(error.group(1), body) if error else ()
    ...
    return Snapshot(
        ...
        past_fixes=dict(sorted(past_fixes.items())),
