...
_RESEARCH_CONTRACT = """Reply with ONE JSON object and NOTHING else: no prose
before or after it, no markdown code fences, no explanation.

{
  "cause": "plain factual prose: what causes this error, distilled only from the page text below",
  "action": "plain factual prose: what to do about it; a short '- ' bullet list in one string is fine"
}"""
...
def build_research_prompt(code: str, page_text: str) -> str:
    ...
    return (
        f"You are summarizing an Oracle documentation page about error "
        f"{code} for an operations wiki.\n"
        f"{_RESEARCH_CONTRACT}\n\n"
        f"Rules:\n"
        f"- State only what the page text below supports. Never invent a "
        f"cause or an action step the text does not show.\n"
        f"- No URLs and no markdown headings in either field.\n"
        f'- Plain factual prose; "action" may be a short "- " bulleted '
        f"list written as one string.\n\n"
        f"Page text for {code}:\n---\n{page_text}\n---\n"
    )
...
def parse_research_proposal(text: str) -> dict:
    ...
    return {"cause": _str(obj, "cause", max_len=MAX_FIELD_CHARS),
