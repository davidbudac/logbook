...
The review takes no wiki lock. It reads the wiki at a pinned head and never
touches its working tree, and it writes only under `.state/review/`, where
the two writers are separated by file rather than serialized:
`state.json` and `reviews/<id>.json` are the cron stage's, `acks.json` is
the portal's, and they are merged at read. The one lock is inside the
portal's side: `acknowledge` and `suppress` read-modify-write `acks.json`,
so they hold an in-process lock (two portal threads) and an `fcntl.flock`
on `acks.lock` (a second process) around that cycle.
"""
...
from . import (alerts, delivery, health, monitoring, prompts, readmodel,
               scores, structured, transaction, validate)
...
#: Serialises the portal's threads; the flock in `_acks_locked` serialises
#: processes. flock alone would do for both on Linux, where each `open` is its
#: own lock, but the in-process lock keeps the rule independent of that.
_ACKS_THREAD_LOCK = threading.Lock()


@contextlib.contextmanager
def _acks_locked(state_dir) -> Iterator[None]:
    """Hold the acks lock for one read-modify-write of `acks.json` and its
    audit row. Blocking and short: the section reads and writes one small
    file, so a second act waits milliseconds rather than being refused."""
    path = _dir(state_dir) / ACKS_LOCK
    path.parent.mkdir(parents=True, exist_ok=True)
    with _ACKS_THREAD_LOCK, open(path, "a") as fh:
...
def acknowledge(state_dir, fingerprint: str, *, actor: str, now: str) -> dict:
    ...
    `acks.json` byte-identical and appends no audit row. Under
    `_acks_locked`, so two acts at once both land."""
    with _acks_locked(state_dir):
        acks = load_acks(state_dir)
        items = acks["items"]
        item = dict(items.get(fingerprint) or _empty_item())
        if item.get("acknowledged_at"):
            return item
        item.update({"acknowledged_at": now, "actor": actor,
                     "updated_at": now})
        items[fingerprint] = item
        save_acks(state_dir, acks)
        _audit(state_dir, "acknowledged", at=now, fingerprint=fingerprint,
               actor=actor)
    _score(state_dir, fingerprint, "acknowledge", now)
    return item
...
def suppress(state_dir, fingerprint: str, *, actor: str, now: str,
    ...
    Under `_acks_locked`, `acknowledge`'s rule."""
    if days <= 0:
    ...
    with _acks_locked(state_dir):
        ...
        if (item.get("suppressed_until") or "") >= until:
            return item
        item.update({"suppressed_until": until, "actor": actor,
                     "updated_at": now})
        items[fingerprint] = item
        save_acks(state_dir, acks)
        _audit(state_dir, "suppressed", at=now, fingerprint=fingerprint,
               actor=actor, until=until)
    _score(state_dir, fingerprint, "suppress", now)
    return item


def _score(state_dir, fingerprint: str, decision: str, now: str) -> None:
    """Queue the decision as a human-feedback score on the review synthesis
    trace (`scores.queue_review_decision`), outside the acks lock. Only an
    act that wrote reaches here, so an idempotent repeat queues nothing."""
    scores.queue_review_decision(state_dir, fingerprint, decision, now)
...
_SYNTHESIS_RULES = prompts.load("review-synthesis") + "\n"
...
def _synthesis_stage(evidence, cfg, *, rules: Rules, state_dir,
    ...
    try:
        ...
        _audit(state_dir, "synthesis_failed", review_id=review_id, at=now,
               category=error)
    fields = _agent_fields(review_id, rules, telemetry, synthesis is not None)
    event_id = health.record_agent_run(state_dir, fields, telemetry)
    # exported like every other stage, so an inbox decision has a trace to
    # score (`scores.review_target`); inert unless `langfuse.enabled`
    from . import observability
    observability.record_agent_run(
        cfg, fields, telemetry,
        result=synthesis.to_dict() if synthesis else None, event_id=event_id)
...
def run(cfg, *, now: str, force: bool = False, deliver_only: bool = False,
        dry_run: bool = False, review_id: str | None = None) -> RunOutcome:
    ...
    one file read and no model call — but it still retries whatever delivery
    the audit does not hold as delivered (an inbox row and every email that
    already went are skipped, so this sends nothing twice); `force`
    republishes over it. `review_id` points `deliver_only` at an earlier
    week, whose failed email would otherwise be out of reach. Synthesis
    ...
    if review_id is not None and not deliver_only:
        raise ValueError("review id: only --deliver-only takes one; a new "
                         "review is always this week's")
    rules = Rules.resolve(cfg)
    policy = delivery.Policy.resolve(cfg)
    review_id = (iso_week(now) if review_id is None
                 else validate.review_id(review_id))
    state_dir = Path(cfg.state_dir)
    ...
    if existing is not None and not force:
        delivered, failed = (0, 0) if dry_run else _delivery_counts(
            _deliver(state_dir, existing, policy, now=now))
        return RunOutcome(review_id, False, "exists",
                          len(existing.get("findings") or []),
                          bool(existing.get("synthesis")),
                          str(existing.get("synthesis_error") or ""),
                          delivered, failed)
