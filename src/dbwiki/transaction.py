...
from . import gitutil
from .lint import Finding, blocking, lint_wiki
...
def last_commits(wiki: Path) -> dict[str, Commit]:
    """The newest commit that touched each path, over the whole history.
    ...
    answers `{}`, the way `page_history` answers `()`."""
    fmt = ("%x1e%H%x00%h%x00%cI%x00%ae%x00%s%x00"
           "%(trailers:key=Actor,valueonly)")
    try:
    ...
    if changed:
        with _candidate(wiki, proposal) as wt:
            ...
            diff = gitutil.git(wt, "diff", "--cached", proposal.base, "--",
                               *changed)
            findings = tuple(lint_wiki(wt, only_paths=list(changed)))
...
def commit(wiki: Path, proposal: Proposal, *, lock: Held | None,
    ...
    with _candidate(wiki, proposal) as wt:
        findings = blocking(lint_wiki(wt, only_paths=list(changed)))
    if findings:
