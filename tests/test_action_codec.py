...
from dbwiki.incidents import (ACTION_KINDS, MAX_PATTERN, ActionMalformed,
                              ActionRecord, Actions, ErrorAbsent, EventPresent,
                              EvidenceRef, FlowResumed, Manual,
...
def test_appending_a_different_record_with_the_same_at_converges():
    ...
    assert (append_action(append_action(PAGE, first), second)
            == append_action(PAGE, second))
...
def test_a_record_between_two_sections_is_replaced_in_place():
    page = (append_action(PAGE, record_with(at="2026-08-30T09:00:00Z"))
            + "\n## Evidence\n\n- digests/2026-08-30-cdb1.md\n")
