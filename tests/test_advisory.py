...
def snapshot() -> Snapshot:
    ...
    return Snapshot(
        ...
        occurrences=rows,
        research={}, resolutions={}, past_fixes={},
...
@pytest.mark.parametrize("breaks", ["pack", "write", "thread"])
def test_a_start_that_breaks_after_admission_gives_its_slot_back(
        tmp_path, monkeypatch, breaks):
    """Issue 19: `pack`, the queued write or the worker's thread raising
    after `_admit` kept the slot and the reservation, so with one slot every
    later click was `busy` until the portal restarted. The broken start
    raises, leaves a failed record rather than a queued one nobody works,
    and the next click is admitted."""
    made = runner(tmp_path, monkeypatch, max_concurrent=1,
                  budget=Budget(24, 2, None))
    spec = made.spec("draft-note")
    first_at, next_at = "2026-08-30T10:00:00Z", "2026-08-30T11:00:00Z"

    def boom(*args, **kwargs):
        raise RuntimeError(f"{breaks} broke")

    with monkeypatch.context() as patch:
        if breaks == "pack":
            patch.setattr(advisory, "pack", boom)
        elif breaks == "write":
            real = made._write
            patch.setattr(made, "_write", lambda run: (
                boom() if run.status is RunStatus.QUEUED else real(run)))
        else:
            patch.setattr(advisory.threading.Thread, "start", boom)
        with pytest.raises(RuntimeError, match="broke"):
            made.start(spec, target(), at=first_at)

    assert made._reserved.get(spec.id, 0) == 0
    broken = made.read(run_id_for("draft-note", SLUG, first_at))
    assert broken is not None and broken.status is RunStatus.FAILED
    assert "broke" in broken.error
    run = made.start(spec, target(), at=next_at)
    assert wait(made, run.run_id).status is RunStatus.SUCCEEDED
    assert len(made.calls) == 1


def test_concurrent_starts_of_one_click_are_one_run_and_one_call(
        tmp_path, monkeypatch):
    """Issue 19: the idempotency read ran outside the lock, so two re-posts
    of one click that landed together both found nothing, both were
    admitted and the model was paid twice for one `run_id`. `pack` is slowed
    so the window the read used to leave open is wide enough to hit every
    time."""
    gate = threading.Event()
    made = runner(tmp_path, monkeypatch, gate=gate,
                  max_concurrent=DEFAULT_MAX_CONCURRENT)
    spec = made.spec("draft-note")
    real_pack = advisory.pack

    def slow_pack(*args):
        time.sleep(0.05)
        return real_pack(*args)

    monkeypatch.setattr(advisory, "pack", slow_pack)
    clicks = DEFAULT_MAX_CONCURRENT + 2
    ready = threading.Barrier(clicks)
    runs, errors = [], []

    def go():
        ready.wait(timeout=10)
        try:
            runs.append(made.start(spec, target(), at=AT))
        except Exception as e:  # noqa: BLE001 — the assertion names it
            errors.append(e)

    threads = [threading.Thread(target=go) for _ in range(clicks)]
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join(timeout=10)
    ...
    assert errors == []
    assert {run.run_id for run in runs} == {run_id_for("draft-note", SLUG, AT)}
    assert wait(made, runs[0].run_id).status is RunStatus.SUCCEEDED
    assert len(made.calls) == 1
    assert len(ledger(made)) == 1
    assert made._reserved.get(spec.id, 0) == 0
