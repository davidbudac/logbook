...
def cfg(wiki, tmp_path):
    ...
    return make_config(
        tmp_path, wiki_repo=wiki, state_dir=tmp_path / "state",
...
@pytest.mark.parametrize("analyst, expected", [
    (None, queue.STALE_CLAIM_HOURS), ({}, queue.STALE_CLAIM_HOURS),
    ({"stale_hours": 6}, 6.0)])
def test_the_claim_reclaims_by_the_configured_stale_budget(
        cfg, monkeypatch, analyst, expected):
    """Issue 22: health flags a claim stale at `analyst.stale_hours`, so the
    reclaim in `claim()` must use the same budget, not only the default."""
    if analyst is not None:
        cfg.analyst = analyst
    seen = {}

    def claim(wiki, claimed_by, *, lock, kind=None, push=True,
              stale_hours=queue.STALE_CLAIM_HOURS):
        seen["stale_hours"] = stale_hours
        return None

    monkeypatch.setattr(queue, "claim", claim)
    monkeypatch.setattr(cli, "load_config", lambda: cfg)
    assert cli.main(["analyst"]) == 0
    assert seen["stale_hours"] == expected
...
def test_an_unexpected_exception_still_rolls_back_and_requeues(
        cfg, wiki, monkeypatch, capsys):
    """Only HarnessError used to reach the requeue: a malformed result left
    the report page in the tree and the request stuck in claimed/."""
    enqueue(wiki, run_id="r1", day="2026-08-05")
    fake_agent(monkeypatch, {
        "reports/2026-08-05.md": "---\ntype: report\n---\n\n# r\n",
        "log.md": "# log\nreport\n"}, "ok")
    monkeypatch.setattr(cli, "load_config", lambda: cfg)
    assert cli.main(["analyst"]) == 1
    assert "FAILED" in capsys.readouterr().err
    assert not (wiki / "reports" / "2026-08-05.md").exists()
    assert (wiki / "log.md").read_text() == "# log\n"
    assert not list((wiki / "queue" / "claimed").glob("*.json"))
    assert queue.pending_requests(wiki)[0]["attempts"] == 1
