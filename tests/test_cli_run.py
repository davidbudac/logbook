...
class FakeOrchestrator:
    ...
    def ingest(self, db, jp, run_id=None) -> dict:
...
def tick(tmp_path, monkeypatch):
    ...
    namespace: tests set `dbs`, `compact_fails`, `ingest_fails`,
    `report_fails`, and read `calls` plus the run-health event back off it."""
    cfg = SimpleNamespace(
        root=tmp_path, wiki_repo=tmp_path / "wiki", state_dir=tmp_path / "state",
        report={}, alerts={}, agents={},
        dbs=["db1", "db2"], routine=(), calls=[], compact_fails={},
