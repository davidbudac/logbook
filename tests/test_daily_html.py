...
from dbwiki.daily_html import (MAX_DEEP_LINKS, available_days, change_rows,
                               day_digests, day_stats, delta_lines,
                               digest_headline,
...
def _changed(root, db, *changes, day=DAY):
    """A routine digest for `db` whose recorded `changes` are `changes`, each
    `(ts, rule, count, message)`."""
    d = _place(root, db, "listener_only_routine_day", day=day)
    d["changes"] = [{"day": day, "ts": ts, "rule": rule, "count": count,
                     "message": message}
                    for ts, rule, count, message in changes]
    (root / "digests" / db / f"{day}.json").write_text(json.dumps(d))


def _strip(page: str) -> str:
    return page.split('<section class="tier t-changes">')[1].split("</section>")[0]


def test_change_rows_are_one_fleet_timeline_ordered_by_time(wiki):
    root = wiki.parent / "changed-wiki"
    _changed(root, "db2", (f"{DAY}T02:31:09Z", "datafile_change", 1,
                           "ALTER DATABASE DATAFILE 7 RESIZE 8G"))
    _changed(root, "db1",
             (f"{DAY}T01:00:00Z", "parameter_change", 2,
              "ALTER SYSTEM SET db_cache_size=6G SCOPE=BOTH;"),
             (f"{DAY}T04:05:33Z", "instance_startup", 1,
              "Starting ORACLE instance (normal)"))
    assert [(db, c.rule) for db, c in change_rows(day_digests(root, DAY))] == [
        ("db1", "parameter_change"), ("db2", "datafile_change"),
        ("db1", "instance_startup")]


def test_the_changes_strip_comes_before_the_errors(wiki):
    """An operator reads what was changed before what went wrong."""
    root = wiki.parent / "changed-wiki"
    _changed(root, "db1", (f"{DAY}T01:00:00Z", "parameter_change", 2,
                           "ALTER SYSTEM SET db_cache_size=6G SCOPE=BOTH;"))
    p = render_daily(root, DAY, link_base=LINK_BASE)
    assert p.index('class="tier t-changes"') < p.index(">Needs attention <")
    strip = _strip(p)
    assert ">Changes <" in strip
    assert "01:00:00" in strip and "parameter_change ×2" in strip
    assert "ALTER SYSTEM SET db_cache_size=6G SCOPE=BOTH;" in strip
    assert f'href="{LINK_BASE}/digests/db1/{DAY}.md">db1</a>' in strip


def test_the_strip_derives_changes_from_a_digest_without_the_key(page):
    """The placed real digests predate the `changes` key; their lifecycle
    groups still show, because the strip reads through `of_digest`."""
    strip = _strip(page)
    assert "instance_startup" in strip and ">busy<" in strip
    assert ">newdb<" in strip
    assert "ALTER DATABASE OPEN" in strip


def test_a_day_without_changes_has_no_strip(wiki):
    root = wiki.parent / "quiet-wiki"
    _place(root, "listenonly", "listener_only_routine_day")
    assert 'class="tier t-changes"' not in render_daily(root, DAY,
                                                       link_base=LINK_BASE)


def test_the_strip_is_capped_and_says_where_the_rest_are(wiki):
    root = wiki.parent / "changed-wiki"
    _changed(root, "db1", *[(f"{DAY}T01:{i:02d}:00Z", "parameter_change", 1,
                             f"ALTER SYSTEM SET p{i}=1;")
                            for i in range(dh.MAX_CHANGE_ROWS + 3)])
    strip = _strip(render_daily(root, DAY, link_base=LINK_BASE))
    assert strip.count("<li>") == dh.MAX_CHANGE_ROWS
    assert f">{dh.MAX_CHANGE_ROWS + 3}</span>" in strip
    assert "+3 more" in strip
    assert f"p{dh.MAX_CHANGE_ROWS - 1}=1" in strip
    assert f"p{dh.MAX_CHANGE_ROWS}=1" not in strip


def test_change_text_is_escaped(wiki):
    root = wiki.parent / "changed-wiki"
    _changed(root, "db1", (f"{DAY}T01:00:00Z", "parameter_change", 1,
                           "ALTER SYSTEM SET x='<script>y</script>';"))
    p = render_daily(root, DAY, link_base=LINK_BASE)
    assert "<script>" not in p
    assert "&lt;script&gt;y&lt;/script&gt;" in p
...
LATER_DAY = "2026-07-29"
RESOLVED_ON_LATER_DAY = "2026-07-29T08:00:00Z"
OPEN_PATH = "incidents/2026-07-12-cdb1-dataguard.md"


def _resolve(wiki, at: str, *, touched: str | None = None) -> None:
    """Resolve the fixture's open incident the way the workbench does: one
    `resolve` action record at `at` and the frontmatter moved with it; then,
    when `touched` is given, a later note on the resolved page, which moves
    `updated` again without changing when it was resolved."""
    path = wiki / OPEN_PATH
    text = set_status(append_action(path.read_text(), ActionRecord(
        at=at, kind="resolve", actor="dba@example.com", intent="close it",
        summary="transport restored", status_after=Status.RESOLVED)),
        Status.RESOLVED, updated=at)
    if touched:
        text = set_status(append_action(text, ActionRecord(
            at=touched, kind="record-action", actor="dba@example.com",
            intent="note the follow-up", summary="checked again",
            status_after=Status.RESOLVED)), Status.RESOLVED, updated=touched)
    path.write_text(text)


def test_an_incident_resolved_later_stays_on_the_days_it_was_open(wiki):
    """Issue 20: `open_incidents` read today's status, so the day an
    incident was resolved it vanished from every earlier page that was
    rendered again (a neighbour refresh, a backfill) and from every earlier
    day's count on the index."""
    _resolve(wiki, RESOLVED_ON_LATER_DAY)
    assert "cdb1 dataguard transport failure" in render_daily(
        wiki, DAY, link_base=LINK_BASE)
    assert day_stats(wiki, DAY)["incidents"] == 1
    assert "cdb1 dataguard transport failure" not in render_daily(
        wiki, LATER_DAY, link_base=LINK_BASE)
    assert day_stats(wiki, LATER_DAY)["incidents"] == 0


def test_a_note_on_a_resolved_incident_does_not_reopen_its_past(wiki):
    """`updated` moves with every act on the page; the day it was resolved
    is the `resolve` record's, not the last touch's."""
    _resolve(wiki, "2026-07-20T08:00:00Z", touched="2026-07-30T09:00:00Z")
    assert "cdb1 dataguard transport failure" not in render_daily(
        wiki, DAY, link_base=LINK_BASE)
    assert "cdb1 dataguard transport failure" in render_daily(
        wiki, "2026-07-19", link_base=LINK_BASE)


def test_a_resolved_page_with_only_updated_is_dated_by_it(wiki):
    """A page resolved by hand or by an agent carries no `resolve` record;
    `updated` is then the best date there is. With neither, the incident
    stays off every page, as before."""
    (wiki / OPEN_PATH).write_text(incident_page(
        "cdb1", "cdb1 dataguard transport failure", status="resolved",
        opened="2026-07-12T00:00:00Z", updated=RESOLVED_ON_LATER_DAY))
    assert day_stats(wiki, DAY)["incidents"] == 1
    assert day_stats(wiki, LATER_DAY)["incidents"] == 0


def test_rendering_the_next_day_leaves_yesterdays_page_as_it_was(wiki):
    """The p6 repro: render a day while the incident is open, resolve it the
    next morning, render the next day. Yesterday's page gets its "later"
    link and nothing else changes; the index still counts it."""
    write_daily(wiki, DAY, link_base=LINK_BASE)
    before = (wiki / "html" / f"{DAY}.html").read_text()
    _resolve(wiki, RESOLVED_ON_LATER_DAY)
    _place(wiki, "cdb1", "listener_only_routine_day", day=LATER_DAY)
    write_daily(wiki, LATER_DAY, link_base=LINK_BASE)

    after = (wiki / "html" / f"{DAY}.html").read_text()
    assert after == before.replace(
        '<span class="disabled">later &#8250;</span>',
        f'<a href="{LATER_DAY}.html">{LATER_DAY} &#8250;</a>')
    index = (wiki / "html" / "index.html").read_text()
    day_row = next(line for line in index.splitlines()
                   if f'>{DAY}</a>' in line)
    assert "<b>1</b> open incident" in day_row


def test_a_neighbour_is_rerendered_whole_when_its_nav_cannot_be_found(wiki):
    """A page from before the nav had one spelling is still repaired: the
    fallback is the full render the refresh always did."""
    (wiki / "html").mkdir()
    (wiki / "html" / "2026-07-27.html").write_text("<!-- hand-made -->")
    _place(wiki, "cdb1", "listener_only_routine_day", day="2026-07-27")
    write_daily(wiki, DAY, link_base=LINK_BASE)
    assert f'href="{DAY}.html"' in (
        wiki / "html" / "2026-07-27.html").read_text()
...
def test_a_storm_card_links_its_biggest_groups_and_not_all_of_them(wiki):
    ...
    card = render_daily(wiki, DAY, link_base=LINK_BASE,
                        links=RESOLVER).split(f"<h3>{biggest}</h3>")[1]
...
def orch(git_wiki, tmp_path):
    # the minimal config, never fixture_config(): the orchestrator must not
    # pick up the real adapter/mode or the real state dir
    cfg = fx.make_config(tmp_path, wiki_repo=git_wiki,
                         state_dir=tmp_path / "state",
                         report={"link_base": LINK_BASE})
...
def test_run_renders_html_even_when_nothing_was_ingested(tmp_path, monkeypatch):
    ...
    calls = []
    cfg = fx.make_config(tmp_path, state_dir=tmp_path / "state",
...
def test_cli_render_daily_writes_the_page(tmp_path, wiki, monkeypatch):
    ...
    cfg = fx.make_config(tmp_path, wiki_repo=wiki, state_dir=tmp_path / "state",
                         report={"link_base": LINK_BASE})
    monkeypatch.setattr(cli, "load_config", lambda: cfg)
