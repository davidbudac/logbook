...
#: the real factory, taken before the autouse fixture swaps it out
REAL_OPEN_SMTP = delivery._open_smtp
...
def cfg_with(block: dict):
    return make_config(delivery=block)
...
def test_the_policy_defaults_when_the_block_is_absent():
    assert Policy.resolve(make_config()) == Policy()
...
def test_a_recipient_may_not_be_called_inbox():
    """Its key would be the inbox row's (`review|inbox`): the inbox always
    delivers first, so that recipient's email was never sent or retried."""
    cfg = make_config(delivery={
        "external_enabled": True, "smtp_host": "relay.example.com",
        "from_address": "dbwiki@example.com",
        "recipients": [{"id": "inbox", "address": "x@example.com",
                        "channels": ["email"]}]})
    with pytest.raises(ValueError, match=r"recipients\[inbox\]\.id.*reserved"):
        Policy.resolve(cfg)
...
def test_starttls_verifies_the_relay_certificate(monkeypatch):
    ...
    class FakeSMTP:
        def __init__(self, host, port, timeout):
            got["host"] = (host, port, timeout)

        def starttls(self, **kw):
            got["starttls"] = kw

    monkeypatch.setattr(delivery.smtplib, "SMTP", FakeSMTP)
    policy = Policy(starttls=True, smtp_host="relay.example.com")
    REAL_OPEN_SMTP(policy)
    ctx = got["starttls"]["context"]
