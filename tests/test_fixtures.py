...
def test_manifest_matches_the_case_files():
    ...
    assert MANIFEST["fixture_version"] == 10
...
def test_dataguard_groups_get_an_after_change_delta_across_sources():
    """A gap after a transport parameter change, a role change after the
    DGMGRL switchover: every dataguard-class group follows a change, and the
    latest change wins even when it sits in the other source."""
    got = [(d["source"], d["rule"], d["gap_s"], d["change_rule"])
           for d in fx.golden_digest("dataguard_after_change")["deltas"]
           if d["type"] == "after_change"]
    assert got == [("alert", "dg_gap", 720, "parameter_change"),
                   ("dataguard", "role_change", 150, "cli_command")]
