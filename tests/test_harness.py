...
def test_ollama_reuses_the_codex_usage_line():
    assert parse_usage("ollama", CODEX_OUT)["input_tokens"] == 12345
...
@pytest.mark.parametrize("adapter", ["claude", "codex", "ollama", "pi", "nope"])
...
def writing(wiki, body: str, stdout: str = "", returncode: int = 0):
    """A fake `subprocess.run` that writes the result JSON the way an adapter
...
def test_telemetry_is_filled_when_no_result_json_is_written(monkeypatch, tmp_path):
    monkeypatch.setattr(subprocess, "run", lambda *a, **kw: proc(0, "done"))
    tele = {}
...
def test_run_agent_works_without_a_telemetry_dict(monkeypatch, tmp_path):
    monkeypatch.setattr(subprocess, "run", writing(tmp_path, "{}"))
...
def test_run_text_returns_stdout_and_fills_telemetry(monkeypatch, tmp_path):
    monkeypatch.setattr(subprocess, "run", lambda *a, **kw: proc(0, '{"a": 1}'))
    tele = {}
...
def test_run_text_raises_on_a_nonzero_exit(monkeypatch, tmp_path):
    monkeypatch.setattr(subprocess, "run", lambda *a, **kw: proc(2, "", "boom"))
...
def test_run_web_text_raises_on_a_nonzero_exit(monkeypatch):
    monkeypatch.setattr(subprocess, "run", lambda *a, **kw: proc(3, "", "boom"))
...
def test_run_web_text_raises_without_result_text(monkeypatch, stdout):
    monkeypatch.setattr(subprocess, "run", lambda *a, **kw: proc(0, stdout))
...
def test_pi_final_text_extracts_the_answer_and_run_text_returns_it(
    ...
    monkeypatch.setattr(subprocess, "run", lambda *a, **kw: proc(0, PI_JSON))
    tele = {}
...
def test_ollama_reuses_the_codex_step_parser():
    assert len(harness.parse_steps("ollama", CODEX_STEPS_JSON)) == 6
...
def test_previews_are_capped_at_200_characters(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(subprocess, "run", writing(tmp_path, "{}", out))
    tele = {}
    run_agent("pi", "p", tmp_path, "m", 5, telemetry=tele)
...
def test_telemetry_has_steps_even_when_the_adapter_streams_nothing(
        monkeypatch, tmp_path):
    monkeypatch.setattr(subprocess, "run", writing(tmp_path, "{}", CODEX_OUT))
    tele = {}
    run_agent("codex", "p", tmp_path, None, 5, telemetry=tele)
