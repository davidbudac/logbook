...
from dbwiki import cli, gitutil, health, observability, orchestrate, transaction
...
def _late_append_during_trim(monkeypatch, path, spawn):
    """Run a trim of `path` and, in the window between its read and its
    rename, let `spawn()` start another writer; returns once both are done."""
    from dbwiki import state as state_mod
    real = state_mod.atomic_write_text

    started = []

    def racing(target, text, **kw):
        if not started:      # race the first trim only
            started.append(spawn())
            started[0](2.0)  # a writer without the lock lands in here
        real(target, text, **kw)

    monkeypatch.setattr(state_mod, "atomic_write_text", racing)
    for i in range(12):
        health._append_capped(path, {"i": i}, 10)
    assert started, "the test never reached a trim"
    started[0](10.0)         # let the late writer finish
    return [json.loads(ln) for ln in path.read_text().splitlines()]


def test_an_append_racing_a_trim_is_not_lost_across_threads(tmp_path,
                                                          monkeypatch):
    """A trim reads the log, then publishes the shortened copy: a line
    another writer appended in between used to vanish with the rename."""
    import threading
    path = tmp_path / "log.jsonl"

    def spawn():
        t = threading.Thread(target=health._append_capped,
                             args=(path, {"late": True}, 10))
        t.start()
        return t.join

    rows = _late_append_during_trim(monkeypatch, path, spawn)
    assert {"late": True} in rows


def test_an_append_racing_a_trim_is_not_lost_across_processes(tmp_path,
                                                            monkeypatch):
    """Same race with the cron tick and the portal as two processes."""
    import sys
    path = tmp_path / "log.jsonl"

    def spawn():
        proc = subprocess.Popen([sys.executable, "-c",
                                 "import sys; from dbwiki import health; "
                                 "health._append_capped(sys.argv[1], "
                                 "{'late': True}, 10)", str(path)])
...
def test_a_future_dated_event_does_not_make_a_dead_source_healthy(cfg):
    """One mis-parsed line dated 2099 used to be the source's newest event
    forever: a negative age passed `<= stale_hours`. The probe now ignores
    events beyond now + slack and reports them as a data-quality problem."""
    h = assess(cfg, now=NOW,
               es=fake_es(cfg, alert=[DEAD, "2099-01-01T00:00:00Z"],
                          listener=[DEAD], dataguard=[DEAD]))
    alert = next(p for p in h["sources"] if p["source"] == "alert")
    assert alert["state"] == "collection_failure"
    assert alert["latest_event"] == DEAD and alert["age_hours"] > 340
    assert alert["future_event"] == "2099-01-01T00:00:00Z"
    assert any("future-dated" in p and "alert" in p for p in h["problems"])


def test_a_little_clock_skew_is_neither_future_nor_stale(cfg):
    ahead = "2026-07-27T02:00:00Z"            # 2h ahead of NOW
    h = assess(cfg, now=NOW, es=fake_es(cfg, alert=[ahead], listener=[FRESH],
                                        dataguard=[FRESH]))
    alert = next(p for p in h["sources"] if p["source"] == "alert")
    assert alert["state"] == "ok" and alert["latest_event"] == ahead
    assert "future_event" not in alert
    assert not any("future-dated" in p for p in h["problems"])
...
def test_the_last_ticks_failed_pull_is_a_problem_until_one_succeeds(cfg):
    """A pull that keeps failing leaves analyst results on the remote and
    every on-prem push rejected; only the tick's own note says so."""
    es = fake_es(cfg, alert=[FRESH], listener=[FRESH], dataguard=[FRESH])
    with run_recorder(cfg.state_dir, "run") as rec:
        rec.note(analyst_pull_error="git_error: git fetch: no route")
    h = assess(cfg, now=NOW, es=es)
    assert any("could not pull the wiki remote" in p and "no route" in p
               for p in h["problems"])
    with run_recorder(cfg.state_dir, "run") as rec:
        rec.note(analyst_pulled=1)
    h = assess(cfg, now=NOW, es=es)
    assert not any("could not pull" in p for p in h["problems"])
...
def seed_failed_run(cfg, command, finished, **facts):
    health.record_run(cfg.state_dir, {
        "schema_version": 1, "run_id": new_run_id(), "command": command,
        "started": finished, "finished": finished, "outcome": "failed",
        "error_category": "harness_error", "dbs": [], "facts": facts})


def test_nightly_history_research_does_not_mask_a_failing_weekly_research(cfg):
    """Every `dbwiki research` variant records `command="research"`; the
    nightly model-free `--history` used to refresh the weekly research's last
    success, so five failed Mondays read as healthy."""
    seed_success(cfg, "research", OLD, pages=3)
    for day in ("2026-07-06", "2026-07-13", "2026-07-20"):
        seed_failed_run(cfg, "research", f"{day}T09:30:00Z", pages=3)
    seed_success(cfg, "research", "2026-07-26T23:46:00Z", mode="history",
                 dry_run=False)
    h = assess(cfg, now=NOW, es=fresh_es(cfg))
    assert h["last_success"]["research"]["at"] == OLD
    assert h["last_success"]["research_history"]["at"] == \
        "2026-07-26T23:46:00Z"
    assert [s["task"] for s in h["stale_stages"]] == ["research"]


def test_each_research_mode_has_its_own_budget(cfg):
    """`--history` is nightly (26h); `--caveats` is weekly like the agentic
    research (192h); neither's success stands in for another's."""
    seed_success(cfg, "research", "2026-07-24T09:00:00Z", pages=3)
    seed_success(cfg, "research", "2026-07-25T00:00:00Z", mode="history",
                 dry_run=False)
    seed_success(cfg, "research", OLD, pages=1, mode="caveats")
    h = assess(cfg, now=NOW, es=fresh_es(cfg))
    stale = {s["task"]: s["threshold_h"] for s in h["stale_stages"]}
    assert stale == {"research_history": 26, "research_caveats": 192}
    text = format_health(h)
    assert "uv run dbwiki research --history" in text
    assert "uv run dbwiki research --caveats" in text


def test_a_dry_run_history_does_not_count_as_a_success(cfg):
    seed_success(cfg, "research", "2026-07-26T23:46:00Z", mode="history",
                 dry_run=True)
    assert assess(cfg, now=NOW, es=fresh_es(cfg))["last_success"][
        "research_history"] is None
...
def publish_review(cfg, review_id="2026-W30"):
    from dbwiki import review
    d = cfg.state_dir / review.REVIEW_DIR / review.REVIEWS_DIR
    d.mkdir(parents=True, exist_ok=True)
    (d / f"{review_id}.json").write_text(json.dumps(
        {"schema_version": 1, "review_id": review_id, "findings": [],
         "explanation": "x", "deliveries": []}))


def delivery_row(cfg, event, review_id="2026-W30", recipient="ops",
                 at="2026-07-20T10:00:00Z"):
    from dbwiki import review
    review._audit(cfg.state_dir, event, review_id=review_id, at=at,
                  key=f"{review_id}|{recipient}", channel="email",
                  recipient=recipient, content_class="internal_full",
                  **({"error": "smtp_failed", "detail": "refused"}
                     if event == "delivery_failed" else {}))


EMAIL_DELIVERY = {"external_enabled": True, "smtp_host": "relay.example.com",
                  "from_address": "dbwiki@example.com",
                  "recipients": [{"id": "ops", "address": "ops@example.com",
                                  "channels": ["email"]}]}


def test_a_failed_review_delivery_is_a_problem_until_retried(cfg):
    """`delivery_failed` rows sat in review/log.jsonl where nothing looked:
    health now surfaces them, so the alert sinks fire."""
    cfg.delivery = EMAIL_DELIVERY
    publish_review(cfg)
    delivery_row(cfg, "delivery_failed")
    h = assess(cfg, now=NOW, es=fresh_es(cfg))
    assert [(f["review_id"], f["recipient"], f["error"])
            for f in h["review_delivery"]["failed"]] == \
        [("2026-W30", "ops", "smtp_failed")]
    assert any("review delivery failed" in p for p in h["problems"])
    assert h["exit_code"] == 1
    from dbwiki import alerts
    found = [f for f in alerts.findings(h) if f.category == "delivery_failed"]
    assert len(found) == 1
    assert "dbwiki review --deliver-only --review-id 2026-W30" in \
        alerts.recovery_hint("delivery_failed", detail=found[0].detail)
    assert "--review-id 2026-W30" in format_health(h)

    delivery_row(cfg, "delivered", at="2026-07-21T10:00:00Z")
    h = assess(cfg, now=NOW, es=fresh_es(cfg))
    assert h["review_delivery"]["failed"] == [] and h["exit_code"] == 0


def test_a_failure_nobody_can_retry_any_more_is_not_a_problem(cfg):
    """A pruned review, or a recipient no longer configured, cannot be
    retried: nagging about it forever would teach people to ignore health."""
    cfg.delivery = EMAIL_DELIVERY
    delivery_row(cfg, "delivery_failed", review_id="2026-W01")  # pruned
    publish_review(cfg)
    delivery_row(cfg, "delivery_failed", recipient="gone")      # unconfigured
    h = assess(cfg, now=NOW, es=fresh_es(cfg))
    assert h["review_delivery"]["failed"] == []
...
def test_a_blocker_hint_follows_its_category_not_its_wording(cfg):
    """A missing wiki whose path happens to say "dirty" is still a missing
    wiki: the hint comes from what the blocker is, not from its prose."""
    cfg.wiki_repo = cfg.wiki_repo.parent / "dirty-wiki"
    text = format_health(assess(cfg, now=NOW, es=fresh_es(cfg)))
    assert "-> restore the wiki/ checkout" in text
    assert "-> commit or stash the non-digest changes" not in text
