...
from dbwiki.lock import LOCK_FILE, LockBusyError, single_flight
...
def test_an_unreadable_lock_file_still_produces_a_busy_message(tmp_path):
    with single_flight(tmp_path, "run"):
        ...
        with pytest.raises(LockBusyError, match="holds the lock"):
            with single_flight(tmp_path, "research"):
                pass
...
def cfg(tmp_path, monkeypatch):
    c = SimpleNamespace(root=tmp_path, wiki_repo=tmp_path / "wiki",
                        digest_dir=tmp_path / "wiki" / "digests",
                        state_dir=tmp_path / "state", report={}, agents={})
