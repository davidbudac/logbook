...
# Termination. A block rule that matches a line but consumes none of it spins
# `_blocks` forever (issue 01: `|a` pinned a portal thread and grew memory
# without bound). Every render below runs under a wall-clock alarm so a
...
def _render_within(text, seconds=2, **kw):
    import signal
...
def test_a_pipe_line_that_starts_no_table_renders_as_text(text):
    out = _render_within(text).html
    assert "<table>" not in out
    assert "|" in out or "&#124;" in out, "the pipe text is kept, not dropped"


def test_a_pipe_line_before_a_real_table_still_renders_the_table():
    out = _render_within("| stray\n\n| a | b |\n|---|---|\n| 1 | 2 |\n").html
...
def test_render_terminates_on_random_block_soup():
    import random
    import time
    ...
    while time.monotonic() < deadline and count < 3000:
        text = "".join(rng.choice(_FUZZ_ATOMS)
                       for _ in range(rng.randint(1, 16)))
        _render_within(text, links=snapshot_links())
        count += 1
    assert count > 100
