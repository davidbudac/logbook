...
def cfg(**lf):
    return make_config(langfuse=lf)
...
def test_warn_once_is_one_line_per_key_per_process(capsys):
    """The one warn-once helper every telemetry path shares: health's
    recorders and the Langfuse export no longer keep their own flags."""
    observability.warn_once("run-health", "run-health recording failed: a")
    observability.warn_once("run-health", "run-health recording failed: b")
    observability.warn_once("other", "something else")
    assert capsys.readouterr().err.splitlines() == [
        "warning: run-health recording failed: a",
