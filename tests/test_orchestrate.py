...
from dbwiki.orchestrate import Orchestrator, ValidationError, telemetry_fields
...
def orch(wiki, tmp_path):
    cfg = make_config(tmp_path, wiki_repo=wiki, state_dir=tmp_path / "state")
...
def test_validate_requires_exact_log_md_path(wiki, orch):
    ...
    (wiki / "concepts").mkdir()
    (wiki / "concepts" / "changelog.md").write_text("# changelog\nedited\n")
    problems = orch._validate(result(pages_touched=["concepts/changelog.md"]),
                              "ingest", changed(wiki))
    assert any("log.md not updated" in p for p in problems)

    (wiki / "log.md").write_text("# log\nentry\n")
    problems = orch._validate(
        result(pages_touched=["log.md", "concepts/changelog.md"]), "ingest",
        changed(wiki))
    assert problems == []
...
def test_structured_research_ignored_when_review_sources_present(research_wiki,
    ...
    assert orch.last_telemetry["mode"] == "agentic"
...
# ---- the trigger decision rides on the ledger entry ----------------------------

def _decision(digest_json):
    from dbwiki.trigger import decide
    digest = json.loads(digest_json.read_text())
    return decide(digest, ledger_entry={}, window_to=digest["window"]["to"],
                  manual=True)


def test_ingest_records_the_decision_beside_its_own_entry(wiki, orch,
                                                          monkeypatch):
    """Issue 11 (python-workarounds): `ingest` takes the trigger decision
    and merges it after its own ledger write, so no caller writes the entry
    a second time."""
    digest_json = ingest_setup(wiki, orch)
    decision = _decision(digest_json)
    fake_agent(monkeypatch, {"log.md": "# log\n[ingest] cdb1\n"},
               {"task": "ingest", "summary": "ingested cdb1",
                "pages_touched": ["log.md"]})
    orch.ingest("cdb1", digest_json, decision=decision)
    entry = orch.state.get_ledger()[f"digests/cdb1/{DAY}.json"]
    assert entry["status"] == "ingested"
    assert entry["last_decision"] == decision.to_dict()
    # a content-unchanged rerun records its decision too
    again = _decision(digest_json)
    assert orch.ingest("cdb1", digest_json, decision=again)["skipped"]
    assert orch.state.get_ledger()[f"digests/cdb1/{DAY}.json"][
        "last_decision"] == again.to_dict()


def test_a_failed_ingest_still_records_its_decision(wiki, orch, monkeypatch):
    digest_json = ingest_setup(wiki, orch)
    decision = _decision(digest_json)
    fake_agent(monkeypatch, {"concepts/notes.md": "no log.md\n"},
...
def test_a_dry_run_ingest_records_nothing(wiki, orch, capsys):
    digest_json = ingest_setup(wiki, orch)
    orch.ingest("cdb1", digest_json, dry_run=True,
                decision=_decision(digest_json))
    assert f"digests/cdb1/{DAY}.json" not in orch.state.get_ledger()


def test_record_decision_keeps_the_rest_of_the_entry(wiki, orch):
    digest_json = ingest_setup(wiki, orch)
    key = f"digests/cdb1/{DAY}.json"
    orch.state.set_ledger_entry(key, {"status": "ingested", "commit": "abc"})
    decision = _decision(digest_json)
    orch.record_decision(digest_json, decision)
    assert orch.state.get_ledger()[key] == {
        "status": "ingested", "commit": "abc",
...
def test_ingest_retries_once_on_validation_failure_then_succeeds(wiki, orch,
    ...
    calls = sequenced_agent(monkeypatch, [
        ({"concepts/notes.md": "attempt 1, no log.md\n"},
         {"task": "ingest", "summary": "bad", "pages_touched": []}),
        ({"log.md": "# log\n[ingest] cdb1\n"},
         {"task": "ingest", "summary": "fixed",
          "pages_touched": ["log.md", "concepts/notes.md"]}),
    ])
    res = orch.ingest("cdb1", digest_json)
    ...
    # a retry never rolls back, so attempt 1's page is still in the tree
    # and attempt 2 has to declare it (undeclared change)
    assert "undeclared change: concepts/notes.md" in calls[1]
...
def test_ingest_fail_then_fail_rolls_back_with_two_attempts(wiki, orch,
    ...
    digest_json = ingest_setup(wiki, orch)
    calls = sequenced_agent(monkeypatch, [
        ({"concepts/notes.md": "attempt 1, still no log.md\n"},
    ...
    assert orch.last_telemetry["validation_ok"] is False
    assert orch.last_telemetry["rolled_back"] is True
    assert not (wiki / "concepts" / "notes.md").exists()
...
def test_lint_synthesizes_a_result_when_only_the_json_is_missing(
        wiki, orch, monkeypatch):
    no_result_agent(monkeypatch, {"concepts/changelog.md": "# changelog\ntidied\n"})
    res = orch.lint()
    assert res["summary"].startswith("(result synthesized")
...
def test_a_base_that_moved_at_publish_time_leaves_the_tree_clean(wiki, orch,
    ...
    digest_json = ingest_setup(wiki, orch)
    fake_agent(monkeypatch, {"log.md": "# log\n[ingest] cdb1\n",
                             "concepts/notes.md": "half an ingest\n"},
...
def spy_escalated_prompt(monkeypatch) -> list:
    """Record the `history_days` each escalated structured prompt is built
    with (ingest-db-history #03)."""
    from dbwiki import structured
    seen = []
    real = structured.build_escalated_report_prompt

    def spy(*a, **kw):
        seen.append(kw.get("history_days"))
        return real(*a, **kw)

    monkeypatch.setattr(structured, "build_escalated_report_prompt", spy)
    return seen


@pytest.mark.parametrize("days", [0, 30])
def test_the_escalated_structured_report_honours_history_days(
        wiki, orch, monkeypatch, days):
    orch.cfg.agents.update(mode="structured", escalated_report="structured",
                           history_days=days)
    monkeypatch.setattr("dbwiki.orchestrate.run_agent", lambda *a, **k:
                        pytest.fail("the structured path ran an agent"))
    seen = spy_escalated_prompt(monkeypatch)
    fake_generate(monkeypatch, escalated_proposal())
    orch.report(DAY, WINDOW, NOTABLE_INGESTED)
    assert seen == [days]


def test_the_structured_fallback_honours_history_days(wiki, orch, monkeypatch):
...
# ---- a timed-out agent's stragglers (issue 03) ---------------------------------

def test_a_timeout_rollback_waits_for_the_tree_to_stop_changing(
        wiki, orch, monkeypatch):
    """The harness kills the adapter's process group, but a tool that left
    the group keeps writing for a while; restoring under it leaves debris
    every later tick refuses on. The rollback waits it out first."""
    import threading
    import time
    from dbwiki import orchestrate
    monkeypatch.setattr(orchestrate, "SETTLE_S", 0.05)
    digest_json = ingest_setup(wiki, orch)
    threads = []

    def straggler(repo):
        for i in range(12):
            (repo / "late.md").write_text(f"still writing {i}\n")
            time.sleep(0.03)

    def _run(adapter, prompt, repo, model, timeout, web=False, provider=None,
             telemetry=None):
        telemetry.update(timed_out=True)
        (repo / "log.md").write_text("# log\nhalf\n")
        threads.append(threading.Thread(target=straggler, args=(repo,)))
        threads[0].start()
        raise HarnessError("agent timed out after 1s: codex")
...
# ---- one stage wrapper: any failure rolls back and is recorded (issue 02) ------
# Only HarnessError used to be caught. Anything else between the agent's
# writes and the publish left them in the tree with no ledger entry, and every
# later tick refused on "uncommitted changes".

@pytest.mark.parametrize("res", [
    [], "ok", None,
    {"task": "ingest", "summary": "s", "pages_touched": None},
    {"task": "ingest", "summary": 5, "pages_touched": ["log.md"]}])
def test_a_malformed_result_rolls_back_and_lands_in_the_ledger(wiki, orch,
                                                               monkeypatch, res):
    digest_json = ingest_setup(wiki, orch)
    fake_agent(monkeypatch, {"log.md": "# log\nagent\n"}, res)
    ...
    entry = orch.state.get_ledger()[f"digests/cdb1/{DAY}.json"]
    assert entry["status"] == "failed" and entry["rolled_back"] is True
    assert entry["content_hash"] and entry["problems"]


def test_a_structured_writer_that_raises_leaves_no_partial_write(
        wiki, orch, monkeypatch):
    from dbwiki import structured
    digest_json = ingest_setup(wiki, orch)
    orch.cfg.agents["mode"] = "structured"
    monkeypatch.setattr(structured, "build_prompt", lambda *a, **k: "p")
    monkeypatch.setattr(structured, "propose", lambda *a, **k: {})

    def half_then_die(wiki_root, *a, **k):
        (wiki_root / "databases").mkdir()
        (wiki_root / "databases" / "cdb1.md").write_text("half\n")
        raise IsADirectoryError("incidents/")

    monkeypatch.setattr(structured, "apply_proposal", half_then_die)
    with pytest.raises(IsADirectoryError):
        orch.ingest("cdb1", digest_json)
    assert transaction.stray_paths(wiki) == ()
    entry = orch.state.get_ledger()[f"digests/cdb1/{DAY}.json"]
    assert entry["status"] == "failed" and entry["mode"] == "structured"


def test_a_structured_update_naming_the_incident_directory_is_skipped(
        wiki, orch, monkeypatch):
    from dbwiki import structured
    digest_json = ingest_setup(wiki, orch)
    digest_json.with_suffix(".md").write_text("# digest\n")
    (wiki / "incidents").mkdir()
    (wiki / "incidents" / "a.md").write_text(
        "---\ntype: incident\nstatus: open\ndb: cdb1\n---\n# a\n")
    git(wiki, "add", "-A")
    git(wiki, "commit", "-m", "inc")
    orch.cfg.agents["mode"] = "structured"
    prop = structured.parse_proposal(json.dumps({
        "schema_version": structured.STRUCTURED_SCHEMA_VERSION,
        "summary": "s", "notable": False, "journal_entry": "j",
        "incident": {"action": "update", "existing_page": "incidents/",
                     "body": "b"}}))
    monkeypatch.setattr(structured, "propose", lambda *a, **k: prop)
    monkeypatch.setattr(structured, "build_prompt", lambda *a, **k: "p")
    res = orch.ingest("cdb1", digest_json)
...
def test_lint_rolls_back_on_an_unexpected_exception(wiki, orch, monkeypatch):
    fake_agent(monkeypatch, {"concepts/x.md": "# x\n"}, ["not", "a", "dict"])
...
def test_report_rolls_back_on_an_unexpected_exception(wiki, orch, monkeypatch):
    fake_agent(monkeypatch, report_edits(), "ok")
    with pytest.raises(AttributeError):
        orch.report(DAY, WINDOW, [{"db": "cdb1", "notable": False,
                                   "summary": "s"}])
    assert transaction.stray_paths(wiki) == ()
    assert orch.last_telemetry["rolled_back"] is True


def test_a_rollback_keeps_a_file_a_human_staged_on_any_failure(
        wiki, orch, monkeypatch):
    digest_json = ingest_setup(wiki, orch)
    ...
    with pytest.raises(AttributeError):
        orch.ingest("cdb1", digest_json)
    assert (wiki / "human.md").read_text() == "mine\n"
    assert (wiki / "log.md").read_text() == "# log\n"


# ---- per-stage path allowlists (issue 02) --------------------------------------

@pytest.mark.parametrize("rel", ["AGENTS.md", "CLAUDE.md", "notes.md",
                                 "reports/x.md", "queue/pending/x.json",
                                 "sources/x.md"])
def test_ingest_may_not_write_outside_its_directories(wiki, orch, monkeypatch,
                                                      rel):
    digest_json = ingest_setup(wiki, orch)
    fake_agent(monkeypatch, {"log.md": "# log\nagent\n", rel: "x\n"},
               {"task": "ingest", "summary": "s",
                "pages_touched": ["log.md", rel]})
    with pytest.raises(ValidationError, match=f"outside .*: {rel}"):
        orch.ingest("cdb1", digest_json)
    assert transaction.stray_paths(wiki) == ()


@pytest.mark.parametrize("rel", [".lint-exceptions", ".gitignore",
                                 "errors/.hidden.md", ".github/x.yml"])
def test_no_stage_may_touch_a_hidden_path(wiki, orch, monkeypatch, rel):
    digest_json = ingest_setup(wiki, orch)
    fake_agent(monkeypatch, {"log.md": "# log\nagent\n", rel: "x\n"},
               {"task": "ingest", "summary": "s",
                "pages_touched": ["log.md", rel]})
    with pytest.raises(ValidationError, match=f"hidden path: {rel}"):
        orch.ingest("cdb1", digest_json)
    assert transaction.stray_paths(wiki) == ()


def test_an_agent_cannot_suppress_the_lint_that_judges_it(wiki, orch,
                                                          monkeypatch):
    """`.lint-exceptions` in the same run would have let a broken page land."""
    digest_json = ingest_setup(wiki, orch)
    fake_agent(monkeypatch, {
        "log.md": "# log\nagent\n",
        "errors/ORA-1.md": "---\ntype: nonsense\n---\n# x\n[[does-not-exist]]\n",
        ".lint-exceptions": "errors/ORA-1.md\n"},
        {"task": "ingest", "summary": "s", "notable": False,
         "pages_touched": ["log.md", "errors/ORA-1.md", ".lint-exceptions"]})
    with pytest.raises(ValidationError):
        orch.ingest("cdb1", digest_json)
    assert not (wiki / "errors" / "ORA-1.md").exists()


def test_report_may_not_write_an_incident_page(wiki, orch, monkeypatch):
    fake_agent(monkeypatch, {**report_edits(), "incidents/x.md": "# x\n"},
               {"task": "report", "summary": "s",
                "pages_touched": [f"reports/{DAY}.md", "log.md",
                                  "incidents/x.md"]})
    with pytest.raises(ValidationError, match="outside reports/"):
        orch.report(DAY, WINDOW, [{"db": "cdb1", "notable": False,
                                   "summary": "s"}])
    assert transaction.stray_paths(wiki) == ()


def test_pages_touched_must_name_wiki_paths(wiki, orch):
    (wiki / "log.md").write_text("# log\nentry\n")
    problems = orch._validate(
        result(pages_touched=["log.md", "/etc/passwd", "../x.md"]), "ingest",
        changed(wiki))
    assert sum("not a wiki path" in p for p in problems) == 2


# ---- commit guards (issue 15) ------------------------------------------------

def test_an_agent_cannot_delete_index_and_log(wiki, orch, monkeypatch):
    """A deletion used to count as "log.md updated", and both pages are
    exempt from the undeclared-change check."""
    (wiki / "index.md").write_text("---\ntype: index\n---\n# idx\n")
    git(wiki, "add", "-A")
    git(wiki, "commit", "-m", "idx")
    digest_json = ingest_setup(wiki, orch)
    ...
    def _run(adapter, prompt, w, model, timeout, web=False, provider=None,
        ...
        (w / "log.md").unlink()
        (w / "index.md").unlink()
        return {"task": "ingest", "summary": "s", "notable": False,
                "pages_touched": []}

    monkeypatch.setattr("dbwiki.orchestrate.run_agent", _run)
    with pytest.raises(ValidationError, match="log.md was deleted"):
        orch.ingest("cdb1", digest_json)
    assert (wiki / "log.md").read_text() == "# log\n"
    assert (wiki / "index.md").exists()


def test_emptying_log_md_is_refused(wiki, orch):
    (wiki / "log.md").write_text("  \n")
    problems = orch._validate(result(), "ingest", changed(wiki))
    assert "log.md was deleted or emptied" in problems


def test_only_lint_may_delete_a_page(wiki, orch):
    (wiki / "concepts").mkdir()
    (wiki / "concepts" / "dup.md").write_text("# dup\n")
    git(wiki, "add", "-A")
    git(wiki, "commit", "-m", "dup")
    (wiki / "concepts" / "dup.md").unlink()
    (wiki / "log.md").write_text("# log\nentry\n")
    ingest = orch._validate(result(pages_touched=["log.md", "concepts/dup.md"]),
                            "ingest", changed(wiki))
    assert "ingest may not delete a page: concepts/dup.md" in ingest
    lint = orch._validate(result(task="lint",
                                 pages_touched=["log.md", "concepts/dup.md"]),
                          "lint", changed(wiki))
    assert not any("delete" in p for p in lint)


def test_a_declared_dotfile_is_not_misread_as_undeclared(wiki, orch):
    """`lstrip("./")` stripped characters, not a prefix: `.lint-exceptions`
    read as `lint-exceptions`, never matching the changed path."""
    (wiki / "concepts").mkdir()
    (wiki / "concepts" / "a.md").write_text("# a\n")
    (wiki / "log.md").write_text("# log\nentry\n")
    problems = orch._validate(
        result(pages_touched=["./log.md", "./concepts/a.md"]), "ingest",
        changed(wiki))
    assert problems == []
    (wiki / ".lint-exceptions").write_text("x\n")
    problems = orch._validate(
        result(pages_touched=["log.md", "concepts/a.md", ".lint-exceptions"]),
        "ingest", changed(wiki))
    assert not any("undeclared" in p for p in problems)
    assert any("hidden path: .lint-exceptions" in p for p in problems)


def test_a_commit_during_the_run_is_not_reverted_by_the_rollback(
        wiki, orch, monkeypatch):
    """The restore used the run's old base: a path the human committed
    meanwhile was checked out at its old content (a staged revert) or, absent
    at base, deleted."""
    digest_json = ingest_setup(wiki, orch)
    ...
    def _run(adapter, prompt, w, model, timeout, web=False, provider=None,
        ...
        (w / "concepts").mkdir()
        (w / "concepts" / "notes.md").write_text("human notes\n")
        git(w, "add", "concepts/notes.md")
        git(w, "commit", "-m", "human notes", "--", "concepts/notes.md")
        (w / "concepts" / "notes.md").write_text("agent notes v2\n")
        return {"task": "ingest", "summary": "s", "notable": False,
                "pages_touched": ["log.md", "concepts/notes.md"]}

    monkeypatch.setattr("dbwiki.orchestrate.run_agent", _run)
    with pytest.raises(RuntimeError, match="also changed concepts/notes.md"):
        orch.ingest("cdb1", digest_json)
    assert (wiki / "concepts" / "notes.md").read_text() == "human notes\n"
    assert porcelain(wiki) == ""


def test_a_failure_after_a_mid_run_commit_restores_to_the_new_head(
        wiki, orch, monkeypatch):
    digest_json = ingest_setup(wiki, orch)
    ...
    def _run(adapter, prompt, w, model, timeout, web=False, provider=None,
             telemetry=None):
        (w / "concepts").mkdir()
        (w / "concepts" / "new.md").write_text("human page\n")
        git(w, "add", "concepts/new.md")
        git(w, "commit", "-m", "human page")
        (w / "concepts" / "new.md").write_text("agent scribble\n")
        raise HarnessError("codex exited 3: boom")

    monkeypatch.setattr("dbwiki.orchestrate.run_agent", _run)
    with pytest.raises(HarnessError):
        orch.ingest("cdb1", digest_json)
    assert (wiki / "concepts" / "new.md").read_text() == "human page\n"
    assert porcelain(wiki) == ""


# ---- telemetry block ------------------------------------------------------------

@pytest.mark.parametrize("mode", ["agentic", "structured", "offload",
                                  "caveats", "history"])
def test_the_telemetry_block_names_its_mode(mode):
    """Issue 07 (python-workarounds): `mode` is a parameter of the block, set
    on every path, not a key written back into it afterwards."""
    fields = telemetry_fields("research", "r1", {}, model_tier="cheap",
                              mode=mode, validation_ok=True, rolled_back=False)
