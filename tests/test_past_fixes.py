...
def stage(tmp_path):
    ...
    cfg = make_config(tmp_path, wiki_repo=repo, state_dir=tmp_path / "state")
...
def test_cli_refuses_while_the_config_key_is_off(wiki, capsys):
    ...
    cfg = make_config(research={"history": {"enabled": False}},
...
def test_cli_runs_by_default_when_the_config_block_is_absent(stage, capsys):
    ...
    git(repo, "add", "-A")
    git(repo, "commit", "-m", "seed incidents")
    cfg = make_config(wiki_repo=repo, state_dir=repo.parent / "state")
