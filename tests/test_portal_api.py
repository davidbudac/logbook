...
from dbwiki import (alerts, daily_html, events, gitutil, incident_action,
                    incidents, lifecycle, monitoring)
from dbwiki import observability
from dbwiki import readmodel
from dbwiki import stats
...
def test_the_portal_blocking_itself_says_so_rather_than_naming_its_own_pid(
        api, root):
    """The lock file names `pid <self>` for a portal-versus-portal
...
def test_the_host_check_ignores_case(api):
    """Issue 24: `LOCALHOST:port` names the same socket and was a 403."""
    for spelling in ("LOCALHOST", "LocalHost"):
        status, _ = api.json("GET", "/api/health",
                             headers={"Host": f"{spelling}:{api.port}"})
        assert status == 200, spelling
    status, _ = api.json("GET", "/api/incidents",
                         headers={"Origin": f"HTTP://LOCALHOST:{api.port}"})
    assert status == 200


ALLOWED_CONFIG = CONFIG.replace("portal: {lock_wait_s: 1.0}", """\
portal:
  lock_wait_s: 1.0
  allowed_hosts: [labhost, "Wiki.Example.com:443", "[fd7a::5]"]
""")


@pytest.fixture
def allowed(root):
    """The server with `portal.allowed_hosts` naming the names a LAN browser
    and a TLS reverse proxy reach it by."""
    (root / "config" / "dbwiki.yaml").write_text(ALLOWED_CONFIG)
...
def test_a_configured_host_is_answered_and_every_other_is_still_refused(
        allowed):
    """Issue 24: a `--bind 0.0.0.0` workbench answered only to
    `Host: 0.0.0.0:port`, so a hostname, a LAN IP or a proxy's Host was a
    403. `allowed_hosts` names them; a bare name matches on any port, a
    `name:port` on that port only, and the DNS-rebinding refusal stands for
    every name nobody listed."""
    port = allowed.port
    for host in (f"labhost:{port}", "labhost", "LABHOST:8080",
                 "wiki.example.com:443", f"[fd7a::5]:{port}",
                 f"127.0.0.1:{port}"):
        status, _ = allowed.json("GET", "/api/health",
                                 headers={"Host": host})
        assert status == 200, host
    for host in ("wiki.example.com", "wiki.example.com:8443",
                 f"evil.example:{port}", f"labhost.evil.example:{port}",
                 f"xlabhost:{port}", ""):
        status, body = allowed.json("GET", "/api/incidents",
                                    headers={"Host": host})
        assert (status, body["error"]) == (403, "bad_host"), host


def test_a_configured_host_is_also_an_allowed_origin(allowed):
    """A browser behind a TLS proxy sends `Origin: https://name`, default
    port left out; that is the configured `name:443`."""
    for origin in ("https://wiki.example.com", "https://wiki.example.com:443",
                   "http://labhost:8765", f"http://127.0.0.1:{allowed.port}"):
        status, _ = allowed.json("GET", "/api/incidents",
                                 headers={"Origin": origin})
        assert status == 200, origin
    for origin in ("http://wiki.example.com", "https://evil.example",
                   "https://wiki.example.com.evil.example", "null",
                   "https://wiki.example.com/path"):
        status, body = allowed.json("GET", "/api/incidents",
                                    headers={"Origin": origin})
        assert (status, body["error"]) == (403, "bad_origin"), origin


PROXIED_CONFIG = CONFIG.replace("portal: {lock_wait_s: 1.0}", """\
portal:
  lock_wait_s: 1.0
...
def test_a_trusted_proxy_is_judged_by_the_host_it_forwards(root):
    """A proxy that rewrites `Host` to the upstream loopback address still
    forwards what the browser asked for in `X-Forwarded-Host`; from a peer
    in `trusted_proxies` that is the name checked, so a rebinding page
    behind the proxy is refused even though the proxy's own Host is
    loopback."""
    (root / "config" / "dbwiki.yaml").write_text(PROXIED_CONFIG)
    srv, thread = serve(load_config(root))
    try:
        client = Client(srv.server_address)
        status, _ = client.json(
            "GET", "/api/incidents",
            headers={"X-Forwarded-Host": "wiki.example.com"})
        assert status == 200
        ...
        status, _ = client.json(
            "GET", "/api/incidents",
            headers={"X-Forwarded-Host": "wiki.example.com, evil.example"})
        assert status == 200, "the first hop is the browser's own request"
    finally:
        srv.shutdown()
        srv.server_close()
        thread.join(timeout=10)
...
def test_a_forwarded_host_from_an_untrusted_peer_is_ignored(allowed):
    """Without `trusted_proxies` anybody can send the header, so it never
    replaces the Host the socket was reached by."""
    status, body = allowed.json(
        "GET", "/api/incidents",
        headers={"Host": "evil.example", "X-Forwarded-Host": "labhost"})
    assert (status, body["error"]) == (403, "bad_host")


@pytest.mark.parametrize("portal_block, word", [
    ("allowed_hosts: ['*']", "allowed_hosts"),
    ("allowed_hosts: ['http://wiki.example.com']", "allowed_hosts"),
    ("allowed_hosts: ['wiki example']", "allowed_hosts"),
    ("allowed_hosts: wiki.example.com", "allowed_hosts"),
    ("allowed_hosts: ['wiki.example.com:http']", "allowed_hosts"),
    ("trusted_proxies: [wiki.example.com]", "trusted_proxies"),
    ("trusted_proxies: 127.0.0.1", "trusted_proxies"),
])
def test_a_host_setting_that_would_weaken_the_check_refuses_to_start(
        root, portal_block, word):
    """A wildcard or a URL would either disable the DNS-rebinding refusal or
    silently match nothing; either way the server does not start on it,
    the `advisory:` precedent."""
    (root / "config" / "dbwiki.yaml").write_text(CONFIG.replace(
        "portal: {lock_wait_s: 1.0}",
        f"portal:\n  lock_wait_s: 1.0\n  {portal_block}\n"))
    with pytest.raises(ValueError, match=word):
        portal.make_server(load_config(root))
...
def _raw_post(api, length: str) -> bytes:
    """The head of a POST whose `Content-Length` is `length` verbatim, and
    then silence on a connection left open: a server that reads to
    end-of-stream instead of trusting a count sits in the read until its
    socket timeout, ten seconds later."""
    sock = socket.create_connection((api.host, api.port), timeout=5)
    try:
        sock.sendall(f"POST /api/inbox HTTP/1.1\r\nHost: {api.authority}\r\n"
                     f"Content-Type: application/json\r\n"
                     f"Content-Length: {length}\r\n\r\n".encode())
        answer = b""
        while part := sock.recv(4096):  # the refusal closes the connection
            answer += part
        return answer
    finally:
        sock.close()
...
@pytest.mark.parametrize("length", ["-1", "-1048577", "+12", "1_0", " "])
def test_a_content_length_that_is_not_a_plain_count_is_refused_unread(
        api, length):
    """`int("-1")` passed the cap and `rfile.read(-1)` then buffered until
    the client hung up (issue 11): 200 MB sent, 700 MB resident. Only
    digits are a byte count; the refusal is immediate and reads nothing."""
    started = time.monotonic()
    answer = _raw_post(api, length)
    assert time.monotonic() - started < 4
    status_line, _, rest = answer.partition(b"\r\n")
    assert status_line.split()[1] == b"400", answer[:200]
    assert b'"bad_length"' in rest
...
def _fleet_wiki(root):
    ...
    (wiki / f"errors/{CODE}.md").write_text(
        ...
        "## Past fixes\n\n"
        "2 fixes across 2 incidents on 1 database: 1 held, 1 reopened.\n\n"
        "| when | db | incident | what was done | outcome | held |\n"
        "|---|---|---|---|---|---|\n"
        f"| 2026-08-02 | {DB} | [[incidents/{OLD_FIX_SLUG}]] | "
        "raised the listener queue size | succeeded | held (14d) |\n"
        f"| 2026-07-02 | {DB} | [[incidents/{DONE_SLUG}]] | "
        "restarted the listener | succeeded | reopened 2026-07-20 |\n")
    git(wiki, "add", "-A")
...
def test_the_past_fixes_reach_the_case_file_beside_the_resolutions(reading):
    """The error page's `## Past fixes` table rides on the same card as its
    resolution history, with whether each fix held."""
    _, body = reading.get(f"/api/incidents/{OPEN_SLUG}")
    rows = {row["code"]: row for row in body["research"]}
    assert rows[CODE]["past_fixes"] == [
        {"day": "2026-08-02", "db": DB, "incident": OLD_FIX_SLUG,
         "path": f"incidents/{OLD_FIX_SLUG}.md",
         "action": "raised the listener queue size", "outcome": "succeeded",
         "held": "held (14d)"},
        {"day": "2026-07-02", "db": DB, "incident": DONE_SLUG,
         "path": f"incidents/{DONE_SLUG}.md",
         "action": "restarted the listener", "outcome": "succeeded",
         "held": "reopened 2026-07-20"}]
...
#: A standby whose digest predates the `changes` key, so its one change is
#: derived from a lifecycle group the way the daily page derives it.
STBY = f"{DB}_stby"
BROKEN_DB = "cdb9"
STBY_DIGEST = f"digests/{STBY}/{QUIET_DAY}.md"

RECORDED_CHANGES = [
    {"day": QUIET_DAY, "ts": f"{QUIET_DAY}T02:10:00Z",
     "rule": "alter_system_set", "count": 9,
     "message": "ALTER SYSTEM SET log_archive_dest_state_2=DEFER SCOPE=BOTH;"},
    {"day": QUIET_DAY, "ts": f"{QUIET_DAY}T04:00:00Z",
     "rule": "instance_startup", "count": 1,
     "message": "Starting ORACLE instance (normal)"},
]


def _changed_wiki(root):
    """The fleet wiki plus a day on which the operators did things: `DB`'s
    digest records two changes under the `changes` key, the standby's digest
    predates the key and carries a lifecycle group beside an error group, a
    third database's sidecar will not parse, and the loud day before holds a
    change of its own that the quiet day's screens must not show."""
    _fleet_wiki(root)
    wiki = root / "wiki"
    quiet = json.loads(_digest_sidecar(QUIET_DAY, {"alert": {"routine": 40}}))
    quiet["changes"] = RECORDED_CHANGES
    (wiki / "digests" / DB / f"{QUIET_DAY}.json").write_text(json.dumps(quiet))
    loud = json.loads((wiki / "digests" / DB / f"{LOUD_DAY}.json").read_text())
    loud["changes"] = [{"day": LOUD_DAY, "ts": f"{LOUD_DAY}T23:59:00Z",
                        "rule": "instance_shutdown", "count": 1,
                        "message": "Shutting down instance (immediate)"}]
    (wiki / "digests" / DB / f"{LOUD_DAY}.json").write_text(json.dumps(loud))
    stby = json.loads(_digest_sidecar(QUIET_DAY, {"alert": {"routine": 7}}))
    stby["db"] = STBY
    stby["sources"]["alert"]["notable"] = [
        {"class": "lifecycle", "rule": "mrp_start", "count": 2,
         "first_ts": f"{QUIET_DAY}T03:00:00Z",
         "message": "Tue Aug 30 03:00:00 2026\nMRP0 started",
         "headline": "MRP0: Background Managed Standby Recovery process "
                     "started"},
        {"class": "error", "rule": "ora_error", "count": 3,
         "codes": ["ORA-16055"], "first_ts": f"{QUIET_DAY}T03:05:00Z",
         "message": "ORA-16055"}]
    (wiki / "digests" / STBY).mkdir(parents=True)
    (wiki / "digests" / STBY / f"{QUIET_DAY}.json").write_text(
        json.dumps(stby))
    (wiki / STBY_DIGEST).write_text(f"# {STBY} {QUIET_DAY}\n")
    (wiki / "digests" / BROKEN_DB).mkdir(parents=True)
    (wiki / "digests" / BROKEN_DB / f"{QUIET_DAY}.json").write_text(
        "{not json")
    git(wiki, "add", "-A")
    git(wiki, "commit", "-m", "ingest: a day of changes")
...
def test_a_digest_screen_carries_the_changes_its_sidecar_recorded(changed):
    """What the operators did on that database that day, above the errors:
    the recorded `changes`, in order, each naming the digest it came from."""
    status, body = page_at(changed, DIGEST)
    assert status == 200
    assert body["changes"] == [
        {**change, "db": DB, "page": DIGEST} for change in RECORDED_CHANGES]


def test_a_digest_written_before_the_changes_key_still_shows_its_changes(
        changed):
    """`changes.of_digest` derives from the lifecycle groups, the reader the
    markdown `## Changes` section and the daily page use; the error group
    beside it is not a change."""
    _, body = page_at(changed, STBY_DIGEST)
    assert body["changes"] == [
        {"db": STBY, "day": QUIET_DAY, "ts": f"{QUIET_DAY}T03:00:00Z",
         "rule": "mrp_start", "count": 2,
         "message": "MRP0: Background Managed Standby Recovery process "
                    "started",
         "page": STBY_DIGEST}]


def test_the_day_screen_is_one_fleet_timeline_of_that_days_changes(changed):
    """The report is the portal's page for a day, so it carries every
    database's changes on that day as one timeline — and not the day
    before's, and not the database whose sidecar will not parse."""
    _, body = page_at(changed, REPORT_PATH)
    assert [(row["ts"][11:16], row["db"], row["rule"])
            for row in body["changes"]] == [
        ("02:10", DB, "alter_system_set"),
        ("03:00", STBY, "mrp_start"),
        ("04:00", DB, "instance_startup")]
    assert {row["page"] for row in body["changes"]} == {DIGEST, STBY_DIGEST}


def test_the_day_screen_and_the_daily_page_list_the_same_changes(changed,
                                                                 root):
    """Both views read through `daily_html.change_rows`, so the strip the
    portal draws and the strip the daily page draws cannot disagree."""
    _, body = page_at(changed, REPORT_PATH)
    daily = daily_html.change_rows(
        daily_html.day_digests(root / "wiki", QUIET_DAY))
    assert [(row["db"], row["ts"], row["rule"], row["count"], row["message"])
            for row in body["changes"]] == [
        (db, c.ts, c.rule, c.count, c.message) for db, c in daily]


@pytest.mark.parametrize("path", [OPEN_PATH, f"databases/{DB}.md",
                                  "index.md"])
def test_a_page_that_is_neither_a_day_nor_a_digest_carries_no_changes(
        changed, path):
    _, body = page_at(changed, path)
    assert body["changes"] == []
...
def test_freshness_is_the_pinned_now_against_the_configured_budget(automation):
    ...
    assert set(rows) == {"run", "report", "lint", "research",
                         "research_caveats", "research_history"}
...
def test_research_freshness_is_per_mode_and_a_dry_run_proves_nothing():
    """Issue 17 in the portal: a nightly `--history` pass must not prove the
    weekly agentic research ran, and a dry run proves no mode at all. The
    rows are keyed as `health._last_successes` keys them."""
    from dbwiki import health
    from dbwiki.portal import api

    def research(at, mode="", dry_run=None, outcome="ok"):
        return events.RunEvent(kind=events.Kind.RUN, run_id=at, at=at,
                               finished=at, command="research",
                               outcome=outcome, mode=mode, dry_run=dry_run)

    stream = [research("2026-08-31T03:00:00Z", mode="history"),
              research("2026-08-31T04:00:00Z", dry_run=True),
              research("2026-08-30T03:00:00Z", mode="caveats",
                       outcome="failed"),
              research("2026-08-20T03:00:00Z")]
    rows = {row["task"]: row for row in api._freshness(
        stream, RUNS_NOW, health.DEFAULT_STAGE_STALE_HOURS)}
    assert rows["research_history"]["at"] == "2026-08-31T03:00:00Z"
    assert rows["research"]["at"] == "2026-08-20T03:00:00Z"
    assert rows["research"]["stale"] is True
    assert rows["research_caveats"]["at"] == ""


def test_a_research_run_carries_its_mode_and_dry_run_out_of_the_log(tmp_path):
    state = tmp_path / ".state"
    state.mkdir()
    (state / HEALTH_LOG).write_text(json.dumps(
        {"run_id": "aaaaaaaaaaaa", "command": "research",
         "started": LINT_AT, "finished": LINT_AT, "outcome": "ok",
         "facts": {"mode": "caveats", "dry_run": False}}) + "\n")
    [run] = [ev for ev in events.load_events(state).events
             if ev.kind is events.Kind.RUN]
    assert run.mode == "caveats" and run.dry_run is False
...
def _cli_stage_counts(root, hours):
    """What `dbwiki stats --since <hours>h --by model` counts per model over
    the same agent-run log, the model's rows summed across task, adapter and
    mode — the split the Agents table carries as columns instead."""
    lines = [json.loads(line) for line in
             (root / ".state" / AGENT_LOG).read_text().splitlines()
             if line.strip()]
    now = dt.datetime.fromisoformat(RUNS_NOW.replace("Z", "+00:00"))
    report = stats.collect({}, events=lines, by="model",
                           since=stats.parse_when(f"{hours}h", now))
    counts: dict[str, int] = {}
    for row in report["groups"]:
        counts[row["model"]] = counts.get(row["model"], 0) + row["n"]
    return counts


def _portal_stage_counts(body):
    return {row["model"] or stats.UNKNOWN: row["totals"]["stages"]
            for row in body["models"]}


@pytest.mark.parametrize("hours", [48, 6])
def test_the_model_table_counts_what_dbwiki_stats_counts(agentic, root,
                                                        hours):
    """incident-workbench #31: the table groups through `stats.group`, so a
    model's stage count on the screen is what the CLI counts for it over the
    same window."""
    _, body = agentic.get(f"/api/agents?hours={hours}")
    assert _portal_stage_counts(body) == _cli_stage_counts(root, hours)


def test_a_stage_the_log_holds_twice_is_counted_once_as_the_cli_counts_it(
        root):
    """`event_id` is the fold-in's idempotency key, and `dbwiki stats`
    dedupes on it. A line the log holds twice is one stage run once, so the
    screen draws it once too: in its tick, in its model's row and in the
    window's total."""
    _state(root)
    log = root / ".state" / AGENT_LOG
    log.write_text(log.read_text() + json.dumps(
        _agent_line("ev-ingest-cdb1", RUN)) + "\n")
    srv, thread = serve(load_config(root), now=lambda: RUNS_NOW)
    try:
        _, body = Client(srv.server_address).get("/api/agents?hours=48")
    finally:
        srv.shutdown()
        srv.server_close()
        thread.join(timeout=10)
    tick = next(t for t in body["ticks"] if t["run_id"] == RUN)
    assert [s["event_id"] for s in tick["stages"]].count("ev-ingest-cdb1") \
        == 1
    assert body["totals"]["stages"] == sum(
        row["totals"]["stages"] for row in body["models"])
    assert _portal_stage_counts(body) == _cli_stage_counts(root, 48)
