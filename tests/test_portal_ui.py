...
QUIET_QUEUE = dict(QUEUE, incidents=[
...
QUIET_SLUG = QUIET_QUEUE["incidents"][0]["slug"]
...
def mock_page(tmp_path, name, commits=None, incident=None, tools=None,
    ...
    data = {"slug": SLUG,
            ...
            "db": DB if db is None else db, "pages": PAGES,
            "search": SEARCH,
...
QUIET_INCIDENT = dict(INCIDENT, status="open", label="Open",
...
def test_an_incident_the_wiki_can_answer_nothing_about_draws_only_the_line(
    ...
    blank = dict(INCIDENT, research=[
        dict(row, researched="", cause="", action="", citations=[], notes=[],
             resolutions=[]) for row in RESEARCH])
