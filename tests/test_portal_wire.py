...
from dbwiki.incident_action import (PRESENTATION, VERBS, LockBusy,
...
from dbwiki.readmodel import Citation, Note, Research, Resolution
...
OUTCOMES = (Committed("6765a05", (PATH, "log.md"), True),
            ...
            LintBlocked((FINDING,)),
            LockBusy(holder_message(os.getpid() + 1)))
...
def test_a_busy_lock_the_portal_itself_holds_says_so_in_both_copies():
    ...
    the same screen."""
    body = wire.outcome_json(LockBusy(holder_message(os.getpid())), base=BASE,
...
def test_a_lock_this_very_process_holds_reads_as_the_workbench_not_a_stranger():
    body = wire.error_json("lock_busy", holder_message(os.getpid()),
                           retry_after_s=RETRY_AFTER_S)
    assert body == {"error": "lock_busy",
                    "message": "another workbench request is publishing",
                    "retry_after_s": RETRY_AFTER_S}
...
def test_only_a_lock_busy_message_is_ever_rewritten():
    message = holder_message(os.getpid())
    assert wire.error_json("tree_dirty", message)["message"] == message


def test_a_holder_message_that_does_not_parse_passes_through_unchanged():
...
def incident_view(**overrides) -> dict:
    args = {"revision": BASE, "body": PAGE, "allowed": (),
            ...
            "resolutions": RESOLUTIONS, "last_seen": LAST_SEEN}
