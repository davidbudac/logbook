...
def cfg_for(wiki, **research):
    return make_config(wiki_repo=wiki, agents={"claude": {"cheap": "sonnet"}},
                       research=research)
...
def test_cli_refuses_while_the_config_key_is_off(wiki, capsys):
    ...
    cfg = make_config(research={"caveats": {"enabled": False}},
...
def test_cli_dry_run_prints_the_candidates_and_the_first_prompt(wiki, capsys):
    ...
    cfg = make_config(research={"caveats": {"enabled": True, "limit": 2}},
                      wiki_repo=wiki, state_dir=wiki / ".state")
...
def test_cli_says_nothing_is_due_when_no_page_qualifies(wiki, capsys):
    from dbwiki.cli import _run_caveats
    cfg = make_config(research={"caveats": {"enabled": True}},
                      wiki_repo=wiki, state_dir=wiki / ".state")
    assert _run_caveats(cli_args(), cfg, None, wiki) == 0
    assert "nothing to review" in capsys.readouterr().out


def _research_events(state_dir):
    from dbwiki.health import HEALTH_LOG
    path = state_dir / HEALTH_LOG
    if not path.exists():
        return []
    return [json.loads(line) for line in path.read_text().splitlines()]


def test_a_caveat_pass_with_nothing_due_still_proves_the_stage_ran(wiki):
    """Issue 17 split research freshness per mode; a pass that finds nothing
    due is the stage working, so it records an ok run with `mode: caveats`
    that health counts. A dry run records nothing."""
    from dbwiki import health
    from dbwiki.cli import _run_caveats
    cfg = make_config(research={"caveats": {"enabled": True}},
                      wiki_repo=wiki, state_dir=wiki / ".state")
    assert _run_caveats(cli_args(), cfg, None, wiki) == 0
    assert _research_events(cfg.state_dir) == []
    assert _run_caveats(cli_args(dry_run=False), cfg, None, wiki) == 0
    [ev] = _research_events(cfg.state_dir)
    assert ev["command"] == "research" and ev["outcome"] == "ok"
    assert ev["facts"]["mode"] == "caveats"
    last = health._last_successes([ev], {})
    assert last["research_caveats"]["at"] == ev["finished"]
    assert last["research"] is None


def test_a_research_run_with_nothing_due_still_proves_the_stage_ran(
        wiki, monkeypatch, capsys):
    from dbwiki import cli, health
    cfg = make_config(wiki_repo=wiki, state_dir=wiki / ".state")
    monkeypatch.setattr(cli, "load_config", lambda: cfg)
    monkeypatch.setattr("dbwiki.orchestrate.Orchestrator",
                        lambda cfg, lock=None: None)
    monkeypatch.setattr("dbwiki.research.select_error_candidates",
                        lambda wiki, limit: [])
    monkeypatch.setattr("dbwiki.research.sources_due_review",
                        lambda wiki, today: [])
    args = SimpleNamespace(caveats=False, history=False, sources_only=False,
                           review_sources=False, limit=None, dry_run=False)
    assert cli.cmd_research(args) == 0
    assert "nothing to research" in capsys.readouterr().out
    [ev] = _research_events(cfg.state_dir)
    assert ev["outcome"] == "ok" and "mode" not in ev["facts"]
    assert health._last_successes([ev], {})["research"]["at"] == ev["finished"]
