...
def snapshot(incidents: tuple[Incident, ...] = (),
    ...
    return Snapshot(revision=REV, built_at=NOW,
                    ...
                    past_fixes={}, journals={}, touches_by_run={},
...
def test_input_ordering_does_not_move_the_answer():
    ...
    shuffled = Snapshot(
        ...
        past_fixes={}, journals={}, touches_by_run={}, touches_by_incident={},
...
def test_rules_default_when_the_block_is_absent():
    assert Rules.resolve(make_config()) == Rules()
...
def test_concurrent_acts_on_different_findings_all_land(tmp_path,
                                                         monkeypatch):
    """Issue 19: `acknowledge` and `suppress` read-modify-write `acks.json`,
    and two tabs (or two operators) acting at once each wrote back the file
    they had read, dropping the other's act: 40 parallel acks left 8 in the
    file beside 14 audit rows. The read is slowed so the window is hit every
    time rather than on a lucky schedule."""
    real = review.load_acks
    ...
    monkeypatch.setattr(review, "load_acks", slow_load)
    fps = [f"fp{n:02d}" for n in range(40)]
    ready = threading.Barrier(len(fps))
    errors: list[BaseException] = []

    def act(n: int, fp: str) -> None:
        ready.wait(timeout=10)
        try:
            if n % 2:
                suppress(tmp_path, fp, actor="alice", now=NOW, days=7)
            else:
                acknowledge(tmp_path, fp, actor="bob", now=NOW)
        except BaseException as e:  # noqa: BLE001 — the assertion names it
            errors.append(e)

    threads = [threading.Thread(target=act, args=(n, fp))
    ...
    assert errors == []
    items = review.load_acks(tmp_path)["items"]
    assert sorted(items) == fps
    assert all(items[fp]["acknowledged_at"] == NOW for fp in fps[0::2])
    assert all(items[fp]["suppressed_until"] for fp in fps[1::2])
    assert sorted(r["fingerprint"] for r in audit_rows(tmp_path)) == fps


def test_an_act_waits_for_another_process_holding_the_acks_lock(tmp_path):
    """The in-process lock covers two portal threads; the flock covers a
    second portal (or a CLI) on the same `.state`."""
    lock_path = tmp_path / REVIEW_DIR / review.ACKS_LOCK
    ...
    with open(lock_path, "a") as held:
        ...
        worker.start()
        assert not done.wait(0.3), "acknowledge wrote under a held flock"
        fcntl.flock(held.fileno(), fcntl.LOCK_UN)
    worker.join(timeout=10)
    assert done.is_set()
    assert review.load_acks(tmp_path)["items"]["abc123"]["acknowledged_at"] \
        == NOW
...
def cfg_for_model():
    return make_config(agents={"pi": {"cheap": "gemma-3", "strong": "qwen"},
                               "timeout_seconds": 60})
...
def cfg(tmp_path, wiki):
    return make_config(tmp_path, state_dir=tmp_path / ".state", wiki_repo=wiki,
...
def test_a_same_week_rerun_retries_a_failed_delivery(cfg, monkeypatch):
    """The `exists` short-circuit used to come before delivery, so a second
    cron run in the same week never retried the email the first one lost."""
    fake_propose(monkeypatch)
    relay_for(cfg, monkeypatch, OSError("connection refused"))
    review.run(cfg, now=NOW)
    live = Relay()
    monkeypatch.setattr(delivery, "_open_smtp", live)
    outcome = review.run(cfg, now="2026-09-08T10:00:00Z")
    assert outcome.skipped == "exists" and not outcome.published
    assert (outcome.delivered, outcome.failed) == (1, 0)
    assert live.sent == ["ops@example.com"]
    again = review.run(cfg, now="2026-09-09T10:00:00Z")
    assert (again.delivered, again.failed) == (0, 0)
    assert live.sent == ["ops@example.com"]


def test_a_same_week_dry_run_delivers_nothing(cfg, monkeypatch):
    fake_propose(monkeypatch)
    relay_for(cfg, monkeypatch, OSError("connection refused"))
    review.run(cfg, now=NOW)
    live = Relay()
    monkeypatch.setattr(delivery, "_open_smtp", live)
    outcome = review.run(cfg, now=NOW, dry_run=True)
    assert (outcome.delivered, outcome.failed) == (0, 0) and live.sent == []


def test_deliver_only_can_reach_an_earlier_week(cfg, monkeypatch):
    """Last week's failure was out of reach: `--deliver-only` only ever
    looked at `iso_week(now)`."""
    fake_propose(monkeypatch)
    relay_for(cfg, monkeypatch, OSError("connection refused"))
    review.run(cfg, now=NOW)
    live = Relay()
    monkeypatch.setattr(delivery, "_open_smtp", live)
    outcome = review.run(cfg, now=later(7), deliver_only=True,
                         review_id=REVIEW_ID)
...
def test_review_id_is_validated_and_only_for_deliver_only(cfg):
    with pytest.raises(ValueError, match="not YYYY-Www"):
        review.run(cfg, now=NOW, deliver_only=True, review_id="../../x")
    with pytest.raises(ValueError, match="--deliver-only"):
        review.run(cfg, now=NOW, review_id=REVIEW_ID)
