...
def test_a_failed_write_leaves_the_previous_file_intact(tmp_path, monkeypatch):
    ...
    def half_open(fd, *a, **kw):
        f = real_fdopen(fd, *a, **kw)
        real_write = f.write

        def write(text):
            real_write(text[:4])      # a half-written tmp file, then a failure
            raise OSError("disk full")

        f.write = write
        return f

    monkeypatch.setattr(os, "fdopen", half_open)
...
def test_concurrent_writers_in_one_process_never_collide(tmp_path):
    # The portal writes from several threads of one pid; a pid-only tmp name
    # let one thread rename another's tmp away (FileNotFoundError).
    p = tmp_path / "acks.json"
    values = [f"writer {i}\n" * 200 for i in range(20)]
    errors: list[BaseException] = []
    start = threading.Barrier(len(values))
    ...
    for t in threads:
        t.join()
    assert errors == []
    assert p.read_text() in values
    assert sorted(q.name for q in tmp_path.iterdir()) == ["acks.json"]


def test_atomic_write_keeps_the_umask_mode_not_mkstemps_0600(tmp_path):
    p = tmp_path / "f.json"
    atomic_write_text(p, "x")
    q = tmp_path / "g.json"
    q.write_text("x")
    assert stat.S_IMODE(p.stat().st_mode) == stat.S_IMODE(q.stat().st_mode)
