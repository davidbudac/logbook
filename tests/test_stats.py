...
from dbwiki.stats import (MIN_SAMPLE, collect, format_stats, group,
                          parse_when)
...
def test_a_raising_telemetry_step_does_not_block_the_commit(cfg, orch,
    ...
    assert "mode" in e                  # so does the mode (2026-09-24)
...
def test_stats_cli_prints_a_table_and_json(cfg, orch, monkeypatch, capsys):
    ...
    assert cli.cmd_stats(SimpleNamespace(json=False, task=None, since=None,
                                          until=None, by="tier")) == 0
    ...
    assert cli.cmd_stats(SimpleNamespace(json=True, task="ingest", since=None,
                                          until=None, by="tier")) == 0
...
def test_stats_cli_reads_the_agent_run_log(cfg, monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli, "load_config", lambda: cfg)
    assert cli.cmd_stats(SimpleNamespace(json=True, task=None, since=None,
                                          until=None, by="tier")) == 0
    s = json.loads(capsys.readouterr().out)
    assert s["runs"] == 2
...
# ---- time window and model grouping (incident-workbench #30) -------------------

NOW = dt.datetime(2026, 9, 24, 12, 0, tzinfo=dt.timezone.utc)


@pytest.mark.parametrize("text, expected", [
    ("12h", NOW - dt.timedelta(hours=12)),
    ("90m", NOW - dt.timedelta(minutes=90)),
    ("2d", NOW - dt.timedelta(days=2)),
    ("1w", NOW - dt.timedelta(weeks=1)),
    ("2026-09-23", dt.datetime(2026, 9, 23, tzinfo=dt.timezone.utc)),
    ("2026-09-23T18:30:00Z", dt.datetime(2026, 9, 23, 18, 30,
                                         tzinfo=dt.timezone.utc)),
    ("2026-09-23T20:30:00+02:00", dt.datetime(2026, 9, 23, 18, 30,
                                              tzinfo=dt.timezone.utc)),
])
def test_since_takes_an_iso_instant_or_a_duration_back_from_now(text, expected):
    assert parse_when(text, NOW) == expected


@pytest.mark.parametrize("text", ["yesterday", "12", "-3h", "12x", ""])
def test_an_unreadable_window_bound_is_refused(text):
    with pytest.raises(ValueError, match="ISO date"):
        parse_when(text, NOW)


def test_the_window_filters_events_before_they_become_rows():
    evs = [event(at="2026-09-23T10:00:00Z", cost_usd=1.0),
           event(at="2026-09-23T22:00:00Z", cost_usd=2.0),
           event(at="2026-09-24T11:00:00Z", cost_usd=4.0),
           event(at=None, cost_usd=8.0)]         # no instant: not in any window
    last_night = collect({}, events=evs,
                         since=dt.datetime(2026, 9, 23, 18, tzinfo=dt.timezone.utc),
                         until=dt.datetime(2026, 9, 24, 6, tzinfo=dt.timezone.utc))
    assert last_night["runs"] == 1
    assert last_night["totals"]["total_cost_usd"] == 2.0
    assert last_night["since"] == "2026-09-23T18:00:00Z"
    assert last_night["until"] == "2026-09-24T06:00:00Z"
    assert collect({}, events=evs)["runs"] == 4  # no window: every line


def test_the_window_applies_to_legacy_ledger_rows_too():
    led = ledger(entry(at="2026-09-01T00:00:00Z"), entry(at="2026-09-23T23:00:00Z"),
                 entry())                         # no `at`: outside any window
    s = collect(led, since=dt.datetime(2026, 9, 23, tzinfo=dt.timezone.utc))
    assert s["runs"] == 1


def test_by_model_splits_what_by_tier_lumps_together():
    evs = [event(adapter="pi", model="nemotron", model_tier="cheap"),
           event(adapter="pi", model="lfm2.5-2.6b", model_tier="cheap"),
           event(adapter="codex", model="gpt-5-codex", model_tier="strong",
                 mode="agentic")]
    assert len(collect({}, events=evs)["groups"]) == 2
    s = collect({}, events=evs, by="model")
    assert s["by"] == "model"
    assert {(g["adapter"], g["model"]) for g in s["groups"]} == {
        ("pi", "nemotron"), ("pi", "lfm2.5-2.6b"), ("codex", "gpt-5-codex")}
    assert all("model_tier" not in g for g in s["groups"])
    text = format_stats(s)
    assert "nemotron" in text and "lfm2.5-2.6b" in text
    assert text.splitlines()[2].split()[:3] == ["task", "adapter", "model"]


def test_the_default_report_keeps_its_shape():
    s = collect({}, events=[event()])
    assert not {"since", "until", "by"} & set(s)
    assert "tier" in format_stats(s).splitlines()[2].split()


def test_stats_cli_window_and_grouping(cfg, monkeypatch, capsys):
    recent = dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    lines = [json.dumps(event(event_id="old", at="2020-01-01T00:00:00Z")),
             json.dumps(event(event_id="new", at=recent, model="nemotron"))]
    (cfg.state_dir / "agent_runs.jsonl").write_text("\n".join(lines) + "\n")
    monkeypatch.setattr(cli, "load_config", lambda: cfg)
    assert cli.main(["stats", "--json", "--since", "12h", "--by", "model"]) == 0
    s = json.loads(capsys.readouterr().out)
    assert s["runs"] == 1 and s["groups"][0]["model"] == "nemotron"
    assert cli.main(["stats", "--since", "12h"]) == 0
    assert "since=" in capsys.readouterr().out.splitlines()[0]
    assert cli.main(["stats", "--until", "last tuesday"]) == 2
    assert "--until" in capsys.readouterr().err


def test_group_rolls_a_callers_rows_up_with_the_callers_measure():
    """The workbench's Agents table (incident-workbench #31) groups through
    `stats.group` by model alone and keeps its own measure (tokens, wall
    time); the partition and its key order are this module's."""
    rows = [{"model": "qwen", "x": 1}, {"model": "luna", "x": 2},
            {"model": "qwen", "x": 3}]
    rolled = group(rows, by="model_only",
                   agg=lambda held: {"x": sum(r["x"] for r in held)})
    assert rolled == [{"model": "luna", "x": 2}, {"model": "qwen", "x": 4}]


def test_the_cli_offers_only_the_groupings_its_table_can_draw():
    """`model_only` has no third column for `format_stats` to print, so it
    is the workbench's key and not a `--by` choice."""
    with pytest.raises(SystemExit):
        cli.main(["stats", "--by", "model_only"])
