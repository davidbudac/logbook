...
def test_propose_retries_once_quoting_the_validation_error(monkeypatch):
    ...
    tele = {}
    got = propose("PROMPT", make_config(), telemetry=tele)
    assert got["summary"] == "ORA-00600 storm on cdb1"
    assert len(seen) == 2
...
def test_propose_accepts_the_first_answer_without_a_retry(monkeypatch):
    ...
    tele = {}
    assert propose("PROMPT", make_config(),
...
def test_a_retried_proposal_records_two_attempts(monkeypatch):
    ...
    tele = {}
    propose("PROMPT", make_config(), telemetry=tele)
...
@pytest.mark.parametrize("target", [
    "incidents/", "incidents", "./incidents/", "incidents/sub",
    "incidents/sub/x.md", "incidents/../log.md", "incidents/.hidden.md",
    "incidents/2026-07-11-cdb1-standby-gap", "incidents/link.md"])
def test_incident_update_names_a_regular_incident_page_or_nothing(
        wiki_with_incident, target):
    """`incidents/` passed the old prefix check and raised IsADirectoryError
    after the database page and the journal were already on disk."""
    (wiki_with_incident / "incidents" / "sub").mkdir()
    (wiki_with_incident / "incidents" / "sub" / "x.md").write_text(OPEN_INCIDENT)
    (wiki_with_incident / "incidents" / ".hidden.md").write_text(OPEN_INCIDENT)
    (wiki_with_incident / "incidents" / "link.md").symlink_to(
        "2026-07-11-cdb1-standby-gap.md")
    res = apply(wiki_with_incident, parsed(incident={
        "action": "update", "slug": None, "title": None, "body": "b",
        "existing_page": target}))
    assert res["incidents_updated"] == []
    assert any("skipped" in f for f in res["flags"])


def test_incident_update_accepts_a_leading_dot_slash(wiki_with_incident):
    rel = "incidents/2026-07-11-cdb1-standby-gap.md"
    res = apply(wiki_with_incident, parsed(incident={
        "action": "update", "slug": None, "title": None, "body": "b",
        "existing_page": f"./{rel}"}))
    assert res["incidents_updated"] == [rel]
...
def test_agentic_mode_is_untouched_by_the_structured_switch(orch, repo, monkeypatch):
    ...
    # the ledger block names the mode on every path (2026-09-24)
    assert orch.state.get_ledger()[f"digests/cdb1/{DAY}.json"]["mode"] == "agentic"
