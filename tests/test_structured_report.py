...
def orch(repo, tmp_path):
    ...
    # reach a real model
    cfg = make_config(
        tmp_path, wiki_repo=repo, state_dir=tmp_path / "state",
...
def test_material_carries_the_changes_section(tmp_path):
    """ADR-0006 moved lifecycle groups out of `### Notable` into `## Changes`;
    the excerpt must follow them there or an escalated window loses every
    parameter change and bounce it used to see."""
    root = tmp_path / "wiki"
    (root / "digests" / "cdb1").mkdir(parents=True)
    (root / f"digests/cdb1/{DAY}.md").write_text(
        "# Digest: cdb1 — " + DAY + "\n\n"
        "## Changes\n\n"
        "- 2026-07-27T02:15:12Z parameter_change ×1 — ALTER SYSTEM SET "
        "log_archive_dest_state_2='DEFER' SCOPE=BOTH;\n\n"
        "## alert — 3 events\n\nclasses: lifecycle: 1, info: 2\n")
    ingested = [{"db": "cdb1", "notable": True, "summary": "s"}]
    p = build_escalated_report_prompt(DAY, WINDOW, ingested, root)
    assert "Notable ingest — cdb1" in p
    assert "## Changes" in p
    assert "log_archive_dest_state_2='DEFER'" in p
    assert "classes: lifecycle" not in p


def journal_entry(root, db: str, day: str, headline: str) -> None:
    j = root / "databases" / db / "journal" / f"{day[:7]}.md"
    j.write_text(j.read_text() + f"\n## {day} — {headline}\n\nProse.\n")


def test_material_carries_each_notable_dbs_history(escalated_wiki):
    journal_entry(escalated_wiki, "cdb1", "2026-07-20", "CDB1-EARLIER-MARKER")
    journal_entry(escalated_wiki, "cdb2", "2026-07-20", "CDB2-EARLIER-MARKER")
    p = build_escalated_report_prompt(DAY, WINDOW, ESCALATED_INGESTED,
                                      escalated_wiki)
    material = p.partition("\nMaterial (")[2]
    assert "### Database history — cdb1\n" in material
    assert ("Database history for cdb1 (last 90 days; context from earlier "
            "days, not evidence for today):") in material
    assert "- 2026-07-20 — CDB1-EARLIER-MARKER" in material
    # cdb2 was routine this window: no Material for it, history included
    assert "Database history — cdb2" not in p
    assert "CDB2-EARLIER-MARKER" not in p


def test_a_notable_dbs_history_follows_its_digest_excerpt(escalated_wiki):
    journal_entry(escalated_wiki, "cdb1", "2026-07-20", "earlier")
    p = build_escalated_report_prompt(DAY, WINDOW, ESCALATED_INGESTED,
                                      escalated_wiki)
    excerpt = p.index("### Notable ingest — cdb1")
    history = p.index("### Database history — cdb1")
    assert excerpt < history < p.index("### Open incident: ")


def test_a_notable_db_without_a_digest_still_brings_its_history(tmp_path):
    root = tmp_path / "wiki"
    (root / "databases" / "cdb9" / "journal").mkdir(parents=True)
    (root / "databases" / "cdb9" / "journal" / "2026-07.md").write_text(
        "---\ntype: journal\ndb: cdb9\n---\n\n# cdb9 — journal 2026-07\n")
    journal_entry(root, "cdb9", "2026-07-20", "CDB9-MARKER")
    ingested = [{"db": "cdb9", "notable": True, "summary": "s"}]
    p = build_escalated_report_prompt(DAY, WINDOW, ingested, root)
    assert "Notable ingest" not in p
    assert "CDB9-MARKER" in p
    assert "nothing to show" not in p


def test_history_reads_the_codes_of_the_notable_dbs_excerpt(
        escalated_wiki, monkeypatch):
    from dbwiki import db_history
    seen = []
    real = db_history.gather

    def spy(root, db, **kw):
        seen.append((db, list(kw["codes"]), kw["today"].isoformat(),
                     kw["days"]))
        return real(root, db, **kw)

    monkeypatch.setattr(db_history, "gather", spy)
    build_escalated_report_prompt(DAY, WINDOW, ESCALATED_INGESTED,
                                  escalated_wiki, history_days=30)
    assert seen == [("cdb1", ["TNS-12543", "ORA-00600"], DAY, 30)]


def test_history_days_zero_removes_the_history_and_nothing_else(
        escalated_wiki):
    journal_entry(escalated_wiki, "cdb1", "2026-07-20", "earlier")
    full = build_escalated_report_prompt(DAY, WINDOW, ESCALATED_INGESTED,
                                         escalated_wiki)
    head, _, rest = full.partition("### Database history — cdb1\n")
    tail = rest.partition("\n### ")[2]
    assert head + "### " + tail == build_escalated_report_prompt(
        DAY, WINDOW, ESCALATED_INGESTED, escalated_wiki, history_days=0)


def test_escalated_rules_say_the_history_is_context_not_evidence():
    assert ('- A "Database history" section is what that database did on '
            "earlier days: use it to say whether this window is new, "
            "recurring, or follows a recorded change; never cite it as "
            "evidence for this window.") in structured._ESCALATED_RULES
...
def test_propose_report_escalated_contract_only_when_notable_dbs_given(monkeypatch):
    cfg = make_config(agents={"pi": {"cheap": "gemma", "strong": "gemma"}})
    escalate_seen = []
