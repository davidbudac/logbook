...
def two_linked_pages(wiki, other_body=""):
    (wiki / "a.md").write_text(page("a", "see [[b]]" + other_body))
    (wiki / "b.md").write_text(page("b", "see [[a]]"))
    git(wiki, "add", "-A")
    git(wiki, "commit", "-m", "linked")


def test_deleting_a_page_lints_the_pages_that_link_to_it(wiki, held):
    """The deleted page has nothing to lint; its broken links are all on the
    page that pointed at it, which linting the proposal's own paths never
    reads."""
    two_linked_pages(wiki)
    p = proposal(wiki, {"b.md": None})
    out = transaction.commit(wiki, p, lock=held, push=False)
    assert isinstance(out, LintBlocked)
    assert [(f.file, f.rule) for f in out.findings] == [("a.md",
                                                         "wikilink-broken")]
    assert (wiki / "b.md").exists()
    assert transaction.preview(wiki, p).blocked()


def test_a_linking_page_s_older_debt_does_not_block_a_deletion(wiki, held):
    two_linked_pages(wiki, " and [[long-gone]]")
    out = transaction.commit(wiki, proposal(wiki, {"b.md": None}), lock=held,
                             push=False)
    assert isinstance(out, LintBlocked)
    assert [(f.file, f.message) for f in out.findings] == [
        ("a.md", "[[b]] matches no page")]
    # once the linker is fixed in the same proposal, its old debt is its own
    # path's finding again, exactly as before
    p = proposal(wiki, {"b.md": None, "a.md": page("a", "unlinked")})
    assert isinstance(transaction.commit(wiki, p, lock=held, push=False),
